...
TL;DR: OTSurv是一个基于最优传输（OT）的多实例学习框架，用于全切片图像（WSIs）的生存预测，通过全局和局部约束显式捕捉病理异质性，显著提升了预测性能。
...
Result: 在零样本和真实世界动态视频深度基准测试中，StereoDiff表现最优，一致性和准确性显著提升。
...
TL;DR: 提出了一种名为ConViTac的视觉-触觉表示学习网络，通过对比嵌入条件机制（CEC）增强特征融合的对齐性，显著提升了材料分类和抓取预测任务的性能。
...
Method: 采用对比编码器预训练和跨模态注意力机制，将视觉和触觉输入投影到统一的潜在嵌入空间，实现特征融合。

Result: 实验表明，ConViTac在材料分类和抓取预测任务中比现有方法准确率提升高达12.0%。

Conclusion: ConViTac通过CEC机制有效提升了视觉-触觉特征融合的对齐性和下游任务性能。
...
<details>
  <summary>Details</summary>
Motivation: 评估和比较不同提示质量下多种医学图像分割模型的性能，填补现有研究的空白。

Method: 使用SAM、SAM 2、MedSAM、SAM-Med-3D和nnU-Net在BraTS 2023数据集上进行零样本推理，并比较不同提示（点和边界框）下的性能。随后对部分模型进行微调。

Result: SAM和SAM 2在高精度边界框提示下Dice分数达0.894和0.893，优于nnU-Net，但nnU-Net仍因实用性占优。微调后点提示性能显著提升，但仍不及边界框或nnU-Net。

Conclusion: 高精度提示下SAM系列表现优异，但nnU-Net因实用性仍是医学图像分割的首选。微调展示了未来研究的潜力。
...
Method: 比较了V-JEPA、Gemini Flash 2.0和HD-GCN在NUGGET数据集上的表现，评估其在动态全身手势识别中的效果。

Result: HD-GCN表现最佳，V-JEPA接近且简化了系统复杂性；Gemini在零样本设置中表现不佳。
...
TL;DR: 论文提出了一种利用视觉语言模型（VLM）从扩散模型生成的超分辨率（SR）图像中选择最可信样本的自动化框架，并提出了新的可信度评分（TWS）来量化SR的可靠性。
...
Method: 利用VLM（如BLIP-2、GPT-4o）通过结构化查询评估语义正确性、视觉质量和伪影存在，并引入TWS（基于CLIP嵌入、SSIM和小波分解）量化可靠性。
...
<details>
  <summary>Details</summary>
Motivation: 半监督领域泛化（SSDG）在标签稀缺时性能不佳，现有方法未显式正则化领域不变表示。

Method: 引入FixCLR，利用伪标签的类别信息和排斥项，适配对比学习以实现领域不变性。
...
TL;DR: 论文提出了一种名为COVER的向量对比学习框架，解决了传统对比学习在像素级表示中的过分散问题，通过向量回归建模特征距离，显著提升了像素级自监督预训练的性能。


<details>
  <summary>Details</summary>
Motivation: 传统对比学习（CL）在像素级表示中存在过分散问题，破坏了像素级特征相关性。为了解决这一问题，作者提出了向量对比学习（vector CL）的新范式。

Method: 论文提出了COVER框架，将对比学习重新定义为向量回归问题，通过建模特征距离来量化分散，并采用向量金字塔架构进行粒度适应。

Result: 在8个任务、2个维度和4种模态的广泛实验中，COVER显著提升了像素级自监督预训练的性能。

Conclusion: COVER框架通过向量回归和距离建模，有效解决了像素级表示中的过分散问题，推动了通用医学视觉基础模型的发展。
...
TL;DR: 提出MIDAS数据增强方法，通过软标签和视频帧混合提升动态面部表情识别（DFER）性能，尤其在模糊表情识别上表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决实际应用中模糊面部表情识别的问题，提升DFER性能。

Method: 提出MIDAS方法，通过软标签和视频帧的凸组合增强训练数据。
...
TL;DR: ThirdEye提出了一种显式利用单目深度线索的深度估计方法，通过预训练网络提供线索，并结合记忆模块和自适应变换器生成高分辨率视差图。


<details>
  <summary>Details</summary>
Motivation: 传统方法隐式学习深度信息，忽略了人类视觉系统依赖的显式线索（如遮挡边界、阴影和透视）。ThirdEye旨在显式利用这些线索。

Method: 通过预训练且冻结的专家网络提供显式线索，结合三阶段皮层层次结构和记忆模块加权融合线索，最后用自适应变换器生成视差图。

Result: ThirdEye继承了外部监督的优势，仅需少量微调，生成高分辨率视差图。

Conclusion: ThirdEye通过显式利用单目线索，提供了一种更高效的深度估计方法。
...
TL;DR: 论文提出了MultiHuman-Testbench基准，用于评估多人生成模型，包含1800个样本和5550张人脸图像，并提出多维度评估指标。


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏专门用于多人生成模型的基准，导致生成复杂动作时难以保留面部身份。

Method: 构建包含多样化文本提示和人脸图像的基准，提出基于分割和匈牙利匹配的新技术，并设计四项关键评估指标。

Result: 新基准和评估方法显著提升了ID相似性，为多人生成模型提供了标准化工具。

Conclusion: MultiHuman-Testbench为多人生成研究提供了重要见解和标准化工具，推动了该领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 探索几何约束和深度学习特征匹配如何改进立体视觉系统的深度重建。
...
TL;DR: 提出了一种名为FaSTA$^*$的高效神经符号代理，结合LLMs的快速高层规划和A$^*$搜索的精确工具使用，用于多轮图像编辑任务。通过归纳推理和子程序重用显著降低成本。


<details>
  <summary>Details</summary>
Motivation: 解决多轮图像编辑任务的高成本问题，结合快速规划和精确工具使用以提高效率。

Method: 结合LLMs的快速规划和A$^*$搜索的精确工具使用，通过归纳推理提取可重用的子程序。

Result: FaSTA$^*$在计算效率上显著优于现有方法，同时保持高成功率。

Conclusion: FaSTA$^*$是一种高效且成本优化的方法，适用于复杂图像编辑任务。
...
Method: M2SFormer在跳跃连接中统一多频率和多尺度注意力机制，利用全局上下文信息捕捉伪造痕迹，并通过全局先验图和难度引导注意力模块保留细微篡改。

Result: 在多个基准数据集上的实验表明，M2SFormer在伪造检测和定位方面优于现有最先进模型，具有更强的跨领域泛化能力。

Conclusion: M2SFormer通过创新的注意力机制和全局先验图设计，有效解决了现有方法的局限性，为图像伪造定位提供了更优的解决方案。
...
TL;DR: PhysRig提出了一种基于物理的可微分皮肤绑定框架，解决了传统线性混合皮肤绑定（LBS）的体积损失和变形不自然问题。


<details>
  <summary>Details</summary>
Motivation: 传统LBS方法在模拟弹性材料（如软组织、毛发等）时存在缺陷，无法生成物理合理的变形效果。

Method: PhysRig将刚性骨架嵌入体积表示（如四面体网格），通过连续介质力学和粒子离散化实现可微分模拟，并引入材料原型减少学习空间。

Result: 在合成数据集上，PhysRig优于传统LBS方法，生成更真实且物理合理的结果，并在姿态迁移任务中展示了其多功能性。

Conclusion: PhysRig为动画和物体建模提供了一种更真实、物理合理的皮肤绑定解决方案。
...
### [16] [Boosting Generative Adversarial Transferability with Self-supervised Vision Transformer Features](https://arxiv.org/abs/2506.21046)
...
Method: 设计了dSVA方法，利用生成器框架结合CL和MIM的特征，并通过自监督ViT的注意力机制训练生成器。

Result: 实验表明，CL和MIM的特征互补性显著提升了对抗样本的黑盒迁移能力，优于现有方法。

Conclusion: 自监督ViT的双重特征攻击方法dSVA在对抗样本生成中表现出卓越的泛化性能，为对抗攻击提供了新思路。
...
### [17] [AIR-VIEW: The Aviation Image Repository for Visibility Estimation of Weather, A Dataset and Benchmark](https://arxiv.org/abs/2506.20939)
...
<details>
  <summary>Details</summary>
Motivation: 航空天气的低成本替代方案需求增长，但缺乏适合监督学习的多样化、大规模公开数据集。

Method: 通过FAA天气摄像头网络进行为期一年的数据收集，构建新数据集，并测试三种常用方法和基线模型。

Result: 数据集适用于航空能见度估计，基准测试结果与ASTM标准进行了对比。

Conclusion: 新数据集为航空天气研究提供了重要资源，基准测试展示了其有效性。
...
### [18] [Hierarchical Sub-action Tree for Continuous Sign Language Recognition](https://arxiv.org/abs/2506.20947)
*Dejie Yang,Zhu Xu,Xinjie Gao,Yang Liu*

Main category: cs.CV

TL;DR: 提出HST-CSLR方法，通过层次化子动作树（HST）结合视觉与文本模态，提升连续手语识别性能。


<details>
  <summary>Details</summary>
Motivation: 现有连续手语识别（CSLR）因缺乏大规模数据集和精确标注导致性能受限，且现有方法未充分利用文本知识。

Method: 构建层次化子动作树（HST），结合大语言模型的文本知识，逐步对齐视觉与文本模态，并引入对比对齐增强。

Result: 在四个数据集（PHOENIX-2014等）上验证了HST-CSLR的有效性。

Conclusion: HST-CSLR通过更有效的文本知识利用和模态对齐，显著提升了CSLR性能。
...
TL;DR: OmniEval是一个用于评估全模态模型（如MiniCPM-O 2.6）的基准测试，涵盖视觉、听觉和文本输入。其特点包括全模态协作、视频多样性以及任务多样性和细粒度。


<details>
  <summary>Details</summary>
Motivation: 现有基准测试未能全面评估全模态模型的协作感知能力，因此需要一个新的基准测试来填补这一空白。

Method: 设计了包含810个音视频同步视频和2617个问答对的OmniEval基准测试，任务分为3大类12小类，并引入细粒度的视频定位任务Grounding。

Result: 实验在多个全模态模型上进行，展示了OmniEval的全面性和实用性。

Conclusion: OmniEval为评估全模态模型的上下文理解和构建能力提供了一个平台，代码和数据已公开。
...
<details>
  <summary>Details</summary>
Motivation: 传统病理学模型缺乏自然语言指令和文本上下文整合，现有MLLM在多图像理解、训练数据和自主诊断能力方面存在不足。

Method: 开发PathChat+，训练超过100万病理学指令样本和550万问答对，并推出SlideSeek系统进行多图像自主诊断。

Result: PathChat+在多个病理学基准测试中显著优于现有模型，SlideSeek在DDxBench上达到高准确率并生成可解释报告。

Conclusion: PathChat+和SlideSeek为病理学提供了更强大的多模态和自主诊断能力，填补了现有技术的空白。
...
<details>
  <summary>Details</summary>
Motivation: 现有视频编辑方法应用于Video DiTs时计算开销大，需要注意力修改或微调，DFVEdit旨在解决这一问题。

Method: 提出条件增量流向量（CDFV）和隐式交叉注意力（ICA）引导及嵌入强化（ER），统一编辑与采样流程。
...
TL;DR: Cradle2Cane提出了一种基于扩散模型的两阶段人脸老化框架，通过自适应噪声注入和身份感知嵌入解决了年龄-身份权衡问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法在实现真实且无缝的跨年龄段人脸老化时，难以平衡年龄准确性和身份一致性。
...
Conclusion: Cradle2Cane有效解决了人脸老化中的Age-ID权衡问题，为实际应用提供了更优的解决方案。
...
Method: 引入内外光度模型，提出完整光度模型和相机表示，结合深度正则化优化光度参数，构建包含场景辐射场和相机光度模型的完整映射。

Result: 实验表明，该方法能在成像退化条件下（如渐晕和污垢）实现高质量的3D场景表示。

Conclusion: 通过联合优化相机光度参数和场景表示，有效提升了3D场景表示的质量和鲁棒性。
...
Method: 扩展OpenPose为可学习的空间表示，并引入关键点概念学习，增强关键点嵌入的表达力和姿态对齐能力。

Result: 在动物和人类图像生成任务中，SP-Ctrl优于现有稀疏姿态引导方法，甚至媲美基于密集信号的方法，并展示了跨物种生成的潜力。

Conclusion: 稀疏信号在姿态引导图像生成中具有潜力，SP-Ctrl通过改进稀疏信号的可控性，为未来研究提供了新方向。
...
Method: 提出EVA框架，结合领域专家适应和语义变体对齐，实现高质量原始特征表示和精确组合泛化。

Result: 在三个流行基准测试中，EVA在封闭和开放世界设置下均显著优于其他先进方法。

Conclusion: EVA通过多专家和语义对齐有效解决了CZSL任务中的关键问题，展示了其优越性。
...
<details>
  <summary>Details</summary>
Motivation: 当前病理图像分割方法在临床应用中存在标注数据不足和类别定义狭窄的问题，限制了其效果。

Method: 提出PathSegmentor模型，利用自然语言提示进行语义分割，无需空间输入（如点或框）。同时构建了PathSeg数据集，包含275k图像-掩码-标签对，覆盖160个类别。

Result: PathSegmentor在Dice分数上显著优于现有空间和文本提示模型（分别提高0.145和0.429），并展现出更强的鲁棒性和泛化能力。

Conclusion: PathSegmentor不仅提升了分割精度和适用性，还通过特征重要性估计和生物标志物发现增强了诊断模型的可解释性，为精准肿瘤学提供了支持。
...
TL;DR: 提出TSDASeg模型，通过两阶段设计和直接跨模态对齐模块提升点云分割性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在点级任务（如分割）中表现不佳，因缺乏直接的3D-文本对齐。

Method: 采用两阶段模型，结合直接跨模态对齐模块和记忆模块，动态更新场景特征。

Result: 在多个3D数据集上实现最优性能。

Conclusion: TSDASeg通过直接对齐和动态记忆模块有效解决了交互分割中的不一致性问题。
...
TL;DR: 提出了一种逐步视频到音频生成方法，通过分步生成特定声音事件的音频轨道，模仿传统Foley工作流程，提升合成音频质量。


<details>
  <summary>Details</summary>
Motivation: 传统方法难以全面捕捉视频中的所有声音事件，且缺乏对多语义音频合成的支持。

Method: 采用分步视频到音频合成任务，结合目标文本提示和已生成音频轨道，利用预训练模型避免专用数据集需求。

Result: 实验表明，该方法能为单一视频生成多个语义不同的音频轨道，合成质量优于现有基线。

Conclusion: 该方法在视频到音频生成中实现了更全面的声音捕捉和更高的合成质量。
...
Method: 采用稀疏控制的高斯分布方法（Gaussian Splatting），从模糊视频中恢复清晰度并重建动态3D几何。
...
TL;DR: 提出了一种风格对齐的图像合成方法（SAIC），通过合成高质量病理图像提升异常细胞检测模型的性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 解决病理图像中高质量标注不足、长尾数据分布和染色风格不一致等问题。

Method: SAIC方法通过属性指导选择异常细胞库中的候选，利用高频特征重建实现风格对齐的高保真合成，并引入大视觉语言模型筛选高质量合成图像。
...
TL;DR: 本文提出逆向场景文本移除（ISTR），用于检测图像是否经过文本移除处理，并定位被移除的文本区域，实验证明其高准确性，同时尝试恢复移除的文本内容。
...
Method: 提出逆向STR（ISTR），通过二进制分类检测图像是否经过STR处理，并定位被移除的文本区域。同时训练文本识别器尝试恢复移除的文本内容。

Result: 实验表明，ISTR在检测和定位任务中具有高准确性，并能部分恢复移除的文本内容。
...
TL;DR: VisionGuard框架通过多阶段设计解决摩托车头盔违规检测中的环境多变性和数据不一致问题，提升检测性能。


<details>
  <summary>Details</summary>
Motivation: 摩托车头盔违规检测因环境多变、数据不一致等问题难以实现可靠检测，需改进现有方法。

Method: 提出VisionGuard框架，包含自适应标签模块和上下文扩展模块，分别优化分类一致性和数据不平衡问题。

Result: 实验显示VisionGuard比基线检测器mAP提升3.1%，验证其有效性。
...
TL;DR: 提出一种结合SAM和FFCL的深度学习框架，用于提高乳腺癌术中标本边缘评估的准确性和速度。
...
Method: 结合Segment Anything Model（SAM）和Forward-Forward Contrastive Learning（FFCL），通过预训练ResNet-18分类边缘状态，并重构二元掩码提示SAM进行精细分割。
...
Method: 提出无训练扩散方法，结合注意力混合和模拟老化正则化策略，平衡身份保留、年龄准确性和条件控制。

Result: 实验和用户研究表明，在身份保留、老化真实性和条件对齐方面表现优于现有方法。
...
### [35] [Multimodal Prompt Alignment for Facial Expression Recognition](https://arxiv.org/abs/2506.21017)
...
Method: 采用多粒度硬提示生成策略，利用大语言模型（如ChatGPT）生成详细表情描述，并通过最小化软提示与硬提示的特征差异注入外部知识。同时，结合原型引导的视觉特征对齐和跨模态全局-局部对齐模块，优化文本与视觉特征的对齐。

Result: 在三个面部表情识别基准数据集上优于现有方法，同时保持了预训练模型的泛化能力并降低了计算成本。
...
### [36] [User-in-the-Loop View Sampling with Error Peaking Visualization](https://arxiv.org/abs/2506.21009)
*Ayaka Yasunaga,Hideo Saito,Shohei Mori*

Main category: cs.CV

TL;DR: 论文提出了一种基于局部重建光场和可视化误差的方法，用于增强现实中的新视角合成，减少了用户负担并扩展了场景探索范围。


<details>
  <summary>Details</summary>
Motivation: 现有方法需要用户通过AR显示对齐3D注释来采集数据，任务繁重且限制了场景范围。

Method: 使用局部重建光场和可视化误差，引导用户插入新视角以消除误差。

Result: 误差峰值可视化方法侵入性更低，减少了用户失望，且需要更少的视角样本。
...
### [37] [Bridging Video Quality Scoring and Justification via Large Multimodal Models](https://arxiv.org/abs/2506.21011)
*Qizhi Xie,Kun Yuan,Yunpeng Qu,Jiachao Gong,Mingda Wu,Ming Sun,Chao Zhou,Jihong Zhu*

Main category: cs.CV

TL;DR: 论文提出了一种基于评分的指令生成（SIG）方法，用于视频质量评估（VQA），通过自动化生成指令数据，解决了传统方法依赖人工标注和专有系统的问题。


<details>
  <summary>Details</summary>
Motivation: 传统VQA方法仅生成数值评分，无法描述视频质量的复杂维度，限制了其应用。通过语言输出和指令调优，视频大型多模态模型（LMMs）有望解决这一问题。

Method: 提出SIG方法，通过评分和层次化思维链（CoT）生成指令数据，自动化流程减少对人工标注的依赖，并构建了Score2Instruct（S2I）数据集。

Result: 实验表明，SIG方法在多基准测试中显著提升了视频LMMs的质量评分和解释能力。

Conclusion: SIG方法为视频质量评估提供了高效、可扩展的解决方案，并通过S2I数据集和基准测试推动了视频LMMs的发展。
...
### [38] [Improving Diffusion-Based Image Editing Faithfulness via Guidance and Scheduling](https://arxiv.org/abs/2506.21045)
*Hansam Cho,Seoung Bum Kim*

Main category: cs.CV

TL;DR: 提出FGS方法，通过忠实度引导和调度策略，在图像编辑中平衡可编辑性和忠实度，实现高质量编辑。
...
Method: 引入忠实度引导（Faithfulness Guidance）和调度策略（Scheduling），增强输入图像信息的保留，并解决可编辑性与忠实度的错位问题。

Result: 实验表明，FGS在保持可编辑性的同时显著提升了忠实度，且兼容多种编辑方法。

Conclusion: FGS是一种有效的方法，能够在多样化的编辑任务中实现精确且高质量的图像编辑。
...
TL;DR: 提出了一种名为FedSC的联邦学习方法，通过语义感知协作解决数据异构性问题，利用关系原型和一致性原型捕获客户端特定和类别相关知识。


<details>
  <summary>Details</summary>
Motivation: 联邦学习中数据异构性问题（如客户端标签偏好）影响模型性能，现有方法常忽略客户端内部的语义信息。

Method: FedSC通过构建关系原型和一致性原型，结合对比学习和差异聚合策略，优化本地模型。
...
TL;DR: 论文提出了一种结合经典与量子计算的生成对抗网络（GAN），用于生成彩色医学图像，解决了现有量子方法只能生成低质量灰度图像的局限。


<details>
  <summary>Details</summary>
Motivation: 皮肤疾病检测中，机器学习模型需要大量高质量数据，但现有数据集存在类别不平衡、隐私问题和对象偏差等问题，数据增强成为关键。量子计算虽具潜力，但现有方法只能生成低质量灰度图像。

Method: 通过一种新颖的经典-量子潜在空间融合技术，开发了首个能够生成彩色医学图像的经典-量子GAN。

Result: 该模型在图像生成质量和分类性能提升上优于经典深度卷积GAN和现有混合经典-量子GAN，且参数和训练周期大幅减少。

Conclusion: 结果表明，随着量子硬件的发展，量子图像生成具有广阔前景，并在真实IBM量子机器上验证了模型的鲁棒性。
...
TL;DR: EgoAdapt框架通过跨模态蒸馏和策略学习，显著降低了多模态感知任务的计算成本，同时保持或超越现有模型的性能。


<details>
  <summary>Details</summary>
Motivation: 现代感知模型在多模态任务中性能优异但计算成本高，难以在资源受限环境中部署。

Method: 提出EgoAdapt框架，结合跨模态蒸馏和策略学习，适应不同任务的动作空间。

Result: 在三个数据集上测试，计算量减少89.09%，参数减少82.02%，能耗降低9.6倍，性能与或优于现有模型。

Conclusion: EgoAdapt是一种高效且通用的解决方案，适用于资源受限环境中的多模态感知任务。
...
Method: 使用单特征级融合单元和注意力引导自调制特征融合模块（ASFF），结合轻量级特征注意力变换模块（FATM）。

Result: 在三个数据集上，参数和计算成本分别减少90%和85%，检测精度提升1%-3%。

Conclusion: LASFNet在效率和精度之间取得了良好平衡，代码已开源。
...
TL;DR: 提出了一种1D二进制图像潜在表示方法，显著减少了图像建模的token数量，同时保持了高分辨率细节，提升了训练和推理效率。


<details>
  <summary>Details</summary>
Motivation: 减少高分辨率图像建模的计算需求，提高图像和多模态理解的效率。

Method: 引入1D二进制图像潜在空间，将图像表示为二进制向量序列，替代传统的一热编码token。

Result: 仅需128个离散token即可处理1024x1024图像，token数量减少32倍，训练和推理速度显著提升。
...
### [44] [IPFormer-VideoLLM: Enhancing Multi-modal Video Understanding for Multi-shot Scenes](https://arxiv.org/abs/2506.21116)
...
### [45] [DidSee: Diffusion-Based Depth Completion for Material-Agnostic Robotic Perception and Manipulation](https://arxiv.org/abs/2506.21034)
*Wenzhou Lyu,Jialing Lin,Wenqi Ren,Ruihao Xia,Feng Qian,Yang Tang*

Main category: cs.CV

TL;DR: DidSee是一个基于扩散模型的深度补全框架，针对非朗伯物体，通过改进噪声调度器和训练策略，结合语义增强器，显著提升了深度补全性能。


<details>
  <summary>Details</summary>
Motivation: 商业RGB-D相机在非朗伯物体上生成的深度图通常噪声大且不完整，传统方法因训练数据有限而泛化能力差。扩散模型虽能提供视觉先验，但存在训练-推理偏差问题。

Method: 提出DidSee框架：1) 采用零终端信噪比的噪声调度器消除信号泄漏偏差；2) 设计噪声无关的单步训练策略减少误差累积；3) 引入语义增强器联合深度补全与语义分割。

Result: 在多个基准测试中达到最优性能，表现出强大的现实泛化能力，并有效提升下游任务（如类别级姿态估计和机器人抓取）的表现。
...
### [46] [Boosting Domain Generalized and Adaptive Detection with Diffusion Models: Fitness, Generalization, and Transferability](https://arxiv.org/abs/2506.21042)
...
Method: 提取单步扩散过程的中间特征，构建目标中心辅助分支，应用一致性损失对齐分支，并在统一框架中指导标准检测器。
...
### [47] [Robust Deep Learning for Myocardial Scar Segmentation in Cardiac MRI with Noisy Labels](https://arxiv.org/abs/2506.21151)
...
Method: 通过微调先进模型，采用Kullback-Leibler损失和大量数据增强，解决标签噪声和数据异质性问题。
...
### [48] [Task-Aware KV Compression For Cost-Effective Long Video Understanding](https://arxiv.org/abs/2506.21184)
*Minghao Qin,Yan Shu,Peitian Zhang,Kun Lun,Huaying Yuan,Juenjie Zhou,Shitao Xiao,Bo Zhao,Zheng Liu*

Main category: cs.CV

TL;DR: Video-X^2L通过双层KV压缩和选择性KV重加载，高效解决长视频理解中的计算成本问题，无需额外训练且兼容现有MLLMs。


<details>
  <summary>Details</summary>
Motivation: 长视频理解（LVU）对多模态大语言模型（MLLMs）计算成本过高，现有KV压缩方法在高压缩比下信息损失严重。

Method: 提出Video-X^2L，包含双层KV压缩（生成低压缩和高压缩KV）和选择性KV重加载（解码阶段动态选择KV）。

Result: 在多个LVU基准测试中表现优异，显著优于现有KV压缩方法并节省计算成本。

Conclusion: Video-X^2L简单高效，无需额外训练，兼容现有模型，为LVU任务提供实用解决方案。
...
### [49] [Class-Agnostic Region-of-Interest Matching in Document Images](https://arxiv.org/abs/2506.21055)
...
<details>
  <summary>Details</summary>
Motivation: 现有文档分析解决方案无法满足用户自定义需求，因此需要一种灵活、多粒度、开放式的匹配方法。

Method: 使用视觉提示输入参考文档和目标文档图像，通过孪生网络提取多级特征，并利用交叉注意力层对齐语义。

Result: 实验表明，提出的方法在RoI-Matching-Bench上有效，可作为进一步研究的基线。
...
Method: 采用强化学习，结合上下文奖励、格式奖励、准确性奖励和逻辑奖励，提升模型对多模态信息的理解和推理能力。

Result: 提出的方法在多模态基准测试中表现优于其他开源模型。

Conclusion: 通过强化学习和多模态奖励机制，显著提升了模型的推理能力和多模态理解效果。
...
### [51] [BitMark for Infinity: Watermarking Bitwise Autoregressive Image Generative Models](https://arxiv.org/abs/2506.21209)
*Louis Kerner,Michel Meintz,Bihe Zhao,Franziska Boenisch,Adam Dziedzic*

Main category: cs.CV

TL;DR: BitMark是一种用于Infinity模型的鲁棒位级水印框架，旨在防止模型崩溃，通过在多尺度嵌入水印并保持生成速度和视觉保真度。


<details>
  <summary>Details</summary>
Motivation: 随着文本到图像模型生成的图像在互联网上泛滥，这些图像可能被重新用作训练数据，导致模型崩溃。水印技术可以识别生成内容，从而缓解这一问题。

Method: BitMark在Infinity的图像生成过程中，直接在位级别嵌入水印，覆盖多个分辨率，同时保持视觉质量和生成速度。

Result: BitMark水印具有高放射性，即使生成的图像被用于训练其他模型，水印仍可检测到。

Conclusion: BitMark为图像生成模型提供了一种可靠的水印解决方案，有效防止模型崩溃。
...
### [52] [SAMURAI: Shape-Aware Multimodal Retrieval for 3D Object Identification](https://arxiv.org/abs/2506.21056)
*Dinh-Khoi Vo,Van-Loc Nguyen,Minh-Triet Tran,Trung-Nghia Le*

Main category: cs.CV

TL;DR: SAMURAI方法通过结合CLIP语义匹配和形状引导重排，解决了在复杂室内环境中仅用2D图像和自然语言检索3D对象的挑战。


<details>
  <summary>Details</summary>
Motivation: 在ROOMELSA挑战中，缺乏完整3D场景信息导致对象外观、几何和语义推理困难，需解决视角扭曲、无纹理区域和模糊语言提示等问题。
...
### [53] [PoseMaster: Generating 3D Characters in Arbitrary Poses from a Single Image](https://arxiv.org/abs/2506.21076)
...
<details>
  <summary>Details</summary>
Motivation: 现有基于图像的方法在姿势标准化和3D重建阶段容易因自遮挡和视角问题生成失真图像，影响重建质量。

Method: 提出基于流的3D原生生成框架，利用角色骨骼作为姿势条件，并通过随机清空条件训练提高控制效果。
...
TL;DR: 提出了一种针对希腊多调文本的OCR系统，结合卷积层和循环层的优势，显著提升了识别准确率和效率。
...
### [55] [ESMStereo: Enhanced ShuffleMixer Disparity Upsampling for Real-Time and Accurate Stereo Matching](https://arxiv.org/abs/2506.21091)
*Mahmoud Tahmasebi,Saif Huq,Kevin Meehan,Marion McAfee*

Main category: cs.CV

TL;DR: 论文提出了一种增强型Shuffle Mixer（ESM）方法，通过整合主特征到视差上采样单元，解决了小规模成本体积信息不足的问题，实现了高精度实时立体匹配。


<details>
  <summary>Details</summary>
Motivation: 立体匹配在自动驾驶系统中至关重要，但现有深度学习模型难以同时实现高精度和实时性能。大规模成本体积计算量大，小规模则信息不足。
...
Method: 提出了HalluSegBench，包含1340个反事实实例对和新的量化指标，用于评估视觉连贯场景编辑下的幻觉敏感性。
...
### [57] [Holistic Surgical Phase Recognition with Hierarchical Input Dependent State Space Models](https://arxiv.org/abs/2506.21330)
...
TL;DR: 提出一种基于状态空间模型的分层输入依赖方法，用于高效分析长时手术视频，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决长时手术视频分析中传统Transformer模型因二次注意力机制导致效率低下的问题。

Method: 结合局部聚合和全局关系状态空间模块，采用混合离散-连续监督策略训练模型。

Result: 在多个数据集上显著超越现有方法（Cholec80 +2.8%，MICCAI2016 +4.3%，Heichole +12.9%）。

Conclusion: 提出的方法在长时手术视频分析中高效且性能优越，代码将公开。
...
TL;DR: 本文提出了一种名为OracleFusion的两阶段语义排版框架，用于解决甲骨文（OBS）未解读字符的挑战。该方法结合多模态大语言模型（MLLM）和空间感知推理（SAR）分析字形结构，并通过Oracle Structural Vector Fusion（OSVF）生成语义丰富的矢量字体。实验表明，OracleFusion在语义、视觉和字形保持方面优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 甲骨文是古代文明的重要记录，但未解读字符的结构复杂且抽象，解读难度大。本文旨在通过技术手段提升甲骨文的解读效率和准确性。

Method: 提出两阶段框架OracleFusion：第一阶段利用MLLM和SAR分析字形结构；第二阶段通过OSVF生成语义丰富的矢量字体，确保字形结构的完整性。

Result: OracleFusion在语义、视觉和字形保持方面优于基线模型，显著提升了可读性和美观性，并为未解读字符提供专家级见解。

Conclusion: OracleFusion是一种有效的工具，可推动甲骨文的解读工作，具有重要的文化和学术价值。
...
### [59] [CA-I2P: Channel-Adaptive Registration Network with Global Optimal Selection](https://arxiv.org/abs/2506.21364)
*Zhixin Cheng,Jiacheng Deng,Xinjun Li,Xiaotian Yin,Bohao Liao,Baoqun Yin,Wenfei Yang,Tianzhu Zhang*

Main category: cs.CV

TL;DR: 论文提出了一种改进图像到点云配准的方法，通过引入通道自适应调整模块（CAA）和全局最优选择模块（GOS）来解决特征通道注意力差异和冗余对应问题。


<details>
  <summary>Details</summary>
Motivation: 现有检测自由方法在图像和点云特征匹配时存在特征通道注意力差异和冗余对应问题，导致配准精度下降。
...
### [60] [Pushing Trade-Off Boundaries: Compact yet Effective Remote Sensing Change Detection](https://arxiv.org/abs/2506.21109)
...
<details>
  <summary>Details</summary>
Motivation: 现代深度学习模型在变化检测中计算复杂且资源消耗大，但精度提升有限，因此研究轻量高效模型以满足卫星实时处理需求。
...
<details>
  <summary>Details</summary>
Motivation: 在动态3D环境中，实时更新场景表示对机器人、混合现实和具身AI至关重要，但现有方法存在计算开销大的问题。

Method: 提出CL-Splats，通过稀疏场景捕获增量更新高斯溅射表示，结合变化检测模块实现局部优化，并支持场景状态存储与恢复。
...
### [62] [TITAN: Query-Token based Domain Adaptive Adversarial Learning](https://arxiv.org/abs/2506.21484)
*Tajamul Ashraf,Janibul Bashir*

Main category: cs.CV

TL;DR: 论文提出了一种名为TITAN的方法，用于解决源数据不可用的领域自适应目标检测问题，通过分离目标图像为易/难子集并利用查询-令牌对抗模块提升性能。
...
Method: 提出TITAN方法，将目标图像分为易/难子集，并引入查询-令牌对抗模块以减少领域差异。

Result: 在多个数据集上验证，TITAN性能优于现有方法，mAP提升显著。

Conclusion: TITAN通过改进伪标签生成和减少领域差异，有效提升了源数据不可用时的目标检测性能。
...
### [63] [GoIRL: Graph-Oriented Inverse Reinforcement Learning for Multimodal Trajectory Prediction](https://arxiv.org/abs/2506.21121)
*Muleilan Pei,Shaoshuai Shi,Lu Zhang,Peiliang Li,Shaojie Shen*

Main category: cs.CV

TL;DR: 提出了一种基于图导向逆强化学习（GoIRL）的轨迹预测框架，结合向量化上下文表示和最大熵IRL，显著提升了预测性能。
...
Method: 采用图导向逆强化学习框架，通过特征适配器整合车道图特征，结合最大熵IRL推断奖励分布，并利用分层参数化轨迹生成器优化预测。

Result: 在Argoverse和nuScenes基准测试中达到最优性能，且泛化能力优于现有监督模型。

Conclusion: GoIRL框架有效解决了轨迹预测的挑战，展示了其在自动驾驶中的潜力。
...
### [64] [Learning to See in the Extremely Dark](https://arxiv.org/abs/2506.21132)
*Hai Jiang,Binhao Guan,Zhen Liu,Xiaohong Liu,Jian Yu,Zheng Liu,Songchen Han,Shuaicheng Liu*

Main category: cs.CV

TL;DR: 论文提出了一种用于极低光照RAW图像增强的扩散框架和数据集SIED，填补了现有数据集的空白。


<details>
  <summary>Details</summary>
Motivation: 现有学习方法在极低光照（0.0001 lux）场景下的能力尚未探索，缺乏对应数据集。

Method: 提出数据合成管道生成极低光照RAW图像，并设计扩散框架，结合AICM模块和颜色一致性损失。

Result: 在SIED和公开基准上验证了方法的有效性。

Conclusion: 该方法在极低光照图像增强中表现优异，代码和数据集已开源。
...
### [65] [YOLO-FDA: Integrating Hierarchical Attention and Detail Enhancement for Surface Defect Detection](https://arxiv.org/abs/2506.21135)
*Jiawei Hu*

Main category: cs.CV

TL;DR: YOLO-FDA是一种基于YOLO的检测框架，通过细粒度细节增强和注意力引导特征融合，解决了工业表面缺陷检测中的多尺度问题和特征冗余问题。
...
Method: YOLO-FDA采用BiFPN架构增强多级特征聚合，引入DDFM模块捕获细粒度结构变化，并提出AC和CAF两种注意力融合策略提升上下文表示和减少特征噪声。
...
### [66] [Tree-based Semantic Losses: Application to Sparsely-supervised Large Multi-class Hyperspectral Segmentation](https://arxiv.org/abs/2506.21150)
*Junwen Wang,Oscar Maccormac,William Rochford,Aaron Kujawa,Jonathan Shapey,Tom Vercauteren*

Main category: cs.CV

TL;DR: 论文提出两种基于树结构的语义损失函数，利用标签的层次结构改进高光谱图像（HSI）分割性能，并在稀疏标注数据集上达到SOTA效果。


<details>
  <summary>Details</summary>
Motivation: 现有学习方法对生物医学分割任务中的错误均等惩罚，未能利用标签空间的类间语义关系。

Method: 引入两种基于树结构的语义损失函数，结合稀疏标注训练方法。

Result: 在包含107个类别的稀疏标注HSI数据集上达到SOTA性能，同时有效检测OOD像素。

Conclusion: 提出的方法显著提升了分割性能，同时支持OOD检测，适用于临床定义的语义树结构。
...
### [67] [Whole-Body Conditioned Egocentric Video Prediction](https://arxiv.org/abs/2506.21552)
*Yutong Bai,Danny Tran,Amir Bar,Yann LeCun,Trevor Darrell,Jitendra Malik*

Main category: cs.CV

TL;DR: 训练模型通过人体动作预测第一人称视角视频，结合姿态轨迹和扩散变换器，评估其预测和控制能力。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过人体动作模拟第一人称视角下的环境变化，解决复杂现实环境和行为建模的挑战。

Method: 使用基于3D姿态轨迹的扩散变换器模型，在Nymeria数据集上进行训练，并设计分层评估协议。

Result: 模型能够从第一人称视角预测环境变化，展示了在复杂环境中的预测和控制能力。

Conclusion: 该研究为从人类视角建模复杂环境和行为提供了初步尝试，具有潜在的应用价值。
...
Result: 实验显示方法在新视角合成和3D重建上优于现有方法。

Conclusion: 方法实现了高保真、一致的3D物体生成。
...
TL;DR: 论文提出了一种名为TAM的新框架，用于解决3D点云数据的Sim2Real领域适应问题，通过全局拓扑建模和自监督学习任务提升泛化能力。


<details>
  <summary>Details</summary>
Motivation: 由于数据采集方法不同，3D点云数据存在显著的几何变化，导致Sim2Real领域差距，限制了点分类器的泛化能力。现有UDA方法因缺乏鲁棒的领域不敏感描述符而表现不佳。

Method: 提出TAM框架，利用全局空间拓扑和局部几何特征的拓扑关系，结合自监督学习任务和自训练策略，减少领域差距。
...
TL;DR: 论文提出了一种针对3D语义占用预测中的分布外（OoD）检测方法，通过合成异常数据集和OccOoD框架，显著提升了OoD检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法在分布外场景和长尾分布中表现不佳，存在安全隐患，因此需要改进OoD检测能力。

Method: 提出合成异常集成管道生成数据集（VAA-KITTI和VAA-KITTI-360），并开发OccOoD框架，结合Voxel-BEV渐进融合（VBPF）和RWKV分支增强检测。

Result: OccOoD在1.2m区域内OoD检测性能达到AuROC 67.34%和AuPRCr 29.21%，同时保持竞争力的占用预测表现。

Conclusion: 论文提出的方法和数据集有效解决了3D语义占用预测中的OoD问题，为自动驾驶安全提供了新工具。
...
<details>
  <summary>Details</summary>
Motivation: 现有3DVG方法无法有效处理多步文本指令中的代词和上下文信息，导致在SG3D任务中表现不佳。

Method: 提出GroundFlow模块，选择性提取短期和长期历史信息，增强对上下文的理解。

Result: 在SG3D基准测试中，GroundFlow显著提升基线方法性能（+7.5%和+10.2%），优于预训练的3D大语言模型。

Conclusion: GroundFlow为3DVG模型引入时序推理能力，在SG3D任务中实现最先进性能。
...
Method: 提出UNLOCK框架，包含Omni伪标签学习和Amodal驱动上下文学习模块，无需源数据或目标标签。

Result: 在实验中，UNLOCK性能接近依赖源数据的方法，mAAP和mAP分别达到10.9和11.6，mAPQ提升+4.3。

Conclusion: UNLOCK为全景图像分割提供了一种无需源数据的实用解决方案，性能优越且开源。
...
TL;DR: MedPrompt是一个统一的医学图像分析框架，结合了大型语言模型和模块化卷积神经网络，支持用户自定义任务，无需重新训练整个系统。
...
Method: 使用大型语言模型（Llama-4-17B）进行任务规划，模块化卷积神经网络（DeepFusionLab）进行图像处理，动态路由任务特定权重。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖预训练模型或生成合成数据，性能受限于模型能力或参考集质量，本文关注数据质量问题。

Method: 设计了一个数据质量导向的框架，包括构建高质量参考集和基于相似性的检索方法。
...
<details>
  <summary>Details</summary>
Motivation: 将单目RGB视频直接转换为机械臂的可执行轨迹，简化了传统方法中复杂的多步骤流程。

Method: 包含四个模块：1) 通过ROMP估计3D骨架；2) 使用HPSTM（序列到序列Transformer）平滑轨迹；3) 根据不确定性估计过滤噪声；4) 几何重定向层映射到机械臂工作空间。

Result: 实现了从视频到机械臂轨迹的高效转换，同时保持骨骼长度和解剖合理性。

Conclusion: ESFP提供了一种高效且低成本的方法，适用于桌面机械臂的运动规划。
...
TL;DR: DiMPLe是一种新的多模态提示学习方法，通过解耦不变和伪特征提升分布外性能。


<details>
  <summary>Details</summary>
Motivation: 解决视觉数据中伪相关性问题，提升多模态学习的泛化能力和鲁棒性。

Method: 结合互信息最小化、伪特征正则化和对比学习三个目标。

Result: 在11个数据集上表现优于CoOp-OOD，基础类和新类准确率分别提升15.27和44.31。

Conclusion: DiMPLe在多模态学习中有效解耦特征，显著提升性能。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有方法（CIOD和DIOD）无法同时处理类别和域增量学习的问题，提升目标检测系统在真实场景中的适应性。

Method: 提出DuET框架，基于任务算术和方向一致性损失，实现无示例的增量学习，适用于多种检测器。

Result: 在Pascal Series和Diverse Weather Series上，DuET分别实现了+13.12%和+11.39%的RAI提升，同时保持了高平均保留指数。

Conclusion: DuIOD和DuET在类别和域增量学习中表现出色，为实时目标检测提供了更实用的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 视频虚拟试穿任务需要同时保证时空一致性和服装细节保留，现有方法（如逐帧图像试穿或基于扩散的方法）存在不一致问题。

Method: 将视频虚拟试穿建模为条件视频修复任务，基于Diffusion Transformer构建3D时空注意力框架，采用多阶段训练和掩码策略。

Result: 实验表明，ViTI在时空一致性和细节保留上优于现有方法。

Conclusion: ViTI通过视频生成视角解决了试穿任务，为视频虚拟试穿提供了新思路。
...
TL;DR: 论文提出了一种新的框架TIA和混合参数高效微调方法WordCon，用于解决生成图像中单词级排版控制的挑战。
...
Conclusion: 方法在多个任务中表现优异，数据集和代码将公开。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在手术视频生成中缺乏对手术动作和阶段的语义一致性，需要更精细的引导。

Method: HieraSurg结合手术阶段和初始帧，先预测粗粒度语义变化，再通过第二阶段模型生成视频，整合多级手术信息。

Result: 实验证明HieraSurg在胆囊切除术视频生成中表现优异，支持高帧率生成，且对现有分割图有精细适配。

Conclusion: HieraSurg展示了在手术视频生成中的潜力，为实际应用提供了可能。
...
TL;DR: 提出了一种名为CoSMAE的新型持续自监督学习方法，通过数据混合和模型混合知识蒸馏减少灾难性遗忘，并在遥感领域显著优于现有方法。
...
TL;DR: 本文提出了一种名为\drishtikon的多粒度视觉定位框架，用于提升复杂多语言文档的可解释性和信任度，结合了多语言OCR、大语言模型和区域匹配算法。
...
Method: 整合多语言OCR、大语言模型和新型区域匹配算法，支持块、行、词和点级别的答案定位。
...
TL;DR: 论文提出了一种通过结合人体关键点与传统视觉特征生成专门视觉-语言指令数据的方法，显著提升了模型在人体姿态和动作理解任务上的性能。


<details>
  <summary>Details</summary>
Motivation: 当前视觉-语言模型在通用视觉理解任务上表现良好，但在涉及人体姿态和动作的复杂任务上表现不足，主要缺乏专门的视觉-语言指令数据。

Method: 通过整合人体关键点与标题、边界框等传统视觉特征，构建了一个包含200,328个样本的数据集，用于微调模型。同时提出了E-HPAUB基准来评估模型性能。

Result: 微调后的LLaVA-Pose模型在E-HPAUB基准上相比原始LLaVA-1.5-7B模型整体提升了33.2%。

Conclusion: 研究表明，结合关键点的数据能有效增强多模态模型在人体中心视觉理解任务上的表现。
...
TL;DR: PanSt3R提出了一种联合预测3D几何和多视角全景分割的统一方法，避免了现有方法依赖2D分割和测试时优化的缺点，性能优越且速度快。
...
Method: 基于MUSt3R框架，结合语义感知和多视角全景分割能力，改进掩码合并方法，并引入新视角预测技术。
...
<details>
  <summary>Details</summary>
Motivation: 电磁逆散射问题（EISP）在医学成像等应用中至关重要，但现有机器学习方法（如Img-Interiors）存在泛化能力差、对稀疏发射器设置敏感等问题。
...
TL;DR: ShotBench是一个专为电影语言理解设计的基准测试，揭示了现有视觉语言模型在电影语法理解上的不足，并提出了ShotQA数据集和ShotVL模型以提升性能。
...
Method: 通过构建ShotBench基准测试和ShotQA数据集，并利用监督微调和Group Relative Policy Optimization开发ShotVL模型。

Result: ShotVL在ShotBench上显著优于现有模型，达到最新技术水平。

Conclusion: 开源模型、数据和代码以推动AI在电影理解和生成领域的发展。
...
<details>
  <summary>Details</summary>
Motivation: 解决现有场景图数据缺乏精确标注的问题。

Method: 提出CoPa-SG数据集，包含参数化关系和原型关系，用于比较不同场景图生成模型。

Result: 展示了新关系类型如何增强下游应用的规划和推理能力。

Conclusion: CoPa-SG为场景理解提供了更丰富的数据支持和新关系类型。
...
Method: 通过用户点击车辆特征点，结合几何约束优化（PnP和最小二乘子问题）和概率尺寸先验，估计车辆位置、方向和尺寸。

Result: 在KITTI和Cityscapes3D数据集上验证，证明方法高效且可扩展。
...
TL;DR: GenFlow是一个模块化框架，通过节点编辑器和自然语言处理助手，降低了生成艺术的复杂性，使其更易用。


<details>
  <summary>Details</summary>
Motivation: 生成艺术需要高技术门槛，限制了其广泛应用。GenFlow旨在降低这一门槛。

Method: GenFlow采用节点编辑器实现定制化，结合自然语言处理助手简化工作流。

Result: 用户研究表明，GenFlow优化了工作流，缩短任务时间，提升用户体验。
...
TL;DR: FastRef是一种高效的原型精炼框架，用于解决少样本工业异常检测（FS-IAD）中的原型代表性问题。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常忽略查询图像统计信息，导致原型代表性不足。FastRef旨在通过迭代两阶段过程提升原型质量。

Method: FastRef通过特征转移和异常抑制两阶段迭代优化原型：1）通过可优化变换矩阵将查询特征转移到原型；2）通过原型对齐抑制异常。
...
Conclusion: 该方法通过直接优化参数曲线，显著提升了效率和性能，为3D曲线重建提供了新思路。
...
TL;DR: XVerse模型通过将参考图像转换为偏移量，实现对多主体图像生成的精确独立控制，提升编辑性和一致性。


<details>
  <summary>Details</summary>
Motivation: 解决现有文本到图像生成方法在多主体控制中导致的编辑性差和属性纠缠问题。
...
<details>
  <summary>Details</summary>
Motivation: 内窥镜场景中的非朗伯表面和呼吸引起的动态运动会影响SLAM系统的性能，现有方法仅依赖外观约束，效果有限。

Method: 引入光流损失作为几何约束，提出深度正则化策略，并改进3DGS细化策略以优化关键帧的渲染质量。

Result: 在C3VD静态数据集和StereoMIS动态数据集上，方法在视图合成和姿态估计方面优于现有技术。

Conclusion: 结合几何约束和深度正则化的3DGS方法在内窥镜场景中表现出高效性和鲁棒性，代码将公开。
...
TL;DR: HyperSORT框架通过超网络预测UNet参数，解决医学影像数据中的异质偏差问题，并在实验中验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 医学影像数据常存在异质偏差（如错误标签或不一致的标注风格），影响深度学习分割网络的性能，但识别和表征这些偏差具有挑战性。

Method: HyperSORT使用超网络从表示图像和标注变异的潜在向量预测UNet参数，联合学习超网络参数和训练集中每个样本的潜在向量集合。

Result: 在3D腹部CT数据集（AMOS和TotalSegmentator）上验证，HyperSORT能够识别系统性偏差和错误样本，并生成结构化映射。

Conclusion: HyperSORT通过学习UNet参数的复杂分布，有效处理数据偏差，提升分割任务的鲁棒性。
...
TL;DR: 该研究通过深度学习方法对乳腺癌中的非典型有丝分裂（AMF）进行分类，并引入新的数据集进行基准测试，结果显示低秩适应（LoRA）方法表现最佳。


<details>
  <summary>Details</summary>
Motivation: 非典型有丝分裂是肿瘤恶性的独立预后标志物，但其识别因低发生率、形态差异小、病理学家间一致性低及数据集类别不平衡而具有挑战性。

Method: 研究比较了多种深度学习方法，包括基线模型、线性探测的基础模型和低秩适应（LoRA）微调的基础模型，并引入两个新的AMF数据集进行验证。

Result: 在多个数据集上，LoRA方法表现最佳，平衡准确率分别达到0.8135、0.7696和0.7705。
...
TL;DR: 本文提出了一种专门用于地下矿工检测的热成像数据集，为开发可靠的矿工检测系统奠定了基础。
...
Method: 系统采集了多种采矿活动和场景的热成像数据，并评估了多种先进的目标检测算法（如YOLOv8、YOLOv10、YOLO11和RT-DETR）。
...
TL;DR: 论文提出了一种基于低频信号的新方法LF-CFG，通过自适应阈值减少冗余信息，缓解分类器自由引导（CFG）中的过饱和和不真实伪影问题。
...
Conclusion: LF-CFG通过低频信号优化，显著改善了分类器自由引导的性能，为扩散模型提供了更稳定的生成效果。
...
TL;DR: 该论文提出了动态、静态和混合三种基于转向运动计数（TMC）的交通信号配置方法，并通过实验验证了不同信号设计在六条拉斯维加斯交叉路口的适用性。


<details>
  <summary>Details</summary>
Motivation: 转向运动计数数据对交通信号设计、交叉路口规划及拥堵分析至关重要，但现有方法未能充分适应不同交通模式。

Method: 开发了基于视觉的追踪系统估计TMC，结合仿真工具SUMO评估信号配置，提出动态、静态和混合三种信号设计方法。

Result: 实验表明，90和120秒的信号周期效果最佳；动态配置适用于多数路口，混合方法在高峰和非高峰时段表现更优。

Conclusion: 混合信号方法能根据交通模式动态调整，显著提升流量管理效率，尤其适用于交通分布不均的路口。
...
TL;DR: 本文提出了一种名为RCME的框架，通过显式建模传递性强制蕴含关系，优化视觉语言模型中概念的偏序关系，提升了层次分类和检索任务的性能。
...
Method: 提出了一种基于标记点过程的空间统计框架，通过似然训练提供明确的置信度估计，判断区域是否可驾驶（无障碍物）。

Result: 通过校准评估和性能测试验证了该方法的有效性。

Conclusion: 该方法为未检测区域提供了可靠的不确定性量化，提升了目标检测模型的安全性和可靠性。
...
TL;DR: 论文提出了一种动态对数校准（DLC）方法，通过动态调整生成文本与视觉证据的对齐，减少大型视觉语言模型（LVLM）中的幻觉问题。
...
Method: DLC利用CLIP评估生成文本与输入图像的语义对齐，并通过动态更新的上下文基线调整输出对数，优先选择视觉基础更强的候选词。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在固定视角和小规模音频变化下表现良好，但在大角度头部旋转和分布外音频下表现不佳，且需要耗时的身份特定训练。核心问题是缺乏足够的3D先验，限制了合成说话头部的泛化能力。

Method: GGTalker采用两阶段Prior-Adaptation训练策略，学习高斯头部先验并适应个体特征。训练音频-表情和表情-视觉先验以捕捉唇部运动的通用模式和头部纹理的分布。在定制化适应阶段，精确建模个体说话风格和纹理细节。

Result: GGTalker在渲染质量、3D一致性、唇同步准确性和训练效率方面达到最先进水平。

Conclusion: GGTalker通过结合通用先验和身份特定适应，显著提升了语音驱动3D说话头部的质量和泛化能力。
...
Method: 采用梯度引导蒸馏（G$^{2}$D）框架，结合单模态和多模态目标的自定义损失函数，并引入动态顺序模态优先级（SMP）技术。

Result: 在多个真实数据集上验证，G$^{2}$D显著提升弱模态的重要性，并在分类和回归任务中优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有的自动驾驶环境重建方法难以支持显著改变或新颖驾驶场景的真实感合成。

Method: MADrive引入MAD-Cars数据集和检索模块，从记忆库中检索相似车辆实例，重建3D资产并集成到目标场景中。

Result: 实验表明，该方法能够提供完整的多视角车辆表示，支持显著改变配置的真实感合成。

Conclusion: MADrive通过记忆增强和3D资产替换，提升了场景重建的灵活性和真实感。
...
TL;DR: WAFT是一种简单高效的基于高分辨率扭曲的光流方法，取代了传统的成本体积构建，性能更优且内存成本更低。
...
Method: 使用高分辨率扭曲代替成本体积，减少内存需求并提升精度。

Result: 在Spring和KITTI基准测试中排名第一，零样本泛化性能最佳，速度提升4.1倍。

Conclusion: WAFT是一种高效且灵活的光流方法，性能优于现有技术。
...
TL;DR: 论文提出了一种新的跨模态图像-文本检索方法，通过优化嵌入集的一对一匹配和引入两种损失函数，解决了稀疏监督和集合塌陷问题。
...
TL;DR: StruMamba3D是一种自监督点云表示学习新范式，通过保留空间依赖性和改进状态空间模型（SSM）解决了现有方法的局限性，并在多个下游任务中表现优异。
...
Method: 设计了空间状态代理保留空间依赖性，增强SSM的状态更新策略，并引入轻量级卷积和序列长度自适应策略。

Result: 在四个下游任务中表现优异，ModelNet40准确率达95.1%，ScanObjectNN达92.75%。

Conclusion: StruMamba3D通过改进SSM和保留空间依赖性，显著提升了点云表示学习性能。
...
TL;DR: 提出了一种端到端框架，用于从单张部分遮挡图像生成遮挡感知的多视角视图，提升3D重建质量。


<details>
  <summary>Details</summary>
Motivation: 解决现有扩散模型在遮挡情况下生成视图不一致的问题，避免需要手动修复或标注。

Method: 利用Pix2Gestalt数据集的自监督训练管道，联合学习补全和多视角生成，无需修改原始架构。

Result: 生成了六种结构一致的新视角视图，并建立了首个遮挡感知重建基准。

Conclusion: 该方法有效提升了遮挡情况下的3D重建质量，为未来研究提供了标准化评估协议。
...
TL;DR: SAM4D是一个多模态时序基础模型，用于相机和LiDAR流的可提示分割，通过UMPE和MCMA技术实现跨模态对齐和时序一致性，并通过自动化数据引擎快速生成伪标签。
...
Conclusion: SAM4D在跨模态分割和数据标注方面表现出色，为自动驾驶提供了高效解决方案。
...
Result: SiM3D数据集包含333个实例的高分辨率图像和点云数据，并提供了基于异常体积的新评估指标。

Conclusion: SiM3D为多视角3D ADS任务提供了首个基准，并展示了从合成数据到真实数据的泛化潜力。
...
### [113] [Towards Probabilistic Question Answering Over Tabular Data](https://arxiv.org/abs/2506.20747)
*Chen Shen,Sajjadur Rahman,Estevam Hruschka*

Main category: cs.CL

TL;DR: 论文介绍了LUCARIO基准和框架，用于处理表格数据中的概率性问题，结合贝叶斯网络和LLMs，显著优于基线方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有QA系统在处理概率性问题时的不足，如NL2SQL系统无法有效推理不确定性。
...
### [114] [Multi-lingual Functional Evaluation for Large Language Models](https://arxiv.org/abs/2506.20793)
*Victor Ojewale,Inioluwa Deborah Raji,Suresh Venkatasubramanian*

Main category: cs.CL

TL;DR: 论文提出多语言功能基准测试（CL-GSM Symbolic和CL-IFEval），通过翻译现有英文基准模板到五种语言，评估大语言模型在多语言环境中的实际表现和鲁棒性。
...
Method: 通过将英文功能基准模板翻译为法语、西班牙语、印地语、阿拉伯语和约鲁巴语，创建多语言功能基准测试。

Result: 结果显示，某些静态基准测试（如M-MMLU）与功能性能更接近，而模型在不同语言中的鲁棒性差异显著（如阿拉伯语和英语表现最稳定）。

Conclusion: 多语言功能基准测试能更全面地评估模型的实用性能和鲁棒性，为未来多语言模型开发提供更准确的评估工具。
...
### [115] [The Ideation-Execution Gap: Execution Outcomes of LLM-Generated versus Human Research Ideas](https://arxiv.org/abs/2506.20803)
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI生成的研究想法是否能在执行后产生更好的研究成果。

Method: 招募43位专家研究人员，随机分配执行LLM生成或人类专家写的研究想法，并撰写4页短论文。所有项目由NLP专家盲审。
...
### [116] [MultiFinRAG: An Optimized Multimodal Retrieval-Augmented Generation (RAG) Framework for Financial Question Answering](https://arxiv.org/abs/2506.20821)
...
### [117] [Uncovering Hidden Violent Tendencies in LLMs: A Demographic Analysis via Behavioral Vignettes](https://arxiv.org/abs/2506.20822)
*Quintin Myers,Yanjun Gao*

Main category: cs.CL

TL;DR: 研究评估了大型语言模型（LLMs）在道德模糊场景中的暴力倾向，发现其表面生成与内部偏好不一致，且存在跨人口统计学的偏见。
...
Method: 使用社会科学的暴力行为问卷（VBVQ）评估LLMs，并引入基于身份的提示（如种族、年龄、地理位置）以评估偏见。

Result: 发现LLMs的表面生成与内部暴力倾向不一致，且其暴力反应在不同人口统计学中存在偏见。
...
### [118] [Decide less, communicate more: On the construct validity of end-to-end fact-checking in medicine](https://arxiv.org/abs/2506.20876)
*Sebastian Joseph,Lily Chen,Barry Wei,Michael Mackert,Iain J. Marshall,Paul Pu Liang,Ramez Kouzy,Byron C. Wallace,Junyi Jessy Li*

Main category: cs.CL

TL;DR: 论文探讨了自动事实核查系统在医学领域的应用挑战，指出当前系统未被广泛使用的原因，并建议将其视为交互式沟通问题而非端到端过程。


<details>
  <summary>Details</summary>
Motivation: 由于医学决策的高风险性和医学文献的复杂性，公众对医学事实核查系统的需求增加，但现有系统未被广泛采用。

Method: 通过研究临床专家如何验证社交媒体上的真实医学声明，分析医学事实核查的挑战。

Result: 揭示了医学事实核查中的三大挑战：声明与科学证据的关联困难、模糊声明与意图不匹配、主观真实性标签。
...
### [119] [Optimising Language Models for Downstream Tasks: A Post-Training Perspective](https://arxiv.org/abs/2506.20917)
*Zhengyan Shi*

Main category: cs.CL

TL;DR: 该论文提出了一系列改进语言模型（LM）适应下游任务的方法，包括利用未标记数据、参数高效微调和改进监督微调，显著提升了模型的鲁棒性、效率和泛化能力。
...
Method: 论文提出三种方法：1）从未标记数据中提取任务相关知识；2）参数高效微调以减少计算成本；3）改进监督微调以提升指令遵循能力。同时开发了新评估方法和基准。

Result: 实验表明，这些方法显著提升了语言模型的鲁棒性、效率和泛化能力，使其更适应多样化任务。

Conclusion: 这些进展推动了语言模型向更高效和鲁棒的方向发展，为通用人工智能目标迈出重要一步。
...
### [120] [FineWeb2: One Pipeline to Scale Them All -- Adapting Pre-Training Data Processing to Every Language](https://arxiv.org/abs/2506.20920)
*Guilherme Penedo,Hynek Kydlíček,Vinko Sabolčec,Bettina Messmer,Negar Foroutan,Amir Hossein Kargaran,Colin Raffel,Martin Jaggi,Leandro Von Werra,Thomas Wolf*

Main category: cs.CL

TL;DR: 提出了一种基于FineWeb的自动适应多语言的预训练数据集构建管道，并通过实验验证其性能优于现有数据集。


<details>
  <summary>Details</summary>
Motivation: 当前多语言大语言模型（LLMs）的训练面临数据过滤和去重管道难以适应多语言的挑战。

Method: 设计了一种可自动适应任何语言的数据集构建管道，并通过九种语言实验验证其设计选择。

Result: 新管道构建的数据集性能优于现有数据集，并提出了基于重复计数和质量的数据集再平衡方法。

Conclusion: 成功构建了包含1000多种语言的20TB数据集FineWeb2，并开源了相关代码。
...
### [121] [KaLM-Embedding-V2: Superior Training Techniques and Data Inspire A Versatile Embedding Model](https://arxiv.org/abs/2506.20923)
...
Result: 在MTEB中英评测中显著优于同类模型，甚至与更大模型竞争。
...
### [122] [Can Gradient Descent Simulate Prompting?](https://arxiv.org/abs/2506.20989)
*Eric Zhang,Leshem Choshen,Jacob Andreas*

Main category: cs.CL

TL;DR: 论文提出了一种通过元训练语言模型的方法，使梯度更新能模拟提示的效果，无需真实标签，且在某些任务上表现接近提示模型。


<details>
  <summary>Details</summary>
Motivation: 探索如何通过梯度更新（如微调）模拟提示的效果，以结合两者的优势（提示的泛化能力和参数更新的低成本）。

Method: 使用基于梯度的元学习方法，以语言模型自身的提示预测为目标，无需真实标签，通过梯度下降训练模拟提示效果。

Result: 在“反转诅咒”任务和文本问答任务中，单次梯度更新后表现接近提示模型，甚至在某些情况下完全恢复提示性能。

Conclusion: 适当的初始化下，梯度下降具有惊人的表达能力，为长上下文建模和梯度学习的泛化能力提供了新思路。
...
### [123] [SAC: A Framework for Measuring and Inducing Personality Traits in LLMs with Dynamic Intensity Control](https://arxiv.org/abs/2506.20993)
*Adithya Chittem,Aishna Shrivastava,Sai Tarun Pendela,Jagat Sesh Challa,Dhruv Kumar*

Main category: cs.CL

TL;DR: 论文提出了一种扩展的机器个性清单（MPI），结合16PF模型和特定属性控制（SAC）框架，实现了对LLM个性特征的精细控制和动态调节。


<details>
  <summary>Details</summary>
Motivation: 现有LLM个性模型依赖粗粒度的Big Five框架，且缺乏对特征强度的控制机制，无法满足更细腻的人机交互需求。

Method: 扩展MPI以纳入16PF模型，开发SAC框架，通过形容词语义锚定和行为问题动态调节五个强度因子。

Result: 实验表明，连续强度调节比二元切换更一致可控，且目标特征强度的变化会系统影响相关特征。

Conclusion: 该方法为LLM提供了更细腻的个性表达，推动了人机交互在医疗、教育等领域的应用。
...
### [124] [Large Language Models Acing Chartered Accountancy](https://arxiv.org/abs/2506.21031)
*Jatin Gupta,Akhil Sharma,Saransh Singhania,Mohammad Adnan,Sakshi Deo,Ali Imam Abidi,Keshav Gupta*

Main category: cs.CL

TL;DR: 论文介绍了CA-Ben，一个用于评估大型语言模型（LLMs）在金融、法律和定量推理能力的基准测试，基于印度特许会计师考试内容。测试结果显示Claude 3.5 Sonnet和GPT-4o表现最佳，但在数值计算和法律解释方面仍有挑战。


<details>
  <summary>Details</summary>
Motivation: 当前LLMs在金融领域的应用广泛，但其对专业金融知识的掌握和应用能力尚不明确，尤其在印度金融背景下缺乏评估标准。

Method: 通过CA-Ben基准测试，使用ICAI考试内容构建结构化问答数据集，评估六种主流LLMs的性能。
...
### [125] [A Semi-supervised Scalable Unified Framework for E-commerce Query Classification](https://arxiv.org/abs/2506.21049)
*Chunyuan Yuan,Chong Zhang,Zheng Fang,Ming Pang,Xue Jiang,Changping Peng,Zhangang Lin,Ching Law*

Main category: cs.CL

TL;DR: 提出了一种半监督可扩展统一框架（SSUF），通过知识增强、标签增强和结构增强模块，解决了电子商务查询分类中信息不足和标签依赖问题，显著优于现有模型。
...
Conclusion: SSUF为电子商务查询分类提供了一种高效、统一的解决方案，解决了信息不足和标签依赖问题。
...
### [126] [MT2-CSD: A New Dataset and Multi-Semantic Knowledge Fusion Method for Conversational Stance Detection](https://arxiv.org/abs/2506.21053)
...
<details>
  <summary>Details</summary>
Motivation: 传统立场检测局限于单实例，缺乏真实社交媒体互动动态的数据集，限制了多轮对话立场检测的发展。
...
### [127] [CBF-AFA: Chunk-Based Multi-SSL Fusion for Automatic Fluency Assessment](https://arxiv.org/abs/2506.20243)
*Papa Séga Wade,Mihai Andries,Ioannis Kanellos,Thierry Moudenc*

Main category: cs.CL

TL;DR: 论文提出了一种基于分块的自监督学习（SSL）融合方法，用于非母语者的自动流畅度评估（AFA），显著提升了评估性能。
...
Method: 采用分块策略，结合多种SSL模型（Wav2Vec2、HuBERT、WavLM）和分层CNN-BiLSTM框架，利用Silero-VAD进行语音分段，并通过加权机制融合特征。

Result: 在Avalinguo和Speechocean762数据集上，F1分数和Pearson相关系数分别提升了2.8-4.2点和6.2-4.0点，优于单SSL和Pyannote.audio基线。
...
### [128] [DALR: Dual-level Alignment Learning for Multimodal Sentence Representation Learning](https://arxiv.org/abs/2506.21096)
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在图像和文本对齐上过于粗粒度，存在跨模态偏差和模态内语义分歧，影响表示质量。

Method: DALR通过一致性学习模块软化负样本，利用辅助任务的语义相似性实现细粒度跨模态对齐；同时结合排序蒸馏和全局模态内对齐学习。
...
### [129] [ComRAG: Retrieval-Augmented Generation with Dynamic Vector Stores for Real-time Community Question Answering in Industry](https://arxiv.org/abs/2506.21098)
*Qinwen Chen,Wenbiao Tao,Zhiwei Zhu,Mingfan Xi,Liangzhong Guo,Yuan Wang,Wei Wang,Yunshi Lan*

Main category: cs.CL

TL;DR: ComRAG是一个用于实时工业社区问答的检索增强生成框架，结合静态知识和动态历史问答对，通过基于质心的记忆机制优化检索、生成和存储效率。
...
Method: 提出ComRAG框架，整合静态知识和动态历史问答对，采用基于质心的记忆机制优化检索、生成和存储。

Result: 在三个工业CQA数据集上，ComRAG显著优于基线方法，向量相似度提升25.9%，延迟降低8.7%至23.3%，块增长率从20.23%降至2.06%。
...
### [130] [Progtuning: Progressive Fine-tuning Framework for Transformer-based Language Models](https://arxiv.org/abs/2506.21119)
*Xiaoshuang Ji,Zhendong Zhao,Xiaojun Chen,Xin Zhao,Zeyao Liu*

Main category: cs.CL

TL;DR: Progtuning是一种结合渐进式学习的微调框架，通过逐步减少更新的Transformer块数量，优化资源分配，减少约25%的更新参数，同时保持性能。
...
Method: 提出Progtuning框架，基于Transformer块的贡献逐步减少更新的块数量。

Result: 减少约25%的更新参数，保持竞争性能，并与其他参数高效微调方法兼容。

Conclusion: Progtuning在资源分配和性能之间取得平衡，适用于多种适应场景。
...
### [131] [Compressed and Smooth Latent Space for Text Diffusion Modeling](https://arxiv.org/abs/2506.21170)
*Viacheslav Meshchaninov,Egor Chimbulatov,Alexander Shabalin,Aleksandr Abramov,Dmitry Vetrov*

Main category: cs.CL

TL;DR: Cosmos是一种基于压缩潜在空间的扩散模型，用于文本生成，解决了自回归模型的速度和全局一致性问题，同时克服了扩散模型在高维表示上的困难。


<details>
  <summary>Details</summary>
Motivation: 自回归语言模型在文本生成中存在速度慢和全局一致性差的问题，而扩散模型虽能并行生成但受限于高维表示。Cosmos旨在解决这些问题。

Method: Cosmos通过自编码器在压缩的潜在空间中学习文本表示，结合预训练语言编码器的冻结激活，实现语义基础和扰动增强。

Result: 实验表明，Cosmos能将文本表示压缩8倍，生成质量与基于标记的扩散模型相当，且通过增加潜在序列长度超越基线模型。

Conclusion: Cosmos在多种生成任务中表现优异，生成质量与现有方法相当或更好，且推理速度快2倍以上。
...
### [132] [Maintaining MTEB: Towards Long Term Usability and Reproducibility of Embedding Benchmarks](https://arxiv.org/abs/2506.21182)
...
<details>
  <summary>Details</summary>
Motivation: MTEB已成为文本嵌入模型的标准评估平台，但需要确保其持续的可重现性和扩展性，以保持其相关性和质量。

Method: 通过设计稳健的持续集成管道、自动化测试执行、验证数据集完整性，并制定处理社区贡献和扩展任务的策略。

Result: 这些工程实践成功扩展了MTEB的覆盖范围，同时保持了其质量和可重现性。

Conclusion: 本文为机器学习评估框架的维护者提供了确保可重现性和可用性的宝贵经验。
...
### [133] [Prompt-Guided Turn-Taking Prediction](https://arxiv.org/abs/2506.21191)
*Koji Inoue,Mikey Elmers,Yahui Fu,Zi Haur Pang,Divesh Lala,Keiko Ochi,Tatsuya Kawahara*

Main category: cs.CL

TL;DR: 提出了一种基于文本提示的动态控制对话轮换预测模型，通过指令（如“更快”或“更平静”）实现直观控制，实验证明其有效性和适应性。


<details>
  <summary>Details</summary>
Motivation: 现有对话轮换预测模型缺乏动态控制能力，无法根据对话伙伴和上下文灵活调整，因此需要一种更直观和显式的控制方法。

Method: 在基于Transformer的语音活动预测（VAP）模型基础上，引入文本提示嵌入，结合通道内和跨通道Transformer，利用LLM生成合成提示数据进行训练。

Result: 实验使用950小时人类对话数据，模型显著提高了预测准确性，并能根据文本提示动态调整轮换时机。

Conclusion: 该模型通过文本提示实现了对话轮换的动态控制，为对话系统和机器人提供了更灵活的交互能力。
...
### [134] [Enhancing Automatic Term Extraction with Large Language Models via Syntactic Retrieval](https://arxiv.org/abs/2506.21222)
*Yongchan Chun,Minhyuk Kim,Dongjun Kim,Chanjun Park,Heuiseok Lim*

Main category: cs.CL

TL;DR: 提出了一种基于检索的提示策略，利用句法相似性而非语义相似性在少样本设置中选择示例，用于自动术语提取（ATE），并在实验中验证了其有效性。
...
Method: 采用基于检索的提示策略，通过句法相似性选择示例，该方法与领域无关，能更可靠地捕捉术语边界。

Result: 在三个专门的ATE基准测试中，句法检索方法提高了F1分数。

Conclusion: 研究强调了句法线索在将LLM适应术语提取任务中的重要性。
...
### [135] [Agent-RewardBench: Towards a Unified Benchmark for Reward Modeling across Perception, Planning, and Safety in Real-World Multimodal Agents](https://arxiv.org/abs/2506.21252)
*Tianyi Men,Zhuoran Jin,Pengfei Cao,Yubo Chen,Kang Liu,Jun Zhao*

Main category: cs.CL

TL;DR: 提出了Agent-RewardBench，一个评估多模态大语言模型（MLLMs）奖励建模能力的基准，以解决现有代理在自我修正和泛化方面的不足。


<details>
  <summary>Details</summary>
Motivation: 由于缺乏外部反馈，多模态代理在自我修正和泛化方面表现不佳，需要专门的奖励模型来提升能力。

Method: 构建了一个包含多维度评估（感知、规划、安全）、步级奖励评估和高质量数据集的基准。

Result: 实验表明，即使最先进的多模态模型在奖励建模方面表现有限，需要专门训练。

Conclusion: Agent-RewardBench为提升代理的奖励建模能力提供了有效工具，填补了现有研究的空白。
...
### [136] [Cat and Mouse -- Can Fake Text Generation Outpace Detector Systems?](https://arxiv.org/abs/2506.21274)
...
<details>
  <summary>Details</summary>
Motivation: 研究大型语言模型生成虚假文本的能力是否会导致检测难度无限增加，以及检测方法是否能够持续有效。

Method: 通过统计分类器检测古典侦探小说风格的虚假文本，比较不同版本模型的生成能力。

Result: Gemini模型在生成欺骗性文本方面有所提升，而GPT模型未表现出明显变化。

Conclusion: 即使模型规模增大，虚假文本的可靠检测仍可能实现，但新模型架构可能提高欺骗性。
...
### [137] [Double-Checker: Enhancing Reasoning of Slow-Thinking LLMs via Self-Critical Fine-Tuning](https://arxiv.org/abs/2506.21285)
*Xin Xu,Tianhao Chen,Fan Zhang,Wanlong Liu,Pengxiang Li,Ajay Kumar Jaiswal,Yuchen Yan,Jishan Hu,Yang Wang,Hao Chen,Shiwei Liu,Shizhe Diao,Can Yang,Lu Yin*

Main category: cs.CL

TL;DR: Double-Checker框架通过自我批判和迭代优化提升慢思考LLM的推理能力，显著提高了AIME基准测试的性能。
...
Method: 提出Double-Checker框架，通过1,730个自我批判实例的微调，实现LLM的迭代自我批判和解决方案优化。

Result: Double-Checker将AIME基准测试的pass@1性能从4.4%提升至18.2%。

Conclusion: Double-Checker为开发更可信和有效的LLM提供了一种有前景的方向。
...
### [138] [Small Encoders Can Rival Large Decoders in Detecting Groundedness](https://arxiv.org/abs/2506.21288)
*Istabrak Abbes,Gabriele Prato,Quentin Fournier,Fernando Rodriguez,Alaa Boukhary,Adam Elwood,Sarath Chandar*

Main category: cs.CL

TL;DR: 论文提出了一种轻量级模型（如RoBERTa和NomicBERT）用于检测查询是否基于上下文，以减少LLMs生成答案时的资源消耗和延迟。


<details>
  <summary>Details</summary>
Motivation: LLMs在上下文缺乏信息时容易产生不可靠的推测，因此需要一种机制确保回答的基于性（groundedness）。

Method: 使用轻量级编码器模型（如RoBERTa和NomicBERT）进行微调，检测查询是否基于上下文。

Result: 这些轻量级模型在检测基于性方面与先进LLMs（如Llama3 8B和GPT4o）表现相当，且显著降低了推理延迟。

Conclusion: 轻量级模型可有效检测基于性，减少LLMs的资源消耗和延迟，提升效率。
...
### [139] [Detecting Referring Expressions in Visually Grounded Dialogue with Autoregressive Language Models](https://arxiv.org/abs/2506.21294)
*Bram Willemsen,Gabriel Skantze*

Main category: cs.CL

TL;DR: 本文探讨了仅使用文本的自回归语言模型方法从视觉对话中提取指代表达式，发现仅依赖语言上下文也能有效完成任务，但任务本质是多模态的。


<details>
  <summary>Details</summary>
Motivation: 研究语言上下文是否足以检测视觉对话中具有视觉指涉的提及。

Method: 采用预训练的大型语言模型（LLM），通过下一词预测标注提及范围。

Result: 即使使用中等规模LLM和小数据集，仅文本方法仍有效，表明语言上下文的重要性。
...
### [140] [Structuralist Approach to AI Literary Criticism: Leveraging Greimas Semiotic Square for Large Language Models](https://arxiv.org/abs/2506.21360)
*Fangzhou Dong,Yifan Zeng,Yingpeng Sang,Hong Shen*

Main category: cs.CL

TL;DR: GLASS框架基于Greimas符号学方阵，提升大语言模型在文学批评中的深度分析能力，填补研究空白。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在复杂叙事和深刻思想的文学批评上表现不足，需结构化方法提升其分析能力。

Method: 提出GLASS框架，构建首个GSS文学批评数据集，设计定量指标，并与专家批评对比验证。

Result: GLASS在多个作品和模型上表现优异，应用于39部经典作品，产出高质量分析。
...
### [141] [Leveraging LLM-Assisted Query Understanding for Live Retrieval-Augmented Generation](https://arxiv.org/abs/2506.21384)
...
Result: Omni-RAG能够更有效地处理复杂和噪声查询，满足实际应用需求。

Conclusion: Omni-RAG填补了当前RAG能力与实际应用需求之间的差距，特别是在实时开放域环境中。
...
### [142] [Domain Knowledge-Enhanced LLMs for Fraud and Concept Drift Detection](https://arxiv.org/abs/2506.21443)
...
<details>
  <summary>Details</summary>
Motivation: 动态平台上语言模式的演变和概念漂移使得欺诈对话检测变得困难，现有LLM在风险敏感场景中表现不佳。

Method: 提出DK-Enhanced LLM框架，包含三个模块：检测欺诈对话的DK-LLM、检测语义漂移的OCDD单元，以及分类漂移性质的第二个DK-LLM。
...
### [143] [Text2Cypher Across Languages: Evaluating Foundational Models Beyond English](https://arxiv.org/abs/2506.21445)
*Makbule Gulcin Ozsoy,William Tai*

Main category: cs.CL

TL;DR: 本文研究了基础大语言模型在多语言Text2Cypher任务中的表现，发现性能差异与训练数据和语言特性相关，提示翻译影响较小，呼吁更多多语言评估和开发。
...
### [144] [Aligning Spoken Dialogue Models from User Interactions](https://arxiv.org/abs/2506.21463)
...
Method: 构建了一个包含15万对偏好的大规模数据集，利用离线对齐方法微调全双工自回归语音模型。

Result: 实验表明，通用对话的反馈能显著提升语音模型的事实性、安全性和上下文对齐能力。
...
### [145] [TopK Language Models](https://arxiv.org/abs/2506.21468)
...
<details>
  <summary>Details</summary>
Motivation: 稀疏自编码器（SAE）在分析Transformer语言模型时存在局限性，如特征不稳定、无法区分概念发现失败的原因等，限制了其实际应用。

Method: 在Transformer架构中引入TopK激活函数，使隐藏状态直接等同于TopK SAE的潜在特征，无需后训练。

Result: TopK LMs在模型大小、计算效率和解释性之间取得了平衡，保持了原始能力，同时提供了稳定的稀疏表示。
...
### [146] [Bridging Offline and Online Reinforcement Learning for LLMs](https://arxiv.org/abs/2506.21495)
...
<details>
  <summary>Details</summary>
Motivation: 探讨强化学习方法在不同任务（可验证与不可验证）和训练模式（离线、半在线、在线）下的有效性。

Method: 比较在线和半在线的直接偏好优化（DPO）与群体奖励策略优化（GRPO）目标，分析训练动态和超参数选择。

Result: 在线和半在线方法表现相似且优于离线方法，多任务学习可提升性能。

Conclusion: 在线和半在线强化学习方法在微调大语言模型中表现优异，多任务学习进一步优化性能。
...
### [147] [Enhancing User Engagement in Socially-Driven Dialogue through Interactive LLM Alignments](https://arxiv.org/abs/2506.21497)
...
Conclusion: 直接优化用户反应能有效增强交互式LLM的用户参与度。
...
### [148] [skLEP: A Slovak General Language Understanding Benchmark](https://arxiv.org/abs/2506.21508)
*Marek Šuppa,Andrej Ridzik,Daniel Hládek,Tomáš Javůrek,Viktória Ondrejová,Kristína Sásiková,Martin Tamajka,Marián Šimko*

Main category: cs.CL

TL;DR: 介绍了首个针对斯洛伐克自然语言理解（NLU）模型的综合基准测试skLEP，涵盖九项任务，并评估了多种语言模型。
...
Method: 通过整理新数据集和翻译英语资源构建skLEP，并评估多种预训练语言模型。

Result: 发布了完整的基准数据、开源工具包和公开排行榜。

Conclusion: skLEP为斯洛伐克NLU研究提供了标准化评估工具，促进未来研究。
...
### [149] [Potemkin Understanding in Large Language Models](https://arxiv.org/abs/2506.21521)
*Marina Mancoridis,Bec Weeks,Keyon Vafa,Sendhil Mullainathan*

Main category: cs.CL

TL;DR: 论文探讨了大型语言模型（LLMs）在基准测试中的表现是否真实反映其能力，提出了“虚假理解”的概念，并设计了两种量化方法验证其普遍性。


<details>
  <summary>Details</summary>
Motivation: 基准测试常用于评估LLMs，但其有效性依赖于模型是否以人类相似的方式误解概念，否则可能只是虚假理解。

Method: 提出两种量化方法：一种使用专门设计的基准测试，另一种提供普遍性的下限估计。

Result: 发现虚假理解在模型、任务和领域中普遍存在，且反映概念表征的内部不一致性。

Conclusion: 基准测试可能无法真实反映LLMs的理解能力，需警惕虚假理解现象。
...
### [150] ["What's Up, Doc?": Analyzing How Users Seek Health Information in Large-Scale Conversational AI Datasets](https://arxiv.org/abs/2506.21532)
...
<details>
  <summary>Details</summary>
Motivation: 探索用户如何通过LLM聊天机器人获取医疗信息，以及这些交互中的潜在风险。

Method: 通过过滤大规模对话数据集构建HealthChat-11K，结合临床分类法分析21个医疗领域的用户交互。

Result: 揭示了用户行为模式（如不完整上下文、情感行为、诱导性问题等），指出LLM在医疗支持能力上的不足。

Conclusion: 需改进LLM在医疗对话中的支持能力，以减少潜在风险。
...
### [151] [Data Efficacy for Language Model Training](https://arxiv.org/abs/2506.21545)
*Yalun Dai,Yangyu Huang,Xin Zhang,Wenshan Wu,Chong Li,Wenhui Lu,Shijie Cao,Li Dong,Scarlett Li*

Main category: cs.CL

TL;DR: 本文提出了一种名为DELT的通用范式，通过优化训练数据的组织（数据效能）来提升语言模型性能，包括数据评分、选择和排序三个组件。实验表明DELT能显著提升模型表现，且与数据效率不冲突。


<details>
  <summary>Details</summary>
Motivation: 当前研究主要关注数据效率（如数据过滤和选择），而数据效能（即数据组织的优化）尚未充分探索。本文旨在填补这一空白，提出DELT范式以提升语言模型训练效果。

Method: 提出DELT范式，包含数据评分（如LQS）、数据选择和数据排序（如FO）。LQS从梯度一致性角度评估数据的可学习性和质量，FO解决模型遗忘和数据分布偏差问题。

Result: 实验表明，DELT能不同程度提升语言模型性能，且不增加数据规模或模型大小。LQS与FO的组合效果最佳，数据效能与数据效率可同时实现。

Conclusion: 数据效能是语言模型训练中一个有前景的基础研究方向，DELT为其提供了有效框架。
...
TL;DR: 论文讨论了AI安全的重要性，提出了构建可信生态系统的必要性，并通过2025年新加坡AI会议报告，将AI安全研究分为开发、评估和控制三大领域。


<details>
  <summary>Details</summary>
Motivation: 随着AI能力的快速提升，确保其安全、可信、可靠和安全的必要性日益凸显，以避免潜在风险并促进创新。

Method: 报告基于国际AI安全报告，采用深度防御模型，将AI安全研究分为开发（可信AI系统）、评估（风险）和控制（部署后干预）三大领域。

Result: 会议报告整合了全球AI科学家的意见，明确了AI安全研究的优先方向，为构建可信AI生态系统提供了框架。

Conclusion: 通过多领域协作和深度防御模型，可以有效应对AI安全挑战，推动AI技术的健康发展。
...
TL;DR: 论文研究了LLM智能体在协作任务中是否理解上下文隐私，并评估了现有模型在多轮对话中保护隐私的能力。结果表明当前模型在隐私保护和任务完成方面表现不佳。


<details>
  <summary>Details</summary>
Motivation: 随着LLM智能体的广泛应用，隐私保护变得至关重要，尤其是在涉及专有工具和数据库的高风险场景中。现有基准测试未能充分评估复杂任务中的隐私保护能力。

Method: 提出了MAGPIE基准测试，包含158个高风险场景，评估LLM智能体对上下文隐私的理解和协作能力。

Result: 当前模型（如GPT-4o和Claude-2.7-Sonnet）在隐私分类和多轮对话中表现不佳，分别有25.2%和43.6%的错误率，且在71%的场景中无法完成任务。

Conclusion: 现有模型在上下文隐私保护和协作任务完成方面存在显著不足，亟需改进。
...
TL;DR: 提出了一种动态上下文感知提示推荐系统，用于领域特定的AI应用，结合多种技术生成相关且可操作的提示建议。
...
Result: 在真实数据集上验证了方法的高实用性和相关性。

Conclusion: 系统能有效提升领域特定应用中提示的质量和实用性。
...
Method: 提出了一个概念验证框架，预测模型建议的宏观社会传播，并引入100个间接危害场景数据集。

Result: 在新数据集上性能提升超过20%，在现有安全基准上的平均胜率超过70%。

Conclusion: 该框架为开发更安全的语言模型提供了有前景的方向。
...
TL;DR: 论文探讨了大型语言模型（LLMs）的因果推理能力，发现其仅能进行浅层（level-1）推理，缺乏人类深层（level-2）能力。通过新基准CausalProbe-2024验证后，提出G^2-Reasoner方法，结合通用知识和目标导向提示，显著提升了LLMs的因果推理能力。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs是否具备类似人类的真实因果推理能力，填补其与人类深层推理之间的差距。

Method: 分析LLMs的自回归机制，提出新基准CausalProbe-2024，并开发G^2-Reasoner方法，结合通用知识和目标导向提示。

Result: LLMs在CausalProbe-2024上表现显著下降，验证其仅能进行level-1推理；G^2-Reasoner显著提升了LLMs在新鲜和反事实情境中的推理能力。
...
<details>
  <summary>Details</summary>
Motivation: 现有LVLMs在复杂场景和多步目标任务中表现不佳，主要依赖环境无关的模仿学习，缺乏环境上下文理解。

Method: 提出WAP框架，结合视觉外观建模、空间推理、功能抽象和语法基础，通过课程学习仅使用原始视觉观察数据。

Result: 在EB-ALFRED基准测试中，任务成功率提升60.7%，常识推理和长时规划分别提升60.0和70.0。

Conclusion: WAP显著提升了LVLMs的性能，甚至超越专有系统如GPT-4o和Claude-3.5-Sonnet。
...
Method: 开发了交互式系统IXAII，整合多种解释方法并提供用户定制化视图。
...
TL;DR: 论文提出AI驱动科学发现需解决三个关键差距：抽象、推理与现实差距，而非依赖模型规模或计算资源。


<details>
  <summary>Details</summary>
Motivation: 当前AI系统在科学发现中存在局限性，如架构僵化、推理脆弱及脱离实验现实，需通过新方法突破。

Method: 提出主动推理AI系统，结合因果自监督模型、贝叶斯规划器、知识图谱及闭环实验验证。

Result: 通过内部模型与外部验证的交互，实现科学发现，强调人类判断的不可或缺性。

Conclusion: AI科学发现需整合因果推理、知识增长与实验验证，人类判断是其核心组成部分。
...
Method: 采用神经符号路由机制，动态分配表格元素到专业专家（如Table-to-HTML、Table-to-JSON等），并结合符号推理图进行置信度感知的门控策略。
...
<details>
  <summary>Details</summary>
Motivation: 探讨VLMs是否能像人类一样从少量视角构建完整的空间心理模型，以解决现有模型在空间推理中的表现接近随机的问题。

Method: 通过MindCube基准测试评估VLMs的空间心理模型能力，并探索了三种方法：生成中间视角、自然语言推理链和认知地图。最终采用“map-then-reason”协同方法，结合强化学习。

Result: “map-then-reason”方法将准确率从37.8%提升至60.8%，加入强化学习后进一步提升至70.7%。

Conclusion: 通过构建和利用内部结构化空间表示，VLMs的空间推理能力得到显著提升。
...
TL;DR: 论文介绍了AH2AC2挑战，通过人类代理代理解决人类评估成本高且难以复现的问题，并开源了有限的人类游戏数据集。


<details>
  <summary>Details</summary>
Motivation: 解决AI与人类协调中的评估难题，推动数据高效方法的发展。

Method: 开发人类代理代理作为评估伙伴，开源有限数据集，提供基线结果。

Result: 成功构建了可复现的人类代理代理，并在两和三玩家Hanabi场景中提供基线。

Conclusion: AH2AC2为人类-AI协调研究提供了廉价、可复现的评估框架。
...
TL;DR: Mind2Web 2是一个包含130个长时任务的新基准，用于评估自主网络搜索系统的复杂性和实时性，并提出了一种基于树状结构的自动评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法无法应对自主搜索系统的复杂性和开放性，因此需要新的基准和方法。
...
TL;DR: PsyLite是一个轻量级心理咨询大语言模型，基于InternLM2.5-7B-chat开发，通过两阶段训练策略提升推理能力、心理咨询能力和对话安全性，并采用量化技术实现低硬件部署。
...
Method: 采用混合蒸馏数据微调和ORPO偏好优化的两阶段训练策略，设计条件RAG引入幽默元素并拒绝危险请求。

Result: 在CEval、CPsyCounE和SafeDialBench评估中表现优于基线模型，心理咨询专业性提升47.6%，对话安全性提升2.4%。

Conclusion: PsyLite为资源受限环境提供了可行的心理咨询解决方案。
...
### [165] [Perry: A High-level Framework for Accelerating Cyber Deception Experimentation](https://arxiv.org/abs/2506.20770)
*Brian Singer,Yusuf Saquib,Lujo Bauer,Vyas Sekar*

Main category: cs.CR

TL;DR: Perry是一个高层次框架，用于加速网络欺骗场景的设计和探索，通过简化实现和扩展复杂性。


<details>
  <summary>Details</summary>
Motivation: 现有欺骗工具和平台实现复杂且难以修改，阻碍了操作员实验和评估欺骗方法。

Method: Perry包含高层次抽象层和实验模块，支持攻击者和防御策略的规范，并通过四个关键模块实现高低级转换。

Result: Perry减少了实现多样化欺骗防御的复杂性，并通过55个独特场景展示了其价值。

Conclusion: Perry的高层次抽象和模块化设计显著提升了欺骗策略的实验效率和灵活性。
...
### [166] [SIMulator: SIM Tracing on a (Pico-)Budget](https://arxiv.org/abs/2506.20800)
*Gabriel K. Gegenhuber,Philipp É. Frenzel,Adrian Dabrowski*

Main category: cs.CR

TL;DR: 论文提出了一种低成本实现SIM追踪功能的方法，利用常见组件如UART接口和GPIO端口，显著降低了硬件复杂性和成本。


<details>
  <summary>Details</summary>
Motivation: 传统SIM追踪依赖昂贵专用硬件，限制了研究者和爱好者的参与。本文旨在通过简化硬件需求，推动蜂窝网络研究的普及。

Method: 使用低成本微控制器（如Raspberry Pi Pico）实现SIM追踪功能，通过电气解耦SIM和调制解调器，仅传输APDU级别数据。

Result: 成功实现了低成本、低复杂性的SIM追踪方案，硬件成本低至4美元。

Conclusion: 该方法大幅降低了SIM追踪的门槛，有望促进蜂窝网络研究的广泛探索和实验。
...
### [167] [Poster: Enhancing GNN Robustness for Network Intrusion Detection via Agent-based Analysis](https://arxiv.org/abs/2506.20806)
*Zhonghao Zhan,Huichi Zhou,Hamed Haddadi*

Main category: cs.CR

TL;DR: 提出了一种利用大型语言模型（LLMs）增强图神经网络（GNNs）在入侵检测系统中鲁棒性和泛化能力的新方法。


<details>
  <summary>Details</summary>
Motivation: GNNs在网络入侵检测系统（NIDS）中表现优异，但面临分布漂移和对抗攻击的挑战，现有评估方法不够现实。

Method: 通过LLMs模拟网络安全专家，分析网络流数据的图结构，识别并缓解对抗性扰动。

Result: 实验表明，LLMs能显著提升GNNs在对抗攻击下的鲁棒性。
...
### [168] [Empowering Digital Agriculture: A Privacy-Preserving Framework for Data Sharing and Collaborative Research](https://arxiv.org/abs/2506.20872)
*Osama Zafar,Rosemarie Santa González,Mina Namazi,Alfonso Morales,Erman Ayday*

Main category: cs.CR

TL;DR: 提出了一种隐私保护框架，结合降维技术和差分隐私，支持农民安全共享数据，促进农业研究与合作。
...
Conclusion: 该框架有助于农民和研究者的合作，推动农业创新和可持续发展。
...
### [169] [ZKPROV: A Zero-Knowledge Approach to Dataset Provenance for Large Language Models](https://arxiv.org/abs/2506.20915)
...
<details>
  <summary>Details</summary>
Motivation: 在敏感领域（如医疗）部署LLM时，确保其计算来源的完整性至关重要，同时需满足严格的数据使用要求。

Method: ZKPROV通过零知识证明将训练模型与授权数据集绑定，避免验证每一步训练过程，利用数据集签名元数据和紧凑模型参数承诺。

Result: 实验证明ZKPROV在生成和验证证明时高效且可扩展，适用于实际部署，并提供形式化安全保证。

Conclusion: ZKPROV在保护数据集机密性的同时，提供了可信的数据来源验证，解决了LLM在敏感领域的应用挑战。
...
### [170] [CodeGuard: A Generalized and Stealthy Backdoor Watermarking for Generative Code Models](https://arxiv.org/abs/2506.20926)
*Haoxuan Li,Jiale Zhang,Xiaobing Sun,Xiapu Luo*

Main category: cs.CR

TL;DR: CodeGuard是一种结合注意力机制和分布式触发器嵌入策略的新型水印方法，解决了现有技术在泛化性和隐蔽性上的不足，实现了高验证率和低检测率。


<details>
  <summary>Details</summary>
Motivation: 生成代码模型（GCMs）需要有效的数字版权保护，以防止未经授权的泄露和滥用。现有的后门水印技术存在泛化性不足和隐蔽性差的问题。

Method: CodeGuard通过注意力机制确定水印嵌入位置，并采用同态字符替换和分布式触发器嵌入策略，确保可验证性和隐蔽性。

Result: 实验表明，CodeGuard在代码摘要和代码生成任务中实现了高达100%的水印验证率，且不影响主要任务性能，隐蔽性表现优异。

Conclusion: CodeGuard是一种高效且隐蔽的水印方法，适用于生成代码模型的版权保护。
...
### [171] [SPA: Towards More Stealth and Persistent Backdoor Attacks in Federated Learning](https://arxiv.org/abs/2506.20931)
...
<details>
  <summary>Details</summary>
Motivation: 联邦学习（FL）中的后门攻击通常依赖端到端标签监督，导致特征解耦和持久性有限，因此需要更隐蔽的攻击方法。

Method: SPA通过减少后门触发特征与目标类特征之间的表示距离，并引入自适应对抗触发优化机制，增强攻击效果。

Result: 实验表明，SPA在多种FL基准测试中攻击成功率高，对模型效用影响小，且在非独立同分布数据下仍保持鲁棒性。

Conclusion: SPA展示了后门攻击的日益复杂化，强调了开发特征级防御技术的紧迫性。
...
### [172] [PrivacyGo: Privacy-Preserving Ad Measurement with Multidimensional Intersection](https://arxiv.org/abs/2506.20981)
...
<details>
  <summary>Details</summary>
Motivation: 解决多标识符隐私用户配置文件匹配的挑战性问题，为现代广告分析提供隐私保护解决方案。

Method: 采用反向OPRF和盲密钥旋转技术，结合差分隐私机制，设计高效且安全的协议。

Result: 协议具有强隐私保证和高效率，适用于大规模数据集，为广告行业提供隐私保护新标准。

Conclusion: 通过结合密码学原理和差分隐私，本文为隐私保护广告测量框架设定了新标准。
...
### [173] [TEMPEST-LoRa: Cross-Technology Covert Communication](https://arxiv.org/abs/2506.21069)
*Xieyang Sun,Yuanqing Zheng,Wei Xi,Zuhao Chen,Zhizhen Chen,Han Hao,Zhiping Jiang,Sheng Zhong*

Main category: cs.CR

TL;DR: TEMPEST-LoRa是一种新型电磁隐蔽通道，利用视频电缆的电磁泄漏，通过LoRa接收器远距离传输敏感数据。


<details>
  <summary>Details</summary>
Motivation: 揭示电磁隐蔽通道在隔离网络中的潜在风险，探索无需专用接收器的远距离数据传输方法。

Method: 通过操纵视频电缆精确生成电磁泄漏，利用商业LoRa节点/网关接收数据。

Result: 实验显示，攻击者可在87.5米距离内以21.6 kbps速率可靠解码数据。

Conclusion: TEMPEST-LoRa证明了电磁隐蔽通道的可行性，且可在显示器关闭时隐蔽传输数据。
...
### [174] [PhishKey: A Novel Centroid-Based Approach for Enhanced Phishing Detection Using Adaptive HTML Component Extraction](https://arxiv.org/abs/2506.21106)
*Felipe Castaño,Eduardo Fidalgo,Enrique Alegre,Rocio Alaiz-Rodríguez,Raul Orduna,Francesco Zola*

Main category: cs.CR

TL;DR: PhishKey是一种新型钓鱼检测方法，结合字符级CNN和基于质心的HTML内容提取器（CAPE），通过软投票集成提高分类准确性，实验显示其F1分数高达98.70%，且对抗攻击性能稳定。


<details>
  <summary>Details</summary>
Motivation: 钓鱼攻击快速演变，绕过检测机制并利用人类弱点，现有方法在适应性、鲁棒性和效率方面存在挑战。

Method: PhishKey结合字符级CNN处理URL分类和CAPE处理HTML内容，通过软投票集成预测结果。

Result: 在四个数据集上测试，F1分数达98.70%，对抗攻击性能下降最小。
...
### [175] [Inside Job: Defending Kubernetes Clusters Against Network Misconfigurations](https://arxiv.org/abs/2506.21134)
*Jacopo Bufalino,Jose Luis Martin-Navarro,Mario Di Francesco,Tuomas Aura*

Main category: cs.CR

TL;DR: 该论文分析了Kubernetes集群中的网络配置错误对安全的影响，特别是横向移动问题，评估了287个开源应用，发现634个配置错误，并提出了修复建议。


<details>
  <summary>Details</summary>
Motivation: Kubernetes作为容器编排的事实标准，其安全性日益重要，但网络配置对安全的影响尚未得到充分研究。

Method: 对287个开源应用进行网络配置错误分析，重点关注横向移动问题。

Result: 发现634个配置错误，远超现有解决方案的检测能力，部分问题已修复。
...
### [176] [Balancing Privacy and Utility in Correlated Data: A Study of Bayesian Differential Privacy](https://arxiv.org/abs/2506.21308)
*Martin Lange,Patricia Guerra-Balboa,Javier Parra-Arnau,Thorsten Strufe*

Main category: cs.CR

TL;DR: 论文探讨了在数据相关情况下贝叶斯差分隐私（BDP）的实用性，提出了一种新方法以保持数据效用。


<details>
  <summary>Details</summary>
Motivation: 现实世界数据普遍存在相关性，传统差分隐私（DP）低估隐私泄露风险，BDP虽能解决但实用性不足。

Method: 分析高斯多元分布和马尔可夫链等模型，提出理论联系和新方法以适配BDP需求。

Result: 在真实数据库上验证，新方法能保持BDP机制的竞争性效用。

Conclusion: BDP在相关性数据中可实现实用性，为隐私保护提供新途径。
