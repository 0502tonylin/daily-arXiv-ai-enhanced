...
### [89] [Correspondence-Free Multiview Point Cloud Registration via Depth-Guided Joint Optimisation](https://arxiv.org/abs/2506.18922)
*Yiran Zhou,Yingyu Wang,Shoudong Huang,Liang Zhao*
...
### [90] [Connecting Vision and Emissions: A Behavioural AI Approach to Carbon Estimation in Road Design](https://arxiv.org/abs/2506.18924)
*Ammar K Al Mhdawi,Nonso Nnamoko,Safanah Mudheher Raafat,M. K. S. Al-Mhdawi,Amjad J Humaidi*

Main category: cs.CV

TL;DR: 提出了一种基于YOLOv8的实时车辆检测与分类框架，用于估算城市环境中的碳排放。通过改进YOLOv8架构，结合深度OCR模块和外部数据库验证，实现高精度的车牌识别和车辆分类。


<details>
  <summary>Details</summary>
Motivation: 解决YOLOv8在细粒度识别任务（如车牌读取）上的不足，为智能交通系统提供自动化的碳排放监测解决方案。

Method: 采用混合管道，结合YOLOv8检测与分割、深度OCR模块和外部数据库验证，实现车辆定位、车牌识别和分类。

Result: YOLOv8检测器mAP@0.5达71%，分割掩码达70%；OCR字符级准确率最高99%。

Conclusion: 该方法将实时目标检测与深度OCR结合，为智能交通系统中的碳排放监测提供了可行且可扩展的解决方案。
...
### [91] [Interpretable and Granular Video-Based Quantification of Motor Characteristics from the Finger Tapping Test in Parkinson Disease](https://arxiv.org/abs/2506.18925)
*Tahereh Zarrat Ehsan,Michael Tangermann,Yağmur Güçlütürk,Bastiaan R. Bloem,Luc J. W. Evers*
...
Method: 通过视频记录分析，提出四组临床相关特征（运动减少、运动迟缓、序列效应和犹豫停顿），并利用机器学习分类器预测MDS-UPDRS评分。

Result: 视频分析特征与四种运动缺陷对应，且能进一步区分序列效应和犹豫停顿。方法在MDS-UPDRS评分预测中优于现有技术。
...
### [92] [Reinforcement Learning-Based Dynamic Grouping for Tubular Structure Tracking](https://arxiv.org/abs/2506.18930)
*Chong Di,Shuwang Zhou,Da Chen,Jean-Marie Mirebeau,Minglei Shu,Laurent D. Cohen*
...
### [93] [Bird's-eye view safety monitoring for the construction top under the tower crane](https://arxiv.org/abs/2506.18938)
*Yanke Wang,Yu Hin Ng,Haobo Liang,Ching-Wei Chang,Hao Chen*
...
<details>
  <summary>Details</summary>
Motivation: 塔吊操作日益自动化，但安全保护仍需优先解决，尤其是工人和模块化建筑（MiC）的安全。

Method: 结合摄像头和LiDAR的3D数据融合技术，开发软件管道与硬件显示系统。

Result: 系统验证了方法的准确性和有效性，实际工地显示其可作为实用安全监控工具。
...
### [94] [Damba-ST: Domain-Adaptive Mamba for Efficient Urban Spatio-Temporal Prediction](https://arxiv.org/abs/2506.18939)
*Rui An,Yifeng Zhang,Ziran Liang,Wenqi Fan,Yuxuan Liang,Xuequn Shang,Qing Li*

Main category: cs.CV

TL;DR: Damba-ST是一种基于Mamba的高效城市时空预测模型，通过域自适应状态空间模型和域适配器，解决了跨域泛化问题，同时保持了线性计算复杂度。


<details>
  <summary>Details</summary>
Motivation: 解决现有Transformer模型在城市时空预测中计算复杂度高、内存开销大以及跨域泛化能力不足的问题。

Method: 提出Damba-ST模型，包括域自适应状态空间模型和三种域适配器，分别学习跨域共性和域内特征。

Result: Damba-ST在预测任务中表现优异，具有强零样本泛化能力，适用于新城市环境。

Conclusion: Damba-ST在效率和泛化能力上均优于现有方法，为城市时空预测提供了高效解决方案。
...
### [95] [From Pixels and Words to Waves: A Unified Framework for Spectral Dictionary vLLMs](https://arxiv.org/abs/2506.18943)
*Andrew Kiruluta,Priscilla Burity*
...
### [96] [DiffRIS: Enhancing Referring Remote Sensing Image Segmentation with Pre-trained Text-to-Image Diffusion Models](https://arxiv.org/abs/2506.18946)
*Zhe Dong,Yuzhe Sun,Tianzhu Liu,Yanfeng Gu*
...
<details>
  <summary>Details</summary>
Motivation: 当前遥感图像分割方法在处理复杂对象特性（如尺度变化、多样方向和语义模糊）时存在挑战，需要更高效的跨模态对齐方法。

Method: 提出DiffRIS框架，结合CP-adapter（上下文感知适配器）和PCMRD（渐进式跨模态推理解码器），优化语言特征与视觉区域的语义对齐。

Result: 在三个基准数据集上的实验表明，DiffRIS在所有标准指标上均优于现有方法，达到新的SOTA。

Conclusion: DiffRIS通过预训练扩散模型和自适应框架，显著提升了遥感图像分割的性能，验证了其有效性。
...
### [97] [GLIMPSE: Gradient-Layer Importance Mapping for Prompted Visual Saliency Explanation for Generative LVLMs](https://arxiv.org/abs/2506.18985)
*Guanxi Shen*

Main category: cs.CV

TL;DR: GLIMPSE是一个轻量级、模型无关的框架，用于可视化大型视觉语言模型（LVLM）在开放视觉问答（VQA）中的视觉和文本显著性区域，帮助理解模型行为并揭示幻觉和偏见。


<details>
  <summary>Details</summary>
Motivation: 尽管LVLM在视觉输入生成连贯响应方面取得了进展，但其视觉注意力分布和跨模态推理机制仍不透明，需要工具来诊断幻觉、偏见并提升模型透明度。

Method: GLIMPSE结合梯度加权注意力、自适应层传播和加权令牌聚合，生成响应级别的显著性热图，用于解释跨模态推理。

Result: GLIMPSE在人类对齐性上优于现有可解释性方法，能够追踪令牌级推理动态，分析幻觉、偏见和人类注意力偏差。

Conclusion: GLIMPSE为LVLM的跨模态推理提供了细粒度解释工具，有助于提升模型透明度和可解释性。
...
### [98] [Diffusion Transformer-to-Mamba Distillation for High-Resolution Image Generation](https://arxiv.org/abs/2506.18999)
*Yuan Yao,Yicong Hong,Difan Liu,Long Mai,Feng Liu,Jiebo Luo*
...
<details>
  <summary>Details</summary>
Motivation: 解决扩散Transformer中自注意力机制二次计算复杂度带来的高计算成本问题，同时探索线性复杂度Mamba模型在图像生成中的潜力。

Method: 提出T2MD方法，结合自注意力和Mamba的混合模型，通过层级教师强制和基于特征的知识蒸馏，实现从Transformer到Mamba的高效转换。

Result: 实验表明，T2MD能够以较低开销生成高质量的高分辨率图像（最高2048×2048），并验证了Mamba模型在非因果视觉输出生成中的可行性。

Conclusion: T2MD为高分辨率图像生成提供了一种高效且高质量的训练路径，同时展示了Mamba模型在视觉生成任务中的潜力。
...
### [99] [Orthogonal Projection Subspace to Aggregate Online Prior-knowledge for Continual Test-time Adaptation](https://arxiv.org/abs/2506.19022)
*Jinlong Li,Dong Zhao,Qi Zang,Zequn Jie,Lin Ma,Nicu Sebe*

Main category: cs.CV

TL;DR: 提出了一种名为OoPk的新方法，通过正交投影子空间和在线先验知识聚合策略，解决了持续测试时间适应中的灾难性遗忘和错误累积问题，并在语义分割任务中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有方法在平衡性能和高效模型适应方面存在困难，尤其是在复杂任务如语义分割中。

Method: 使用正交投影子空间保护预训练模型知识完整性，并结合在线先验知识聚合策略增强域适应能力。

Result: 在多个持续TTA基准测试中表现优于现有方法。

Conclusion: OoPk方法有效解决了灾难性遗忘和错误累积问题，提升了语义分割任务的性能。
...
### [100] [LEGATO: Large-scale End-to-end Generalizable Approach to Typeset OMR](https://arxiv.org/abs/2506.19065)
*Guang Yang,Victoria Ebert,Nazif Tamer,Luiza Pozzobon,Noah A. Smith*

Main category: cs.CV

TL;DR: Legato是一种端到端的Transformer模型，用于光学音乐识别（OMR），能够识别全页或多页排版乐谱，并生成ABC符号音乐格式。
...
Result: 在多种数据集上实现最先进的性能。

Conclusion: Legato在OMR任务中表现出强大的泛化能力，并通过多样化评估验证了其优越性。
...
### [101] [HAWAII: Hierarchical Visual Knowledge Transfer for Efficient Vision-Language Models](https://arxiv.org/abs/2506.19072)
*Yimu Wang,Mozhgan Nasr Azadani,Sean Sedwards,Krzysztof Czarnecki*
...
Conclusion: HAWAII框架高效整合多专家知识，显著提升模型性能且计算成本低。
...
### [102] [Reading Smiles: Proxy Bias in Foundation Models for Facial Emotion Recognition](https://arxiv.org/abs/2506.19079)
*Iosif Tsangko,Andreas Triantafyllopoulos,Adem Abdelmoula,Adria Mallol-Ragolta,Bjoern W. Schuller*

Main category: cs.CV

TL;DR: 本文探讨了视觉语言模型（VLMs）在情感计算中依赖的视觉线索，发现牙齿可见性影响性能，并揭示了GPT-4o依赖面部特征（如眉毛位置）进行情感推理。


<details>
  <summary>Details</summary>
Motivation: 研究VLMs在零样本情感识别中依赖的视觉线索是否具有心理学基础或仅是表面学习。

Method: 在AffectNet数据集的牙齿标注子集上对不同规模的VLMs进行基准测试，并通过结构化内省分析最佳模型GPT-4o。

Result: 发现牙齿可见性影响模型性能，且GPT-4o的情感推理高度依赖面部特征（如眉毛位置）。

Conclusion: 揭示了基础模型行为的涌现性，但也指出了在敏感领域（如心理健康和教育）中存在的捷径学习、偏见和公平性问题。
...
### [103] [RareSpot: Spotting Small and Rare Wildlife in Aerial Imagery with Multi-Scale Consistency and Context-Aware Augmentation](https://arxiv.org/abs/2506.19087)
*Bowen Zhang,Jesse T. Boulerice,Nikhil Kuniyil,Charvi Mendiratta,Satish Kumar,Hila Shamon,B. S. Manjunath*
...
<details>
  <summary>Details</summary>
Motivation: 由于小型稀有野生动物（如草原犬鼠）在生态中的重要性与其难以检测的特性（如体型小、分布稀疏）之间的矛盾，现有检测方法效果不佳，亟需更有效的解决方案。

Method: RareSpot结合多尺度一致性学习和上下文感知增强，前者通过特征金字塔对齐增强细粒度表示，后者通过合成困难样本提升模型性能。

Result: 在专家标注的草原犬鼠无人机影像基准测试中，RareSpot的检测精度比基线方法提高了35%以上，并在其他野生动物数据集中表现出良好的泛化能力。
...
### [104] [Inverse-and-Edit: Effective and Fast Image Editing by Cycle Consistency Models](https://arxiv.org/abs/2506.19103)
*Ilia Beletskii,Andrey Kuznetsov,Aibek Alanov*

Main category: cs.CV

TL;DR: 提出了一种基于一致性模型的新框架，显著提升了图像反转质量，仅需四步即可实现高质量编辑。


<details>
  <summary>Details</summary>
Motivation: 现有扩散模型在图像编辑中计算量大，而蒸馏模型的反转质量差，影响编辑效果。高保真反转和重建对精确编辑至关重要。

Method: 采用一致性模型和循环一致性优化策略，平衡可编辑性与内容保留。
...
### [105] [PrITTI: Primitive-based Generation of Controllable and Editable 3D Semantic Scenes](https://arxiv.org/abs/2506.19117)
*Christina Ourania Tze,Daniel Dauner,Yiyi Liao,Dzmitry Tsishkou,Andreas Geiger*
...
Method: 采用混合表示（栅格化地面和矢量化基元），引入稳定的Cholesky参数化解码对象大小和方向。

Result: 在KITTI-360数据集上表现优于体素基线，内存需求减少3倍，支持实例级操作。

Conclusion: PrITTI为3D场景生成提供了高效、灵活且可编辑的解决方案。
...
### [106] [Lightweight RGB-T Tracking with Mobile Vision Transformers](https://arxiv.org/abs/2506.19154)
*Mahdi Falaki,Maria A. Amer*
...
Method: 采用MobileViT构建轻量级模型，设计渐进式融合框架，结合可分离注意力学习模态内和模态间交互。
...
### [107] [PRISM: Perceptual Recognition for Identifying Standout Moments in Human-Centric Keyframe Extraction](https://arxiv.org/abs/2506.19168)
*Mert Can Cakmak,Nitin Agarwal,Diwash Poudel*

Main category: cs.CV

TL;DR: PRISM是一种轻量级、基于感知的关键帧提取框架，适用于实时和资源受限的环境。
...
### [108] [MOSCARD -- Causal Reasoning and De-confounding for Multimodal Opportunistic Screening of Cardiovascular Adverse Events](https://arxiv.org/abs/2506.19174)
*Jialu Pi,Juan Maria Farina,Rimita Lahiri,Jiwoong Jeong,Archana Gurudu,Hyung-Bok Park,Chieh-Ju Chao,Chadi Ayoub,Reza Arsanjani,Imon Banerjee*

Main category: cs.CV

TL;DR: 论文提出了一种多模态因果推理框架MOSCARD，整合胸片和心电图数据，用于心血管事件风险预测，优于单模态和现有模型。


<details>
  <summary>Details</summary>
Motivation: 心血管事件是全球主要死因，现有筛查方法受限于单模态和采样偏差，需要更全面的风险评估方法。

Method: 提出MOSCARD框架，结合多模态对齐、因果推理和去混杂双反向传播图，整合胸片和心电图数据。
...
### [109] [Assessing Risk of Stealing Proprietary Models for Medical Imaging Tasks](https://arxiv.org/abs/2506.19464)
*Ankita Raj,Harsh Swaika,Deepankar Varma,Chetan Arora*
...
<details>
  <summary>Details</summary>
Motivation: 随着深度学习在医学影像中的应用，许多公司部署了专有模型并提供收费服务。尽管模型权重被隐藏以保护知识产权，但这些模型仍面临模型窃取攻击的风险。目前，医学影像模型对此类攻击的脆弱性尚未充分研究。

Method: 论文提出了一种名为QueryWise的两步模型窃取方法，利用公开数据集和有限的查询预算，通过代理分布获取未标记数据训练窃取模型。

Result: 在胆囊癌和COVID-19分类模型中，QueryWise方法表现出高效性，证明了医学影像模型在有限查询预算下仍易受攻击。

Conclusion: 研究表明，医学影像模型在现实条件下易受模型窃取攻击，QueryWise方法为攻击者提供了更高效的窃取手段。
...
### [110] [OpenWildlife: Open-Vocabulary Multi-Species Wildlife Detector for Geographically-Diverse Aerial Imagery](https://arxiv.org/abs/2506.19204)
*Muhammed Patel,Javier Noa Turnes,Jayden Hsiao,Linlin Xu,David Clausi*

Main category: cs.CV

TL;DR: OpenWildlife (OW) 是一种开放词汇的野生动物检测器，用于多样化的航空图像中的多物种识别。它通过语言感知嵌入和 Grounding-DINO 框架的改进，实现了跨物种和环境的泛化能力，并在多个数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 现有自动化方法在特定场景下表现良好，但在跨物种和环境泛化方面表现不佳，主要受限于有限的分类覆盖范围和僵化的模型架构。

Method: OW 结合语言感知嵌入和 Grounding-DINO 框架的改进，支持自然语言输入识别物种，并引入高效的搜索算法优化检测效率。

Result: OW 在 15 个数据集上训练，表现优于现有方法，最高达到 0.981 mAP50（微调后）和 0.597 mAP50（新物种数据集）。搜索算法覆盖 95% 物种，仅需探索 33% 的图像。
...
TL;DR: 论文提出了一种检测和识别面部识别系统中自然触发器的后门攻击的方法，并在实验中展示了其有效性。


<details>
  <summary>Details</summary>
Motivation: 后门攻击通过隐藏触发器在面部识别系统中植入异常行为，对高安全性应用构成威胁。

Method: 提出了一种新技术，用于检测和识别被植入自然触发器的面部识别网络。

Result: 在实验中，该方法在识别触发器（如绿色太阳镜或红色帽子）时，top-5准确率达到74%，优于基线方法的56%。

Conclusion: 该方法能有效检测和识别面部识别系统中的自然触发器后门攻击，提升了安全性。
...
### [112] [Ancient Script Image Recognition and Processing: A Review](https://arxiv.org/abs/2506.19208)
*Xiaolei Diao,Rite Bo,Yanling Xiao,Lida Shi,Zhihan Zhou,Hao Xu,Chuntao Li,Xiongfeng Tang,Massimo Poesio,Cédric M. John,Daqian Shi*

Main category: cs.CV

TL;DR: 该论文综述了古代文字图像识别方法，分类分析了不同文字类型的识别技术，探讨了数据不平衡和图像退化等独特挑战，并总结了未来研究方向。
...
Method: 分类现有研究，分析不同文字类型的识别方法，并探讨针对数据不平衡和图像退化的解决方案。

Result: 总结了古代文字识别的共同挑战和专用方法，提出了未来研究方向。

Conclusion: 论文为古代文字的识别、解释和破译提供了结构化、前瞻性的视角。
...
### [113] [MedErr-CT: A Visual Question Answering Benchmark for Identifying and Correcting Errors in CT Reports](https://arxiv.org/abs/2506.19217)
*Sunggu Kyung,Hyungbin Park,Jinyoung Seo,Jimin Sung,Jihyun Kim,Dongyeong Kim,Wooyoung Jo,Yoojin Nam,Sangah Park,Taehee Kwon,Sang Min Lee,Namkug Kim*
...
<details>
  <summary>Details</summary>
Motivation: CT检查需求增加导致诊断错误风险上升，现有医学视觉问答（VQA）基准缺乏临床相关性和专家级知识评估，需要更严格的验证。

Method: 引入MedErr-CT基准，包含六类错误（四种视觉相关，两种词汇相关）和三个任务级别（分类、检测、纠正），用于评估3D医学MLLMs的性能。

Result: 评估显示不同MLLMs在不同错误类型上的能力差异显著。

Conclusion: MedErr-CT有助于开发更可靠、临床适用的MLLMs，减少诊断错误并提高临床准确性。
...
### [114] [Video-XL-2: Towards Very Long-Video Understanding Through Task-Aware KV Sparsification](https://arxiv.org/abs/2506.19225)
*Minghao Qin,Xiangrui Liu,Zhengyang Liang,Yan Shu,Huaying Yuan,Juenjie Zhou,Shitao Xiao,Bo Zhao,Zheng Liu*
...
Method: 采用基于分块的预填充和双层键值解码技术，分块内使用全注意力，分块间使用稀疏注意力，解码时根据任务相关性选择加载密集或稀疏键值。

Result: Video-XL-2在多个长视频理解基准测试中达到最先进性能，并能在单张A100 GPU上高效处理超过10,000帧视频。

Conclusion: Video-XL-2通过创新的稀疏化技术，成功解决了长视频理解中的效率和性能问题，为未来研究提供了重要参考。
...
### [115] [MSR-Align: Policy-Grounded Multimodal Alignment for Safety-Aware Reasoning in Vision-Language Models](https://arxiv.org/abs/2506.19257)
*Yinan Xia,Yilei Jiang,Yingshui Tan,Xiaoyong Zhu,Xiangyu Yue,Bo Zheng*
...
<details>
  <summary>Details</summary>
Motivation: 现有安全性对齐方法主要针对单模态语言模型，无法应对多模态输入带来的复杂安全威胁，且当前数据集缺乏细粒度的政策基础推理。

Method: 通过构建高质量的多模态安全推理数据集MSR-Align，支持跨视觉和文本模态的细粒度、政策基础推理，并采用严格的质量过滤。

Result: 实验表明，基于MSR-Align微调的VLMs显著提升了对抗文本和视觉语言攻击的鲁棒性，同时保持或增强通用推理性能。

Conclusion: MSR-Align为提升多模态VLMs的安全性对齐提供了可扩展且有效的基础，数据集已公开。
...
### [116] [Automated Image Recognition Framework](https://arxiv.org/abs/2506.19261)
*Quang-Binh Nguyen,Trong-Vu Hoang,Ngoc-Do Tran,Tam V. Nguyen,Minh-Triet Tran,Trung-Nghia Le*

Main category: cs.CV

TL;DR: 提出了一种基于生成AI的自动图像识别框架（AIR），通过生成高质量预标注数据集和自动训练模型，解决了数据收集和标注的难题。
...
Result: 实验证明生成的数据能有效训练深度学习模型，用户研究评分4.4/5.0。
...
### [117] [3D-SSM: A Novel 3D Selective Scan Module for Remote Sensing Change Detection](https://arxiv.org/abs/2506.19263)
*Rui Huang,Jincheng Zeng,Sen Gao,Yan Xing*

Main category: cs.CV

TL;DR: 提出3D-SSM模块解决Mamba方法在遥感变化检测中长距离依赖问题，结合SIM和MBFEM提升特征表示能力，实验验证优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 现有Mamba方法在遥感变化检测中因无法有效捕捉图像通道间的长距离依赖，限制了特征表示能力。

Method: 提出3D-SSM模块从空间和通道角度捕获全局信息，结合SIM实现双时相特征交互，MBFEM融合多域特征。
...
### [118] [Self-Paced Collaborative and Adversarial Network for Unsupervised Domain Adaptation](https://arxiv.org/abs/2506.19267)
*Weichen Zhang,Dong Xu,Wanli Ouyang,Wen Li*

Main category: cs.CV

TL;DR: 本文提出了一种名为协作对抗网络（CAN）的无监督域自适应方法，结合域协作和域对抗学习策略，通过正负权重损失统一两种学习策略，并设计了协作对抗训练方案。进一步提出自步调CAN（SPCAN）增强目标域判别性，实验证明其性能优越。
...
Method: 采用域协作学习保留目标域判别性，域对抗学习减少域分布差异，统一为带正负权重的域分类器学习。设计协作对抗训练方案，并引入自步调学习策略选择伪标签目标样本。
...
### [119] [AirV2X: Unified Air-Ground Vehicle-to-Everything Collaboration](https://arxiv.org/abs/2506.19283)
*Xiangbo Gao,Yuheng Wu,Xuewen Luo,Keshu Wu,Xinghao Chen,Yuping Wang,Chenxi Liu,Yang Zhou,Zhengzhong Tu*

Main category: cs.CV

TL;DR: AirV2X-Perception是一个基于无人机的多车协同驾驶感知数据集，填补了传统V2X系统在覆盖范围和成本上的不足。


<details>
  <summary>Details</summary>
Motivation: 传统基础设施V2X系统部署成本高且存在覆盖盲区，无人机提供了灵活、低成本的替代方案。

Method: 利用无人机采集6.73小时的多场景驾驶数据，覆盖城市、郊区和农村环境。

Result: 数据集支持V2D算法开发，提供标准化评估基准。

Conclusion: AirV2X-Perception为无人机辅助自动驾驶系统的发展提供了重要资源。
...
### [120] [Da Yu: Towards USV-Based Image Captioning for Waterway Surveillance and Scene Understanding](https://arxiv.org/abs/2506.19288)
*Runwei Guan,Ningwei Ouyang,Tianhao Xu,Shaofeng Liang,Wei Dai,Yafeng Sun,Shang Gao,Songning Lai,Shanliang Yao,Xuming Hu,Ryan Wen Liu,Yutao Yue,Hui Xiong*
...
Method: 利用视觉语言模型（VLMs）和图像描述技术，提出WaterCaption数据集和Da Yu模型，采用Nano Transformer Adaptor（NTA）进行视觉特征建模。

Result: Da Yu在WaterCaption和其他描述基准上表现优异，平衡了性能与效率。

Conclusion: WaterCaption和Da Yu为水道环境提供了新的语义理解研究方向，并显著提升了长文本生成能力。
...
### [121] [HoliGS: Holistic Gaussian Splatting for Embodied View Synthesis](https://arxiv.org/abs/2506.19291)
*Xiaoyuan Wang,Yizhou Zhao,Botao Ye,Xiaojun Shan,Weijie Lyu,Lu Qi,Kelvin C. K. Chan,Yinxiao Li,Ming-Hsuan Yang*

Main category: cs.CV

TL;DR: HoliGS是一种新颖的可变形高斯溅射框架，用于从长单目RGB视频中合成视角。它通过分层变形策略实现高质量的自由视角渲染，显著优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 解决现有4D高斯溅射和动态NeRF方法在长视频训练中的高开销问题，提供更高效、准确的动态场景重建方案。

Method: 将场景分解为静态背景和时间变化对象，使用可逆高斯溅射变形网络处理全局刚性变换、骨架驱动和非刚性变形。

Result: 在挑战性数据集上实现更优的重建质量，同时显著减少训练和渲染时间。

Conclusion: HoliGS为实际场景中的EVS提供了实用且可扩展的解决方案。
...
### [122] [Open-Vocabulary Camouflaged Object Segmentation with Cascaded Vision Language Models](https://arxiv.org/abs/2506.19300)
*Kai Zhao,Wubang Yuan,Zheng Wang,Guanyi Li,Xiaoqiang Zhu,Deng-ping Fan,Dan Zeng*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法在分割伪装目标时存在域差距和边界模糊问题，导致分割和分类效果不佳。

Method: 利用VLM引导的Segment Anything Model（SAM）进行分割，并通过VLM特征作为提示；分类阶段避免硬裁剪，使用alpha通道保留上下文。

Result: 在OVCOS和传统伪装目标分割基准上表现出明显优势。

Conclusion: 通过VLM语义的丰富性，显著提升了伪装目标的分割和分类效果。
...
### [123] [Airway Skill Assessment with Spatiotemporal Attention Mechanisms Using Human Gaze](https://arxiv.org/abs/2506.19306)
*Jean-Paul Ainam,Rahul,Lora Cavuoto,Matthew Hackett,Jack Norfleet,Suvranu De*

Main category: cs.CV

TL;DR: 提出一种基于机器学习和人类注视数据的ETI技能评估方法，通过注意力机制和视觉掩码提升识别准确性。


<details>
  <summary>Details</summary>
Motivation: 传统主观评估方法难以准确衡量实际场景中的ETI技能，需要更客观的工具。

Method: 结合人类注视数据和视频，利用注意力机制和自动编码器网络提取特征并分类。

Result: 模型在准确性、敏感性和可信度上优于传统方法。

Conclusion: 该方法为临床技能评估提供了更可靠的工具，尤其适用于高压环境。
...
### [124] [Capturing Fine-Grained Alignments Improves 3D Affordance Detection](https://arxiv.org/abs/2506.19312)
*Junsei Tokumitsu,Yuiga Wada*
...
Method: 提出LM-AD方法，引入AQM模块，利用预训练语言模型捕获细粒度对齐。

Result: 在3D AffordanceNet数据集上，准确率和mIoU均优于现有方法。

Conclusion: LM-AD通过AQM模块有效解决了细粒度对齐问题，性能显著提升。
...
### [125] [Progressive Modality Cooperation for Multi-Modality Domain Adaptation](https://arxiv.org/abs/2506.19316)
*Weichen Zhang,Dong Xu,Jing Zhang,Wanli Ouyang*
...
<details>
  <summary>Details</summary>
Motivation: 解决多模态域适应问题，特别是在目标域中部分模态缺失的情况下，如何有效利用源域的多模态数据。

Method: 提出PMC框架，包含两个新模块，用于选择可靠的伪标记目标样本；在MMDA-PI设置下，提出PMC-PI方法，通过MMG网络生成缺失模态。
...
### [126] [Continual Retinal Vision-Language Pre-training upon Incremental Imaging Modalities](https://arxiv.org/abs/2506.19320)
*Yuang Yao,Ruiqi Wu,Yi Zhou,Tao Zhou*

Main category: cs.CV

TL;DR: 提出RetCoP，首个眼底领域的持续视觉语言预训练框架，整合多模态数据并减少灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 传统眼底图像分析模型忽视多模态互补性，且现有基础模型多为模态特定，需解决动态环境中多模态数据增量整合问题。

Method: 提出RetCoP框架，通过代表性图像-文本对的重放策略和非对角线信息蒸馏方法，持续整合多模态数据。

Result: 实验表明RetCoP优于其他方法，具有最佳泛化能力和最低遗忘率。

Conclusion: RetCoP成功整合多模态数据并减少遗忘，为眼底领域持续学习提供有效解决方案。
...
### [127] [Memory-Augmented Incomplete Multimodal Survival Prediction via Cross-Slide and Gene-Attentive Hypergraph Learning](https://arxiv.org/abs/2506.19324)
*Mingcheng Qu,Guang Yang,Donglin Di,Yue Gao,Tonghua Su,Yang Song,Lei Fan*

Main category: cs.CV

TL;DR: 提出一种基于超图学习的多模态生存预测框架，整合多切片病理和基因组数据，解决模态不平衡问题，并通过记忆机制处理不完整数据。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖FFPE切片与基因组数据，忽略其他保存方式（如FF切片），且高分辨率病理数据主导融合过程，导致模态不平衡和不完整数据问题。

Method: 采用超图学习整合多切片病理和基因组数据，引入记忆机制动态补偿不完整模态。

Result: 在五个TCGA数据集上，模型C-Index提升2.3%，在不完整模态下优于仅病理（3.3%）和仅基因（7.9%）模型。
...
### [128] [Comparative Performance of Finetuned ImageNet Pre-trained Models for Electronic Component Classification](https://arxiv.org/abs/2506.19330)
*Yidi Shao,Longfei Zhou,Fangshuo Tang,Xinyi Shi,Dalang Chen,Shengtao Xia*
...
<details>
  <summary>Details</summary>
Motivation: 电子元件分类在制造业中至关重要，可降低人工成本并推动技术发展。预训练模型（尤其是ImageNet）在图像分类中表现优异，适合数据有限的研究。

Method: 比较了12种ImageNet预训练模型在电子元件分类任务中的表现。
...
### [129] [Segment Any 3D-Part in a Scene from a Sentence](https://arxiv.org/abs/2506.19331)
*Hongyu Wu,Pengwan Yang,Yuki M. Asano,Cees G. M. Snoek*

Main category: cs.CV

TL;DR: 提出了一种基于自然语言描述的3D场景部分分割方法，解决了数据和方法的挑战，并引入了3D-PU数据集和OpenPart3D框架。


<details>
  <summary>Details</summary>
Motivation: 传统3D场景理解局限于对象级别，缺乏部分级别的数据和标注方法。

Method: 引入3D-PU数据集和OpenPart3D框架，通过合成3D场景实现部分级标注。

Result: 实验表明，该方法在部分级3D场景理解任务中表现优越，具有强泛化能力。

Conclusion: 该方法为高级3D部分场景理解提供了新的数据和解决方案。
...
### [130] [Trajectory Prediction in Dynamic Object Tracking: A Critical Study](https://arxiv.org/abs/2506.19341)
*Zhongping Dong,Liming Chen,Mohand Tahar Kechadi*

Main category: cs.CV

TL;DR: 本文综述了动态目标跟踪（DOT）和轨迹预测（TP）方法的最新进展、应用及挑战，探讨了多种方法的优缺点，并提出了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 分析DOT和TP技术的现状及其在多个领域的应用，以推动技术进步并解决现有挑战。

Method: 综述了特征、分割、估计和学习等多种方法，并评估其在实际场景中的表现。

Result: DOT和TP技术在安全性和效率方面有显著影响，但仍存在泛化性、计算效率和数据依赖等问题。
...
### [131] [Image Segmentation using Chan-Vese Active Contours](https://arxiv.org/abs/2506.19344)
*Pranav Shenoy K. P*

Main category: cs.CV

TL;DR: 本文全面推导并实现了Chan-Vese主动轮廓模型用于图像分割，基于区域强度差异而非梯度，适用于噪声或弱边界图像。


<details>
  <summary>Details</summary>
Motivation: 解决噪声或弱边界图像的分割问题，提供一种基于区域强度差异的鲁棒方法。

Method: 基于Mumford-Shah变分框架，通过水平集公式和曲线演化理论推导模型，采用有限差分法实现，注重数值稳定性。

Result: 在医学和合成图像上实验，展示了准确分割、抗噪性强，优于传统基于边缘的方法。
...
### [132] [Training-Free Motion Customization for Distilled Video Generators with Adaptive Test-Time Distillation](https://arxiv.org/abs/2506.19348)
*Jintao Rong,Xin Xie,Xinyi Yu,Linlin Ou,Xinyu Zhang,Chunhua Shen,Dong Gong*
...
<details>
  <summary>Details</summary>
Motivation: 蒸馏视频生成模型在快速合成方面表现优异，但在参考视频引导的运动定制上存在困难，尤其是在无需训练的情况下。现有方法无法适应蒸馏模型的快速生成过程和大去噪步骤。

Method: 提出MotionEcho框架，利用高质量慢速教师模型通过端点预测和插值指导快速学生模型，并根据引导需求动态分配计算资源。

Result: 在多种蒸馏视频生成模型和基准数据集上的实验表明，该方法显著提升了运动保真度和生成质量，同时保持了高效率。

Conclusion: MotionEcho通过教师强制和动态计算分配，成功解决了蒸馏模型在运动定制上的局限性，为高效视频生成提供了新思路。
...
### [133] [Online camera-pose-free stereo endoscopic tissue deformation recovery with tissue-invariant vision-biomechanics consistency](https://arxiv.org/abs/2506.19388)
*Jiahe Chen,Naoki Tomii,Ichiro Sakuma,Etsuko Kobayashi*

Main category: cs.CV

TL;DR: 提出了一种基于立体内窥镜图像的组织变形恢复方法，解决了相机运动、遮挡、大变形等问题，并引入了局部变形图和在线优化。
...
### [134] [Emergence of Text Readability in Vision Language Models](https://arxiv.org/abs/2506.19389)
*Jaeyoo Park,Sanghyuk Chun,Wonjae Kim,Sangdoo Yun,Bohyung Han*
...
<details>
  <summary>Details</summary>
Motivation: 探究视觉语言模型在训练过程中如何发展文本识别能力，并揭示其与语义理解能力的不同发展模式。

Method: 通过分析VLMs的训练过程，观察文本可读性和语义理解能力的发展趋势。

Result: 文本可读性在大量训练迭代后突然出现，而语义理解能力则逐渐发展；图像与渲染文本的匹配能力发展更慢。

Conclusion: 研究强调了需要针对性的训练策略来加速VLMs的文本理解能力，为未来多模态学习的优化研究奠定了基础。
...
### [135] [Generate the Forest before the Trees -- A Hierarchical Diffusion model for Climate Downscaling](https://arxiv.org/abs/2506.19391)
*Declan J. Curran,Sanaa Hobeichi,Hira Saleem,Hao Xue,Flora D. Salim*
...
<details>
  <summary>Details</summary>
Motivation: 传统降尺度方法计算量大，AI降尺度模型（如扩散模型）虽有效但计算仍密集。HDD旨在提供轻量级解决方案。
...
### [136] [A Global-Local Cross-Attention Network for Ultra-high Resolution Remote Sensing Image Semantic Segmentation](https://arxiv.org/abs/2506.19406)
*Chen Yi,Shan LianLei*

Main category: cs.CV

TL;DR: GLCANet是一种轻量级分割框架，用于超高分辨率遥感图像，通过双流架构和自注意力机制提升语义分割的准确性和计算效率。
...
Method: GLCANet采用双流架构融合全局语义和局部细节，结合自注意力机制和掩码交叉注意力机制优化特征融合。

Result: 实验表明，GLCANet在准确性和计算效率上优于现有方法，能够高效处理大尺寸高分辨率图像。
...
### [137] [EvDetMAV: Generalized MAV Detection from Moving Event Cameras](https://arxiv.org/abs/2506.19416)
*Yin Zhang,Zian Ning,Xiaoyu Zhang,Shiliang Guo,Peidong Liu,Shiyu Zhao*

Main category: cs.CV

TL;DR: 本文提出了一种基于事件相机的MAV检测方法，利用高速旋转螺旋桨在事件流中的特征，显著提升了检测性能。


<details>
  <summary>Details</summary>
Motivation: 现有MAV检测方法依赖RGB图像的外观特征，多样性导致泛化能力不足。事件流中螺旋桨的独特特征为解决这一问题提供了新思路。

Method: 提出三个模块，从原始事件流中提取螺旋桨的显著时空特征，同时过滤背景噪声和相机运动干扰。

Result: 在未训练的情况下，方法显著优于现有技术，测试数据集上精确率和召回率分别达到83.0%和81.5%。

Conclusion: 该方法为MAV检测提供了新方向，并发布了首个事件基MAV数据集，推动相关研究。
...
### [138] [Mem4Nav: Boosting Vision-and-Language Navigation in Urban Environments with a Hierarchical Spatial-Cognition Long-Short Memory System](https://arxiv.org/abs/2506.19433)
*Lixuan He,Haoyu Dong,Zhenxing Chen,Yangcheng Yu,Jie Feng,Yong Li*

Main category: cs.CV

TL;DR: Mem4Nav是一种层次化空间认知长短记忆系统，用于增强视觉与语言导航（VLN）任务，通过结合稀疏八叉树和语义拓扑图，显著提升了任务完成率和导航效率。
...
Method: 提出Mem4Nav，结合稀疏八叉树（精细体素索引）和语义拓扑图（高层地标连接），通过可逆Transformer嵌入存储为可训练内存令牌。长短记忆模块分别处理历史观察和实时多模态输入。

Result: 在Touchdown和Map2Seq数据集上，Mem4Nav显著提升了任务完成率（7-13pp）、SPD降低和nDTW改进（>10pp）。

Conclusion: Mem4Nav通过层次化地图和双记忆模块，显著提升了VLN任务的性能，证明了其设计的必要性。
...
### [139] [AMF-MedIT: An Efficient Align-Modulation-Fusion Framework for Medical Image-Tabular Data](https://arxiv.org/abs/2506.19439)
*Congjing Yu,Jing Ye,Yang Liu,Xiaodong Zhang,Zhiyong Zhang*

Main category: cs.CV

TL;DR: AMF-MedIT提出了一种高效的医学图像和表格数据融合框架，通过自适应调制和融合模块解决跨模态差异和噪声问题，并在数据稀缺条件下表现优异。


<details>
  <summary>Details</summary>
Motivation: 解决医学多模态分析中图像和表格数据融合的挑战，包括特征维度差异、模态贡献动态调整以及高维表格数据的噪声问题。

Method: 提出AMF模块（自适应调制和融合），结合调制目标和模态置信比，并引入特征掩码、密度和泄漏损失；同时设计FT-Mamba表格编码器处理噪声数据。

Result: AMF-MedIT在多模态性能和数据效率之间取得优越平衡，对不完整表格数据表现出强适应性。

Conclusion: AMF-MedIT为医学多模态分析提供了一种高效且适应性强的解决方案，FT-Mamba在特征提取和图像编码器指导方面表现出色。
...
### [140] [Sampling Matters in Explanations: Towards Trustworthy Attribution Analysis Building Block in Visual Models through Maximizing Explanation Certainty](https://arxiv.org/abs/2506.19442)
*Róisín Luo,James McDermott,Colm O'Riordan*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法通过向图像添加噪声作为样本，但噪声分布可能导致解释确定性低。此外，额外信息可能使神经网络饱和。

Method: 提出半最优采样方法，通过抑制输入特征生成样本，使样本分布接近自然图像分布。

Result: 在ImageNet上的大规模定量评估表明，该方法有效且优于现有基线。

Conclusion: 解决样本分布不对齐问题是构建可信赖的归因分析的关键，抑制特征的方法能提供更满意的解释。
...
### [141] [Deblurring in the Wild: A Real-World Dataset from Smartphone High-Speed Videos](https://arxiv.org/abs/2506.19445)
*Mahdi Mohd Hossain Noki,Syed Mumtahin Mahmud,Prothito Shovon Majumder,Abdul Mohaimen Al Radi,Md. Haider Ali,Md. Mosaddek Khan*

Main category: cs.CV

TL;DR: 论文提出了一个基于智能手机慢动作视频构建的最大真实世界图像去模糊数据集，包含42,000对高分辨率模糊-清晰图像对，并展示了现有SOTA模型的性能下降。
...
Method: 通过智能手机慢动作视频（240帧/秒）模拟真实长曝光模糊，生成模糊图像，并使用中间帧作为清晰参考。

Result: 数据集规模是现有常用数据集的10倍，场景多样性更高，且测试显示SOTA模型性能显著下降。

Conclusion: 该数据集为去模糊研究提供了更具挑战性和多样性的基准，有助于开发更鲁棒和泛化的模型。
...
### [142] [Stylized Structural Patterns for Improved Neural Network Pre-training](https://arxiv.org/abs/2506.19465)
*Farnood Salehi,Vandit Sharma,Amirhossein Askari Farsangi,Tunç Ozan Aydın*

Main category: cs.CV

TL;DR: 提出了一种两步法改进合成数据质量，通过神经分形和改进的反向风格化技术，显著缩小合成数据与真实图像的分布差距，提升模型性能。


<details>
  <summary>Details</summary>
Motivation: 解决真实图像数据集难以获取且存在隐私和法律问题，同时现有合成数据训练模型性能不足的问题。

Method: 1. 改进神经分形生成新的合成数据；2. 提出反向风格化技术，将少量真实图像特征迁移到合成数据中。

Result: 显著降低了合成数据与真实图像的分布差距（KID指标），在图像生成、数据表示和分类任务中分别提升了11% FID、20%重建误差和10% ImageNet-100准确率。

Conclusion: 该方法为无法获取大规模真实数据时的模型训练提供了实用解决方案，具有广泛的应用潜力。
...
### [143] [Surgery-R1: Advancing Surgical-VQLA with Reasoning Multimodal Large Language Model via Reinforcement Learning](https://arxiv.org/abs/2506.19469)
*Pengfei Hao,Shuaibo Li,Hongqiu Wang,Zhizhuo Kou,Junhang Zhang,Guang Yang,Lei Zhu*
...
Method: 构建Surgery-R1-54k数据集，提出两阶段微调机制（监督微调和强化微调），并设计多模态一致性奖励机制以优化推理能力。

Result: Surgery-R1在Surgical-VQLA任务中表现优于现有SOTA模型和广泛使用的MLLM，验证了其推理能力和方法的有效性。
...
### [144] [Recurrent Visual Feature Extraction and Stereo Attentions for CT Report Generation](https://arxiv.org/abs/2506.19665)
*Yuanhe Tian,Lei Mao,Yan Song*
...
Conclusion: 该方法有效整合了CT切片的多层次特征，提升了报告生成的准确性和有效性。
...
### [145] [USIS16K: High-Quality Dataset for Underwater Salient Instance Segmentation](https://arxiv.org/abs/2506.19472)
*Lin Hong,Xin Wang,Yihao Li,Xia Wang*
...
<details>
  <summary>Details</summary>
Motivation: 受生物视觉系统选择性注意机制的启发，解决水下环境中显著物体或区域的识别问题，填补水下显著实例分割领域的数据集和研究空白。

Method: 构建USIS16K数据集，包含多样化的水下图像和高质量实例级标注，并进行基准评估。

Result: USIS16K数据集在多样性、复杂性和可扩展性方面显著提升，为水下目标检测和显著实例分割任务提供了新基准。
...
### [146] [HMSViT: A Hierarchical Masked Self-Supervised Vision Transformer for Corneal Nerve Segmentation and Diabetic Neuropathy Diagnosis](https://arxiv.org/abs/2506.19474)
*Xin Zhang,Liangxiu Han,Yue Shi,Yanlin Zheng,Alam Uazman,Maryam Ferdousi,Rayaz Malik*

Main category: cs.CV

TL;DR: HMSViT是一种新型分层掩码自监督视觉Transformer，用于角膜神经分割和糖尿病周围神经病变诊断，通过多尺度特征提取和自监督学习，显著提升了性能。
...
### [147] [SceneCrafter: Controllable Multi-View Driving Scene Editing](https://arxiv.org/abs/2506.19488)
*Zehao Zhu,Yuliang Zou,Chiyu Max Jiang,Bo Sun,Vincent Casser,Xiukun Huang,Jiahao Wang,Zhenpei Yang,Ruiqi Gao,Leonidas Guibas,Mingxing Tan,Dragomir Anguelov*

Main category: cs.CV

TL;DR: 论文提出SceneCrafter，一种用于多摄像头捕捉的驾驶场景的3D一致编辑工具，解决现有生成模型在自动驾驶仿真中的不足。


<details>
  <summary>Details</summary>
Motivation: 现有纯合成场景缺乏现实基础，难以保证仿真结果的相关性。编辑模型利用真实驾驶日志，但面临3D一致性、空街先验学习和配对数据生成等挑战。

Method: 基于多视角扩散模型，提出可控框架支持多模态条件编辑，并引入新框架生成几何一致的合成配对数据，结合alpha混合框架处理局部编辑。

Result: SceneCrafter在真实性、可控性、3D一致性和场景编辑质量上优于现有基线。

Conclusion: SceneCrafter为自动驾驶仿真提供了强大的编辑能力，解决了关键挑战，提升了仿真效果。
...
### [148] [ScaleCap: Inference-Time Scalable Image Captioning via Dual-Modality Debiasing](https://arxiv.org/abs/2506.19848)
*Long Xing,Qidong Huang,Xiaoyi Dong,Pan Zhang,Yuhang Zang,Yuhang Cao,Jinsong Li,Shuangrui Ding,Weiming Zhang,Nenghai Yu,Jiaqi Wang,Feng Wu,Dahua Lin*

Main category: cs.CV

TL;DR: ScaleCap是一种推理时可扩展的图像描述生成策略，通过逐步增加推理预算生成更全面和详细的图像描述，解决了LVLMs中的多模态和语言偏见问题。


<details>
  <summary>Details</summary>
Motivation: 解决LVLMs中多模态偏见导致的描述粒度不平衡和语言偏见导致的虚假描述问题。

Method: 提出可扩展的去偏见描述策略，包括启发式问答和对比句子评分两个组件。

Result: 在11个基准测试中表现优异，生成的描述更准确、平衡且信息丰富。

Conclusion: ScaleCap通过逐步增加推理预算有效提升了图像描述的质量和多样性。
...
### [149] [Visual hallucination detection in large vision-language models via evidential conflict](https://arxiv.org/abs/2506.19513)
*Tao Huang,Zhekun Liu,Rui Wang,Yang Zhang,Liping Jing*
...
<details>
  <summary>Details</summary>
Motivation: LVLMs在视觉输入与文本输出之间存在不一致（视觉幻觉），这在安全关键应用中存在风险，需要评估基准和检测方法。

Method: 开发了PRE-HAL数据集评估感知和推理能力；提出基于DST的视觉幻觉检测方法，通过不确定性估计捕捉高级特征冲突。

Result: 新方法在LLaVA-v1.5、mPLUG-Owl2和mPLUG-Owl3上优于基线，AUROC平均提升4%、10%和7%。

Conclusion: PRE-HAL数据集和DST方法有效揭示了LVLMs的视觉幻觉问题，提升了检测性能。
...
### [150] [ReMAR-DS: Recalibrated Feature Learning for Metal Artifact Reduction and CT Domain Transformation](https://arxiv.org/abs/2506.19531)
*Mubashara Rehman,Niki Martinel,Michele Avanzo,Riccardo Spizzo,Christian Micheloni*
...
Result: 通过定性和定量评估验证，模型能有效减少伪影并保留解剖结构，减少重复MVCT扫描的需求。

Conclusion: 该框架为放疗计划提供了高质量的MVCT重建，降低了患者的辐射剂量，具有临床实用价值。
...
### [151] [General Methods Make Great Domain-specific Foundation Models: A Case-study on Fetal Ultrasound](https://arxiv.org/abs/2506.19552)
*Jakob Ambsdorf,Asbjørn Munk,Sebastian Llambias,Anders Nymark Christensen,Kamil Mikolaj,Randall Balestriero,Martin Tolsgaard,Aasa Feragen,Mads Nielsen*

Main category: cs.CV

TL;DR: 该论文探讨了在医学数据上预训练定制基础模型与从通用模型迁移学习的优劣，并通过胎儿超声数据集案例研究表明，定制预训练即使数据较少也能优于通用模型，且无需复杂方法创新。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决在医学数据上是否应预训练定制基础模型或使用迁移学习的问题，以及是否需要新方法。

Method: 采用DINOv2方法在2M胎儿超声图像上预训练基础模型，并与自然图像、超声图像预训练模型及监督基线进行比较。

Result: 结果表明：(i) 定制预训练优于通用模型，即使数据较少；(ii) 计算机视觉方法可直接用于医学领域，无需复杂调整。

Conclusion: 结论是应避免方法创新偏见，在资源限制下直接使用现有方法开发领域专用基础模型。
...
### [152] [MambaOutRS: A Hybrid CNN-Fourier Architecture for Remote Sensing Image Classification](https://arxiv.org/abs/2506.19561)
*Minjong Cheon,Changbae Mun*
...
<details>
  <summary>Details</summary>
Motivation: 尽管状态空间模型（SSMs）在视觉任务中表现出色，但其在2D视觉数据上的适应通常需要复杂修改，可能降低效率。本文旨在探索是否可以通过其他方法替代SSMs。

Method: MambaOutRS采用四阶段分层设计，结合门控CNN块进行局部特征提取，并引入傅里叶滤波门（FFG）模块在频域捕获全局上下文信息。

Result: 在UC Merced、AID等数据集上，MambaOutRS显著优于现有基线模型（包括更大的Transformer和Mamba架构），且参数更少。例如，MambaOutRS-t在UC Merced上F1-score达98.41%。

Conclusion: 研究表明，通过门控卷积和频域全局上下文模块的组合，可以高效替代复杂的SSMs，为遥感和其他视觉领域提供高性能且计算高效的模型。
...
### [153] [SMARTIES: Spectrum-Aware Multi-Sensor Auto-Encoder for Remote Sensing Images](https://arxiv.org/abs/2506.19585)
*Gencer Sumbul,Chang Xu,Emanuele Dalsasso,Devis Tuia*

Main category: cs.CV

TL;DR: SMARTIES是一种通用的基础模型，能够处理多种遥感传感器数据，通过将异构传感器数据映射到共享的频谱感知空间，实现跨传感器的可扩展性和泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有深度学习模型通常针对单一传感器或固定组合，限制了跨传感器的可扩展性和泛化能力。SMARTIES旨在解决这一问题，实现灵活的多传感器数据处理。

Method: SMARTIES通过将异构传感器数据投影到共享的频谱感知空间，并使用统一的Transformer模型重建掩码多传感器数据，结合跨传感器标记混合技术。

Result: SMARTIES在单模态和多模态任务中均优于依赖传感器特定预训练的模型。

Conclusion: SMARTIES为多传感器遥感数据处理提供了一种灵活且可扩展的解决方案，显著提升了模型的泛化能力。
...
### [154] [Vision Transformer-Based Time-Series Image Reconstruction for Cloud-Filling Applications](https://arxiv.org/abs/2506.19591)
*Lujun Li,Yiqun Wang,Radu State*

Main category: cs.CV

TL;DR: 提出了一种基于Vision Transformer的时间序列MSI图像重建框架，结合SAR数据，有效解决云覆盖问题。


<details>
  <summary>Details</summary>
Motivation: 云覆盖导致多光谱影像（MSI）数据缺失或损坏，影响早期作物测绘。合成孔径雷达（SAR）数据不受云干扰但缺乏光谱细节。

Method: 利用时间序列MSI和SAR的互补信息，通过Vision Transformer的注意力机制重建云覆盖区域的MSI数据。

Result: 实验表明，该框架显著优于仅使用非时间序列MSI和SAR或仅时间序列MSI的基线方法。

Conclusion: 提出的Time-series ViT框架有效提升了云覆盖区域的MSI图像重建质量。
...
### [155] [Implementing blind navigation through multi-modal sensing and gait guidance](https://arxiv.org/abs/2506.19593)
*Feifan Yan,Tianle Zeng,Meixi He*
...
<details>
  <summary>Details</summary>
Motivation: 全球视力障碍人群超过2.2亿，传统辅助工具如导盲杖和导盲犬存在不足，需更有效的导航解决方案。

Method: 提出一种可穿戴盲人导航设备，结合步态分析和多模态环境感知技术。

Result: 室内外实验显示，该设备在盲人导航方面表现优于传统导盲杖。

Conclusion: 该设备为视力障碍者提供了更高效的导航辅助工具。
...
### [156] [Self-Supervised Multimodal NeRF for Autonomous Driving](https://arxiv.org/abs/2506.19615)
*Gaurav Sharma,Ravi Kothari,Josef Schmid*

Main category: cs.CV

TL;DR: 提出了一种基于NeRF的自监督框架NVSF，用于联合学习LiDAR和Camera的时空场景表示，无需3D标签，在KITTI-360数据集上表现最佳。
...
### [157] [VideoPCDNet: Video Parsing and Prediction with Phase Correlation Networks](https://arxiv.org/abs/2506.19621)
*Noel José Rodrigues Vicente,Enrique Lehner,Angel Villar-Corrales,Jan Nogga,Sven Behnke*

Main category: cs.CV

TL;DR: VideoPCDNet是一种无监督框架，通过频域相位相关技术分解和预测视频中的对象，表现优于基线模型。
...
### [158] [HOIverse: A Synthetic Scene Graph Dataset With Human Object Interactions](https://arxiv.org/abs/2506.19639)
*Mrunmai Vivek Phatak,Julian Lorenz,Nico Hörmann,Jörg Hähner,Rainer Lienhart*
...
<details>
  <summary>Details</summary>
Motivation: 当前研究缺乏可靠的室内场景理解数据集，尤其是涉及人类的部分。HOIverse旨在填补这一空白，推动相关研究。

Method: 通过生成场景图的结构化表示，计算人-物对的参数化关系，并提供RGB图像、分割掩码、深度图像和人体关键点等数据。

Result: HOIverse数据集提供了精确且密集的关系标注，并用于基准测试现有的场景图生成模型。
...
### [159] [PEVLM: Parallel Encoding for Vision-Language Models](https://arxiv.org/abs/2506.19651)
*Letian Kang,Shixian Luo,Yiqiang Li,Xiaoyang Yu,Shenxuan Zhou,Yong Wu*
...
<details>
  <summary>Details</summary>
Motivation: 解决标准注意力机制在长视频理解中的二次复杂度问题，提升VLM的预填充效率。

Method: 通过分块并行编码、共享sink、保留全注意力位置嵌入和对齐注意力权重来模拟全注意力分布。

Result: 在LongVideoBench基准测试中，PEVLM实现了8.37%的准确率提升，7.47倍注意力计算加速和40%端到端延迟降低。
...
### [160] [Video Compression for Spatiotemporal Earth System Data](https://arxiv.org/abs/2506.19656)
*Oscar J. Pellicer-Valero,Cesar Aybar,Gustau Camps Valls*
...
Method: 通过ffmpeg利用标准视频编解码器将多通道时空数据集编码为视频，实现高效压缩。

Result: 在四个真实数据集上测试，压缩比高达250倍，PSNR表现优异，且不影响下游深度学习任务性能。

Conclusion: xarrayvideo为地球科学社区提供了一种高效、实用的数据集压缩解决方案。
...
### [161] [SAM2-SGP: Enhancing SAM2 for Medical Image Segmentation via Support-Set Guided Prompting](https://arxiv.org/abs/2506.19658)
*Yang Xing,Jiong Wu,Yuheng Bu,Kuang Gong*

Main category: cs.CV

TL;DR: SAM2-SGP框架通过支持集引导提示和低秩适应策略，解决了SAM2在医学图像分割中的依赖人工提示和领域偏移问题，显著提升了性能。
...
### [162] [Genome-Anchored Foundation Model Embeddings Improve Molecular Prediction from Histology Images](https://arxiv.org/abs/2506.19681)
*Cheng Jin,Fengtao Zhou,Yunfang Yu,Jiabo Ma,Yihui Wang,Yingxue Xu,Huajun Zhou,Hao Jiang,Luyang Luo,Luhui Mao,Zifan He,Xiuming Zhang,Jing Zhang,Ronald Chan,Herui Yao,Hao Chen*
...
<details>
  <summary>Details</summary>
Motivation: 精准肿瘤学需要准确的分子信息，但直接从基因组学获取成本高且耗时。PathLUPI旨在通过WSI预测复杂分子特征和患者预后，解决现有深度学习方法的局限性。

Method: PathLUPI在训练中使用转录组特权信息，生成基因组锚定的组织学嵌入，仅需WSI即可在推理阶段进行分子预测。
...
### [163] [Semantic Scene Graph for Ultrasound Image Explanation and Scanning Guidance](https://arxiv.org/abs/2506.19683)
*Xuesong Li,Dianye Huang,Yameng Zhang,Nassir Navab,Zhongliang Jiang*

Main category: cs.CV

TL;DR: 论文提出了一种基于场景图（SG）的方法，用于解释超声图像内容并为普通用户提供扫描指导，利用Transformer模型和LLMs提升超声的可解释性和可用性。
...
Method: 引入超声图像场景图（SG），通过Transformer模型生成SG，再结合LLMs细化解释，并利用SG指导扫描缺失的解剖结构。

Result: 在五名志愿者的颈部和甲状腺区域图像上验证了方法的有效性，提升了超声的可解释性和可用性。

Conclusion: 该方法有望通过增强超声的可解释性和指导性，使其更广泛地服务于普通用户。
...
### [164] [UltraAD: Fine-Grained Ultrasound Anomaly Classification via Few-Shot CLIP Adaptation](https://arxiv.org/abs/2506.19694)
*Yue Zhou,Yuan Bi,Wenjuan Tong,Wei Wang,Nassir Navab,Zhongliang Jiang*

Main category: cs.CV

TL;DR: 提出了一种基于视觉语言模型（VLM）的方法UltraAD，用于医学图像中的异常检测，解决了现有方法在细粒度分类和设备依赖性方面的不足。


<details>
  <summary>Details</summary>
Motivation: 医学图像中的精确异常检测对临床决策至关重要，但现有方法缺乏细粒度分类能力，且超声图像受设备和参数影响较大。

Method: 通过融合图像级和文本级特征，结合记忆库技术，实现异常定位和细粒度分类。

Result: 在三个乳腺超声数据集上表现优异，超越现有方法。
...
### [165] [Systematic Comparison of Projection Methods for Monocular 3D Human Pose Estimation on Fisheye Images](https://arxiv.org/abs/2506.19747)
*Stephanie Käs,Sven Peter,Henrik Thillmann,Anton Burenko,David Benjamin Adrian,Dennis Mack,Timm Linder,Bastian Leibe*

Main category: cs.CV

TL;DR: 评估鱼眼图像中3D人体姿态估计的不同投影方法，提出基于检测框选择最优模型的启发式方法，并发布新数据集FISHnCHIPS。


<details>
  <summary>Details</summary>
Motivation: 鱼眼相机在机器人交互和汽车应用中具有优势，但鱼眼图像的扭曲特性使人体姿态检测困难，现有方法未系统评估其在宽视场下的效果。

Method: 比较针孔、等距、双球面相机模型及圆柱投影方法对3D人体姿态估计的影响，提出基于检测框选择模型的启发式方法。

Result: 近景中针孔投影效果不佳，最优投影方法随视场变化；双球面模型显著提升精度。

Conclusion: 提出启发式方法选择投影模型，并发布包含极端角度和宽视场数据的新数据集FISHnCHIPS。
...
### [166] [CoCo4D: Comprehensive and Complex 4D Scene Generation](https://arxiv.org/abs/2506.19798)
*Junwei Zhou,Xueting Li,Lu Qi,Ming-Hsuan Yang*

Main category: cs.CV

TL;DR: CoCo4D是一个从文本提示生成动态4D场景的框架，支持多视角一致性和沉浸感，通过分离动态前景和背景建模实现高效合成。
...
Method: CoCo4D将4D场景合成分为动态前景建模和背景演化，利用参考运动序列指导生成，并通过渐进式外推方案优化前景轨迹。

Result: 实验表明，CoCo4D在4D场景生成中性能优于或与现有方法相当，实现了高效且逼真的合成效果。

Conclusion: CoCo4D通过分离前景和背景建模，成功解决了多视角一致性和沉浸感问题，为动态4D场景生成提供了有效方案。
...
### [167] [One Prototype Is Enough: Single-Prototype Activation for Interpretable Image Classification](https://arxiv.org/abs/2506.19808)
*Yitao Peng,Lianghua He,Die Hu*

Main category: cs.CV

TL;DR: ProtoSolo是一种新颖的深度神经网络架构，通过单原型激活实现可解释的图像分类，简化解释复杂度。


<details>
  <summary>Details</summary>
Motivation: 现有原型网络依赖多个原型协作决策，增加了认知复杂度。ProtoSolo旨在通过单原型激活简化分类和解释。

Method: 采用基于特征图的相似性比较方法，提出非原型投影学习策略，保留信息关联并避免结构突变。

Result: 在CUB-200-2011和Stanford Cars数据集上表现优异，分类任务和解释复杂度均达到最佳水平。
...
### [168] [Bind-Your-Avatar: Multi-Talking-Character Video Generation with Dynamic 3D-mask-based Embedding Router](https://arxiv.org/abs/2506.19833)
*Yubo Huang,Weiqiang Wang,Sirui Zhao,Tong Xu,Lin Liu,Enhong Chen*
...
Method: 提出基于MM-DiT的模型，包含细粒度Embedding Router、3D-mask嵌入路由器和首个多角色对话数据集。

Result: 实验表明，模型在双角色对话视频生成任务中优于现有方法。

Conclusion: Bind-Your-Avatar为多角色同场景对话视频生成提供了有效解决方案，并填补了数据集空白。
...
### [169] [SimpleGVR: A Simple Baseline for Latent-Cascaded Video Super-Resolution](https://arxiv.org/abs/2506.19838)
*Liangbin Xie,Yu Li,Shian Du,Menghan Xia,Xintao Wang,Fanghua Yu,Ziyan Chen,Pengfei Wan,Jiantao Zhou,Chao Dong*

Main category: cs.CV

TL;DR: 论文提出了一种两阶段视频生成方法，通过分离语义内容生成和细节合成，并重点研究了级联视频超分辨率（VSR）模型的设计原则。


<details>
  <summary>Details</summary>
Motivation: 随着用户对高分辨率视频生成的需求增加，仅依赖潜在计算已不足，需要更高效的方法。

Method: 采用两阶段方法：基础模型生成低分辨率语义内容，轻量级VSR模型合成高分辨率细节；提出了两种退化策略和VSR模型行为分析。

Result: 实验证明该方法优于现有技术，并通过消融研究验证了设计选择的有效性。

Conclusion: 研究为级联视频超分辨率生成提供了简单有效的基准，为未来高效合成系统的发展提供了实用指导。
...
### [170] [Improving Progressive Generation with Decomposable Flow Matching](https://arxiv.org/abs/2506.19839)
*Moayed Haji-Ali,Willi Menapace,Ivan Skorokhodov,Arpit Sahni,Sergey Tulyakov,Vicente Ordonez,Aliaksandr Siarohin*

Main category: cs.CV

TL;DR: DFM是一种简单有效的渐进生成视觉媒体的框架，通过独立应用Flow Matching在多尺度表示中，显著提升图像和视频质量，优于现有多阶段框架。


<details>
  <summary>Details</summary>
Motivation: 解决高维视觉模态生成的计算密集问题，避免现有多阶段架构的复杂性和定制需求。

Method: 提出Decomposable Flow Matching (DFM)，在用户定义的多尺度表示（如拉普拉斯金字塔）中独立应用Flow Matching。

Result: 在Imagenet-1k 512px上，FDD分数提升35.2%，优于基线26.4%；在大型模型（如FLUX）微调中收敛更快。

Conclusion: DFM以单一模型、架构简单性和最小化修改现有训练流程的优势，显著提升生成质量和效率。
...
### [171] [GenHSI: Controllable Generation of Human-Scene Interaction Videos](https://arxiv.org/abs/2506.19840)
*Zekun Li,Rui Zhou,Rahul Sajnani,Xiaoyan Cong,Daniel Ritchie,Srinath Sridhar*

Main category: cs.CV

TL;DR: GenHSI是一种无需训练的方法，用于生成长时间的人类-场景交互视频，解决了现有方法在身份保持和交互真实性上的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的大规模预训练视频扩散模型在生成长电影式视频时存在人类-场景交互不真实、身份保持不足和训练成本高的问题。

Method: GenHSI将长视频生成任务分为三个阶段：脚本编写、预可视化和动画，利用3D关键帧和现成的视频扩散模型生成一致的长视频。

Result: 实验表明，GenHSI能够从单张场景图像生成长视频，有效保持场景内容和角色身份，并提供合理的人类-场景交互。
...
### [172] [Active View Selector: Fast and Accurate Active View Selection with Cross Reference Image Quality Assessment](https://arxiv.org/abs/2506.19844)
*Zirui Wang,Yash Bhalgat,Ruining Li,Victor Adrian Prisacariu*

Main category: cs.CV

TL;DR: 论文提出了一种基于2D图像质量评估（IQA）的主动视图选择方法，用于新视图合成和3D重建，避免了复杂的3D建模，并在速度和性能上优于现有方法。
...
Method: 通过训练一个模型在多视图设置中预测SSIM，并利用该模型指导视图选择，避免了依赖真实图像或缺乏多视图上下文的传统方法。

Result: 该方法在标准基准测试中实现了显著的定量和定性改进，且运行速度比现有方法快14-33倍。

Conclusion: 提出的跨参考IQA框架不仅对3D表示无关，而且在效率和性能上均优于现有方法。
...
### [173] [A Comparative Study of NAFNet Baselines for Image Restoration](https://arxiv.org/abs/2506.19845)
*Vladislav Esaulov,M. Moein Esfahani*

Main category: cs.CV

TL;DR: NAFNet是一种用于图像修复的简单高效深度学习基线，通过CIFAR10图像验证其核心组件（SimpleGate激活、SCA和LayerNorm）的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究非线性激活自由网络（NAFNet）在图像修复中的性能，验证其核心组件的设计合理性。

Method: 使用CIFAR10图像加噪和模糊进行消融实验，比较不同组件（SimpleGate、SCA、LayerNorm）的变体对修复性能的影响。
...
### [174] [Unified Vision-Language-Action Model](https://arxiv.org/abs/2506.19850)
*Yuqi Wang,Xinghang Li,Wenxuan Wang,Junbo Zhang,Yingyan Li,Yuntao Chen,Xinlong Wang,Zhaoxiang Zhang*

Main category: cs.CV

TL;DR: UniVLA是一种统一的多模态视觉-语言-动作模型，通过自回归建模视觉、语言和动作信号，结合世界建模，显著提升了机器人操作任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有方法主要依赖视觉-语言模型的通用理解能力生成动作信号，忽视了视觉观察中的时间和因果结构。

Method: 提出UniVLA模型，将视觉、语言和动作信号建模为离散令牌序列，结合世界建模从视频中学习因果动态。

Result: 在CALVIN、LIBERO等基准测试中取得最优性能，如LIBERO上平均成功率95.5%，显著超越之前的方法。

Conclusion: UniVLA展示了在机器人操作和自动驾驶等任务中的广泛应用潜力。
...
### [175] [AnimaX: Animating the Inanimate in 3D with Joint Video-Pose Diffusion Models](https://arxiv.org/abs/2506.19851)
*Zehuan Huang,Haoran Feng,Yangtian Sun,Yuanchen Guo,Yanpei Cao,Lu Sheng*
...
<details>
  <summary>Details</summary>
Motivation: 传统动作合成方法受限于固定骨架结构或需要高维优化，AnimaX旨在将视频动作知识迁移到3D领域，支持任意骨架的动画生成。

Method: AnimaX将3D动作表示为多视角、多帧的2D姿态图，通过联合视频-姿态扩散模型生成动作，并利用共享位置编码和模态感知嵌入确保对齐。

Result: 在160,000个动画序列的数据集上训练后，AnimaX在VBench上实现了泛化性、动作保真度和效率的先进水平。
...
### [176] [Radial Attention: $O(n\log n)$ Sparse Attention with Energy Decay for Long Video Generation](https://arxiv.org/abs/2506.19852)
*Xingyang Li,Muyang Li,Tianle Cai,Haocheng Xi,Shuo Yang,Yujun Lin,Lvmin Zhang,Songlin Yang,Jinbo Hu,Kelly Peng,Maneesh Agrawala,Ion Stoica,Kurt Keutzer,Song Han*

Main category: cs.CV

TL;DR: 论文提出Radial Attention，一种高效的稀疏注意力机制，显著降低视频扩散模型的计算成本，同时保持生成质量。


<details>
  <summary>Details</summary>
Motivation: 发现视频扩散模型中存在时空能量衰减现象，提出利用这一现象设计更高效的注意力机制。

Method: 提出Radial Attention，通过静态注意力掩码实现稀疏注意力，计算复杂度为O(n log n)。

Result: 在多个模型上验证，Radial Attention保持视频质量，计算效率提升1.9倍，支持生成更长视频。

Conclusion: Radial Attention显著降低训练和推理成本，同时扩展视频生成能力，具有实际应用价值。
...
### [177] [MemeMind: A Large-Scale Multimodal Dataset with Chain-of-Thought Reasoning for Harmful Meme Detection](https://arxiv.org/abs/2506.18919)
...
### [178] [Mirage of Mastery: Memorization Tricks LLMs into Artificially Inflated Self-Knowledge](https://arxiv.org/abs/2506.18998)
...
### [179] [Broken Tokens? Your Language Model can Secretly Handle Non-Canonical Tokenizations](https://arxiv.org/abs/2506.19004)
...
### [180] [Quantifying Fairness in LLMs Beyond Tokens: A Semantic and Statistical Perspective](https://arxiv.org/abs/2506.19028)
...
### [181] [Plan for Speed -- Dilated Scheduling for Masked Diffusion Language Models](https://arxiv.org/abs/2506.19037)
...
### [182] [NLPnorth @ TalentCLEF 2025: Comparing Discriminative, Contrastive, and Prompt-Based Methods for Job Title and Skill Matching](https://arxiv.org/abs/2506.19058)
...
### [183] [MFTCXplain: A Multilingual Benchmark Dataset for Evaluating the Moral Reasoning of LLMs through Hate Speech Multi-hop Explanation](https://arxiv.org/abs/2506.19073)
...
### [184] [Language Models Might Not Understand You: Evaluating Theory of Mind via Story Prompting](https://arxiv.org/abs/2506.19089)
...
### [185] [Human-Aligned Faithfulness in Toxicity Explanations of LLMs](https://arxiv.org/abs/2506.19113)
...
### [186] [Enhanced Hybrid Transducer and Attention Encoder Decoder with Text Data](https://arxiv.org/abs/2506.19159)
...
### [187] [Prompt, Translate, Fine-Tune, Re-Initialize, or Instruction-Tune? Adapting LLMs for In-Context Learning in Low-Resource Languages](https://arxiv.org/abs/2506.19187)
...
### [188] [Augmenting Multi-Agent Communication with State Delta Trajectory](https://arxiv.org/abs/2506.19209)
...
### [189] [Personality Prediction from Life Stories using Language Models](https://arxiv.org/abs/2506.19258)
...
### [190] [What Matters in LLM-generated Data: Diversity and Its Effect on Model Fine-Tuning](https://arxiv.org/abs/2506.19262)
...
### [191] [EmoStage: A Framework for Accurate Empathetic Response Generation via Perspective-Taking and Phase Recognition](https://arxiv.org/abs/2506.19279)
...
### [192] [JCAPT: A Joint Modeling Approach for CAPT](https://arxiv.org/abs/2506.19315)
...
### [193] [Spotting Out-of-Character Behavior: Atomic-Level Evaluation of Persona Fidelity in Open-Ended Generation](https://arxiv.org/abs/2506.19352)
...
### [194] [Measuring and Guiding Monosemanticity](https://arxiv.org/abs/2506.19382)
...
### [195] [Automated Detection of Pre-training Text in Black-box LLMs](https://arxiv.org/abs/2506.19399)
...
### [196] [Learning to Disentangle Latent Reasoning Rules with Language VAEs: A Systematic Study](https://arxiv.org/abs/2506.19418)
...
### [197] [Can Large Language Models Capture Human Annotator Disagreements?](https://arxiv.org/abs/2506.19467)
...
### [198] [MuBench: Assessment of Multilingual Capabilities of Large Language Models Across 61 Languages](https://arxiv.org/abs/2506.19468)
...
### [199] [Commonsense Generation and Evaluation for Dialogue Systems using Large Language Models](https://arxiv.org/abs/2506.19483)
...
### [200] [Dialogic Pedagogy for Large Language Models: Aligning Conversational AI with Proven Theories of Learning](https://arxiv.org/abs/2506.19484)
*Russell Beale*
...
### [201] [Is Long-to-Short a Free Lunch? Investigating Inconsistency and Reasoning Efficiency in LRMs](https://arxiv.org/abs/2506.19492)
...
### [202] [AnTKV: Anchor Token-Aware Sub-Bit Vector Quantization for KV Cache in Large Language Models](https://arxiv.org/abs/2506.19505)
...
### [203] [heiDS at ArchEHR-QA 2025: From Fixed-k to Query-dependent-k for Retrieval Augmented Generation](https://arxiv.org/abs/2506.19512)
...
### [204] [Automatic Posology Structuration : What role for LLMs?](https://arxiv.org/abs/2506.19525)
...
### [205] [KnowMap: Efficient Knowledge-Driven Task Adaptation for LLMs](https://arxiv.org/abs/2506.19527)
...
### [206] [Health Sentinel: An AI Pipeline For Real-time Disease Outbreak Detection](https://arxiv.org/abs/2506.19548)
...
### [207] [RCStat: A Statistical Framework for using Relative Contextualization in Transformers](https://arxiv.org/abs/2506.19549)
...
### [208] [Has Machine Translation Evaluation Achieved Human Parity? The Human Reference and the Limits of Progress](https://arxiv.org/abs/2506.19571)
...
### [209] [ECCoT: A Framework for Enhancing Effective Cognition via Chain of Thought in Large Language Model](https://arxiv.org/abs/2506.19599)
...
### [210] [Social Hatred: Efficient Multimodal Detection of Hatemongers](https://arxiv.org/abs/2506.19603)
...
### [211] [Correcting Hallucinations in News Summaries: Exploration of Self-Correcting LLM Methods with External Knowledge](https://arxiv.org/abs/2506.19607)
...
### [212] [Tailored Conversations beyond LLMs: A RL-Based Dialogue Manager](https://arxiv.org/abs/2506.19652)
...
### [213] [Breaking Barriers: Do Reinforcement Post Training Gains Transfer To Unseen Domains?](https://arxiv.org/abs/2506.19733)
...
### [214] [Evaluating Rare Disease Diagnostic Performance in Symptom Checkers: A Synthetic Vignette Simulation Approach](https://arxiv.org/abs/2506.19750)
...
### [215] [Recycling the Web: A Method to Enhance Pre-training Data Quality and Quantity for Language Models](https://arxiv.org/abs/2506.04689)
...
### [216] [Arabic Dialect Classification using RNNs, Transformers, and Large Language Models: A Comparative Analysis](https://arxiv.org/abs/2506.19753)
...
### [217] [Accurate, fast, cheap: Choose three. Replacing Multi-Head-Attention with Bidirectional Recurrent Attention for Long-Form ASR](https://arxiv.org/abs/2506.19761)
...
### [218] [SRFT: A Single-Stage Method with Supervised and Reinforcement Fine-Tuning for Reasoning](https://arxiv.org/abs/2506.19767)
...
### [219] [Why Do Open-Source LLMs Struggle with Data Analysis? A Systematic Empirical Study](https://arxiv.org/abs/2506.19794)
...
### [220] [How Effectively Can BERT Models Interpret Context and Detect Bengali Communal Violent Text?](https://arxiv.org/abs/2506.19831)
...
### [221] [MAM: Modular Multi-Agent Framework for Multi-Modal Medical Diagnosis via Role-Specialized Collaboration](https://arxiv.org/abs/2506.19835)
...
### [222] [MemeMind: A Large-Scale Multimodal Dataset with Chain-of-Thought Reasoning for Harmful Meme Detection](https://arxiv.org/abs/2506.18919)
*Hexiang Gu,Qifan Yu,Saihui Hou,Zhiqin Fang,Huijia Wu,Zhaofeng He*
...
<details>
  <summary>Details</summary>
Motivation: 社交媒体的快速发展加剧了有害内容的传播，而有害表情包因其隐含语义和多模态复杂性难以检测。现有数据集缺乏系统性、多样性和可解释性，阻碍了研究进展。

Method: 提出MemeMind数据集，具备科学标准、大规模、多样性、双语支持和详细推理标注；并开发MemeGuard框架，整合多模态信息和推理建模。

Result: 实验表明，MemeGuard在有害表情包检测任务中显著优于现有方法。

Conclusion: MemeMind和MemeGuard填补了当前数据集的空白，为有害表情包检测提供了更可靠的基础和方法。
...
### [223] [Mirage of Mastery: Memorization Tricks LLMs into Artificially Inflated Self-Knowledge](https://arxiv.org/abs/2506.18998)
*Sahil Kale,Vijaykant Nadadur*

Main category: cs.CL

TL;DR: 研究发现LLMs倾向于通过记忆而非真正学习推理模式，导致在STEM领域任务中表现出45%以上的不一致性，尤其在科学和医学领域更为明显。


<details>
  <summary>Details</summary>
Motivation: 探讨LLMs是否真正学习推理模式，还是仅通过记忆来表现能力，以提升其可信度和可解释性。

Method: 使用新颖框架评估LLMs在STEM领域的推理能力，分析其记忆与自我认知的关系。

Result: LLMs因记忆解决方案而高估自身推理能力，导致可行性评估中超过45%的不一致性。

Conclusion: 当前LLMs架构和训练模式存在缺陷，需改进以确保模型对其知识的认知一致，提升可信度。
...
### [224] [Broken Tokens? Your Language Model can Secretly Handle Non-Canonical Tokenizations](https://arxiv.org/abs/2506.19004)
*Brian Siyuan Zheng,Alisa Liu,Orevaoghene Ahia,Jonathan Hayase,Yejin Choi,Noah A. Smith*

Main category: cs.CL

TL;DR: 研究发现，现代语言模型对非标准分词具有较强鲁棒性，某些情况下非标准分词还能提升性能。
...
Method: 通过20个基准测试评估指令调优模型在随机和字符级分词下的性能表现，并分析不同分词方案对任务的影响。

Result: 模型在非标准分词下保留高达93.4%的性能，字符级分词在某些任务中提升14%，数字对齐分词提升算术任务33%。

Conclusion: 模型对分词器的依赖性低于预期，非标准分词在推理时可作为性能提升的工具。
...
### [225] [Quantifying Fairness in LLMs Beyond Tokens: A Semantic and Statistical Perspective](https://arxiv.org/abs/2506.19028)
*Weijie Xu,Yiwen Wang,Chi Xue,Xiangkun Hu,Xi Fang,Guimin Dong,Chandan K. Reddy*

Main category: cs.CL

TL;DR: FiSCo是一种新颖的统计框架，用于评估大语言模型（LLMs）在长文本响应中的群体公平性，通过检测不同人口群体间的语义差异来识别潜在偏见。


<details>
  <summary>Details</summary>
Motivation: 现有评估方法常忽略长文本响应中的偏见和LLM输出的内在变异性，导致其在实际应用中的可靠性不足。

Method: FiSCo通过将模型输出分解为语义不同的主张，并利用蕴含检查比较群体间和群体内的相似性，结合统计假设检验来检测细微偏见。

Result: 实验表明，FiSCo能更可靠地识别细微偏见，同时减少LLM随机变异性影响，优于其他评估指标。

Conclusion: FiSCo提供了一种更精细的公平性评估方法，为LLM的偏见检测提供了新思路。
...
### [226] [Plan for Speed -- Dilated Scheduling for Masked Diffusion Language Models](https://arxiv.org/abs/2506.19037)
*Omer Luxembourg,Haim Permuter,Eliya Nachmani*

Main category: cs.CL

TL;DR: Dilated-scheduled Unmasking Strategy (DUS) 是一种无需额外训练的推理方法，通过并行解掩码提高非自回归文本生成的效率和质量。
...
Result: 在数学和代码补全任务中，DUS 显著提升了生成速度和分数。

Conclusion: DUS 是一种轻量级的高效文本生成方法，展现了 MDLMs 的潜力。
...
### [227] [NLPnorth @ TalentCLEF 2025: Comparing Discriminative, Contrastive, and Prompt-Based Methods for Job Title and Skill Matching](https://arxiv.org/abs/2506.19058)
*Mike Zhang,Rob van der Goot*

Main category: cs.CL

TL;DR: 论文分析了多语言职位匹配和基于职位名称的技能预测任务，比较了不同方法，发现提示方法在任务A表现最佳，而微调分类方法在任务B表现较好。


<details>
  <summary>Details</summary>
Motivation: 职位匹配和技能预测在计算性就业市场中具有重要意义，可提升自动候选人匹配、职业路径预测等任务。

Method: 比较了微调分类、微调对比和提示方法，并利用ESCO的额外数据。

Result: 任务A中提示方法表现最佳（MAP 0.492），任务B中微调分类方法表现较好（MAP 0.290）。
...
### [228] [MFTCXplain: A Multilingual Benchmark Dataset for Evaluating the Moral Reasoning of LLMs through Hate Speech Multi-hop Explanation](https://arxiv.org/abs/2506.19073)
*Jackson Trager,Francielle Vargas,Diego Alves,Matteo Guida,Mikel K. Ngueajio,Ameeta Agrawal,Flor Plaza-del-Arco,Yalda Daryanai,Farzan Karimi-Malekabadi*

Main category: cs.CL

TL;DR: MFTCXplain是一个多语言基准数据集，用于通过仇恨言论的多跳解释评估大型语言模型（LLM）的道德推理能力，揭示了LLM在道德推理任务中的局限性。
...
Method: 引入MFTCXplain数据集，包含3000条推文，标注了仇恨言论标签、道德类别和文本级理由。
...
### [229] [Language Models Might Not Understand You: Evaluating Theory of Mind via Story Prompting](https://arxiv.org/abs/2506.19089)
*Nathaniel Getachew,Abulhair Saparov*

Main category: cs.CL

TL;DR: 介绍了$	exttt{StorySim}$框架，用于生成合成故事以评估大型语言模型（LLMs）的心理理论（ToM）和世界建模（WM）能力。


<details>
  <summary>Details</summary>
Motivation: 现有基准可能存在预训练数据污染问题，$	exttt{StorySim}$能生成新颖、可组合的故事提示，并通过可控的$	exttt{Storyboard}$精确操纵角色视角和事件。

Method: 设计了一阶和二阶ToM任务及WM任务，控制心理状态追踪和建模能力，并在多个先进LLMs上进行实验。

Result: 大多数模型在WM任务上表现优于ToM任务，且对人类推理优于对无生命物体；发现启发式行为（如近因偏差和过度依赖早期事件）。

Conclusion: $	exttt{StorySim}$为评估LLMs的ToM和WM能力提供了有效工具，并揭示了模型的行为模式。
...
### [230] [Human-Aligned Faithfulness in Toxicity Explanations of LLMs](https://arxiv.org/abs/2506.19113)
*Ramaravind K. Mothilal,Joanna Roy,Syed Ishtiaque Ahmed,Shion Guha*

Main category: cs.CL

TL;DR: 该论文提出了一种名为HAF的新标准，用于评估LLMs在毒性解释中的可信度，并通过实验发现LLMs在复杂提示下的解释能力存在不足。


<details>
  <summary>Details</summary>
Motivation: 现有方法难以评估LLMs的自由形式毒性解释，因此需要一种新标准来提升其在下游任务中的可信度。

Method: 提出HAF标准，基于不确定性量化开发六个指标，无需人工参与即可全面评估LLMs的解释能力。

Result: 实验表明，LLMs在简单提示下能生成合理解释，但在复杂提示下解释能力下降，产生不一致或无意义的回答。

Conclusion: HAF标准能有效评估LLMs的毒性解释能力，揭示了其在复杂场景中的局限性。
...
### [231] [Enhanced Hybrid Transducer and Attention Encoder Decoder with Text Data](https://arxiv.org/abs/2506.19159)
*Yun Tang,Eesung Kim,Vijendra Raj Apsingekar*

Main category: cs.CL

TL;DR: 提出了一种联合语音和文本优化的方法（J-TAED），通过结合语音和文本输入模态提升ASR准确性，并在推理时仅需语音输入。
...
Method: 联合训练语音和文本输入模态，统一内部表示，并支持基于文本的领域适应。

Result: 在Librispeech数据集上WER降低5.8~12.8%，在两个领域外数据集上分别降低15.3%和17.8%。

Conclusion: J-TAED成功整合语音和语言信息，显著提升ASR性能，尤其在领域适应任务中表现优异。
...
### [232] [Prompt, Translate, Fine-Tune, Re-Initialize, or Instruction-Tune? Adapting LLMs for In-Context Learning in Low-Resource Languages](https://arxiv.org/abs/2506.19187)
*Christopher Toukmaji,Jeffrey Flanigan*

Main category: cs.CL

TL;DR: 研究发现，在低资源语言中，few-shot提示和翻译测试方法优于基于梯度的适应方法，并提出新指标VOR分析模型输出。


<details>
  <summary>Details</summary>
Motivation: 探索如何在高资源语言训练的LLMs中，优化低资源语言的上下文学习表现。

Method: 研究五种目标语言、三种基础LLMs和七种下游任务，比较多种适应技术（如few-shot提示、翻译测试、微调等）。

Result: few-shot提示和翻译测试表现最佳，梯度方法因灾难性遗忘表现较差。

Conclusion: 这是低资源语言上下文学习领域最大规模的研究，公开了数据集和模型。
...
### [233] [Augmenting Multi-Agent Communication with State Delta Trajectory](https://arxiv.org/abs/2506.19209)
*Yichen Tang,Weihang Su,Yujia Zhou,Yiqun Liu,Min Zhang,Shaoping Ma,Qingyao Ai*

Main category: cs.CL

TL;DR: 论文提出了一种新的通信协议（SDE），通过传输自然语言标记和状态转移轨迹来减少多智能体系统中信息损失，提升复杂推理任务的性能。
...
Method: 提出State Delta Encoding（SDE）方法，传输标记生成过程中的状态转移轨迹，而非实际状态值。
...
### [234] [Personality Prediction from Life Stories using Language Models](https://arxiv.org/abs/2506.19258)
*Rasiq Hussain,Jerry Ma,Rithik Khandelwal,Joshua Oltmanns,Mehak Gupta*

Main category: cs.CL

TL;DR: 该研究提出了一种结合预训练语言模型和RNN注意力机制的两步方法，用于从长文本中预测五因素模型人格特质，提高了准确性、效率和可解释性。
...
Method: 采用两步法：1）滑动窗口微调预训练语言模型提取上下文嵌入；2）应用带注意力机制的RNN整合长距离依赖。

Result: 通过消融实验和与LLaMA、Longformer等模型的对比，证明了该方法在预测准确性、效率和可解释性上的提升。

Conclusion: 结合语言特征与长文本建模，有望推动基于生活叙事的人格评估发展。
...
### [235] [What Matters in LLM-generated Data: Diversity and Its Effect on Model Fine-Tuning](https://arxiv.org/abs/2506.19262)
*Yuchang Zhu,Zhonghua zhen,Qunshu Lin,Haotong Wei,Xiaolong Sun,Zixuan Yu,Minghao Liu,Zibin Zheng,Liang Chen*
...
<details>
  <summary>Details</summary>
Motivation: 解决LLM生成数据在迭代训练中导致模型崩溃的问题，并强调数据多样性的重要性。

Method: 通过实验分析不同多样性水平的LLM生成数据对下游模型性能的影响，并研究混合不同比例合成数据的效果。

Result: 适度多样性的LLM生成数据在分布变化较小时能提升模型性能，而高多样性数据则产生负面影响。
...
### [236] [EmoStage: A Framework for Accurate Empathetic Response Generation via Perspective-Taking and Phase Recognition](https://arxiv.org/abs/2506.19279)
*Zhiyang Qi,Keiko Takamizo,Mariko Ukiyo,Michimasa Inaba*

Main category: cs.CL

TL;DR: EmoStage是一个基于开源LLM的框架，通过视角推断和阶段识别提升心理咨询中的共情回应质量，无需额外训练数据。


<details>
  <summary>Details</summary>
Motivation: 心理健康需求增加，现有AI心理咨询系统存在对用户心理状态理解不足、依赖高质量数据及隐私问题。

Method: EmoStage利用开源LLM的推理能力，引入视角推断和阶段识别技术，生成情感共鸣的回应。

Result: 实验表明，EmoStage在日语和中文心理咨询场景中提升了基础模型的回应质量，与数据驱动方法表现相当。

Conclusion: EmoStage为解决AI心理咨询中的挑战提供了有效方案，尤其在共情和阶段适应性方面表现突出。
...
### [237] [JCAPT: A Joint Modeling Approach for CAPT](https://arxiv.org/abs/2506.19315)
*Tzu-Hsuan Yang,Yue-Yang He,Berlin Chen*
...
<details>
  <summary>Details</summary>
Motivation: 发音反馈在第二语言学习中至关重要，APA和MDD的联合建模可以相互促进，但目前缺乏结合语音特征、SSM建模和提示策略的研究。

Method: 利用Mamba（选择性状态空间模型），整合语音特征和提示策略，增强APA和MDD的可解释性和时间推理能力。

Result: 在speechocean762基准测试中，模型表现优于现有方法，尤其在MDD任务上。

Conclusion: 该研究首次将语音特征、SSM建模和提示策略结合，为CAPT系统提供了更高效的解决方案。
...
### [238] [Spotting Out-of-Character Behavior: Atomic-Level Evaluation of Persona Fidelity in Open-Ended Generation](https://arxiv.org/abs/2506.19352)
*Jisu Shin,Juhyun Oh,Eunsu Kim,Hoyun Song,Alice Oh*

Main category: cs.CL

TL;DR: 提出了一种细粒度的评估框架，用于量化大型语言模型（LLMs）中角色一致性的细微偏差，解决了现有方法难以捕捉长文本生成中角色不一致的问题。


<details>
  <summary>Details</summary>
Motivation: 确保LLMs的角色一致性对于连贯的人机交互至关重要，但现有评估方法难以捕捉细微的角色偏差。

Method: 提出了一个原子级评估框架，包含三个关键指标，用于测量角色一致性和生成间的连贯性。

Result: 实验表明，该框架能有效检测现有方法忽略的角色不一致问题，并揭示了任务结构和角色吸引力对模型适应性的影响。

Conclusion: 该框架为角色一致性的评估提供了更精确和实用的方法，揭示了维持角色一致性的挑战。
...
### [239] [Measuring and Guiding Monosemanticity](https://arxiv.org/abs/2506.19382)
*Ruben Härle,Felix Friedrich,Manuel Brack,Stephan Wäldchen,Björn Deiseroth,Patrick Schramowski,Kristian Kersting*
...
<details>
  <summary>Details</summary>
Motivation: 当前方法在定位和操作特征表示方面存在局限性，稀疏自编码器（SAEs）也未能完全解决这些问题。

Method: 引入特征单义性评分（FMS）量化特征单义性，并提出引导稀疏自编码器（G-SAE），在训练中通过标记概念调节潜在表示。

Result: G-SAE在毒性检测、写作风格识别和隐私属性识别等任务中表现出更高的单义性和可控性。
...
### [240] [Automated Detection of Pre-training Text in Black-box LLMs](https://arxiv.org/abs/2506.19399)
*Ruihan Hu,Yu-Ming Shang,Jiankun Peng,Wei Luo,Yazhe Wang,Xi Zhang*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖模型隐藏信息或大量人工干预，无法在黑盒设置下有效检测预训练文本，VeilProbe旨在解决这一问题。

Method: 利用序列到序列映射模型推断输入文本与输出后缀的潜在映射特征，并进行关键令牌扰动以增强区分性；引入原型分类器缓解过拟合。

Result: 在三个广泛使用的数据集上验证了VeilProbe在黑盒设置下的有效性和优越性。

Conclusion: VeilProbe为黑盒环境下的预训练文本检测提供了高效且自动化的解决方案。
...
### [241] [Learning to Disentangle Latent Reasoning Rules with Language VAEs: A Systematic Study](https://arxiv.org/abs/2506.19418)
*Yingji Zhang,Marco Valentino,Danilo S. Carvalho,André Freitas*

Main category: cs.CL

TL;DR: 论文提出了一种在语言模型的潜在空间中嵌入显式推理规则的方法，通过语言变分自编码器（VAEs）增强模型的泛化性、可解释性和可控性。


<details>
  <summary>Details</summary>
Motivation: 当前基于Transformer的语言模型在自然语言推理（NLI）任务中表现优异，但主要依赖记忆而非规则推理。研究旨在探索如何显式嵌入和记忆推理规则。

Method: 提出了一套完整的流程，包括三个基于规则的推理任务、理论框架和端到端架构，利用语言VAEs在Transformer模型中学习推理规则。

Result: 实验表明，推理规则可以在编码器的参数空间中解耦，并在输出特征空间中形成聚类；注入先验知识能提升模型性能；数学推理任务中样本量增加存在性能瓶颈。

Conclusion: 该方法为在语言模型中嵌入显式推理规则提供了可行方案，并揭示了模型在规则推理中的性能限制。
...
### [242] [Can Large Language Models Capture Human Annotator Disagreements?](https://arxiv.org/abs/2506.19467)
*Jingwei Ni,Yu Fan,Vilém Zouhar,Donya Rooein,Alexander Hoyle,Mrinmaya Sachan,Markus Leippold,Dirk Hovy,Elliott Ash*

Main category: cs.CL

TL;DR: LLMs在预测人类标注分歧方面表现不佳，且基于多数标签的评估可能掩盖这一问题。
...
Method: 评估LLMs在无重复标注数据下预测标注分歧的能力，并分析RLVR推理的影响。

Result: LLMs难以建模分歧，且RLVR推理会降低分歧预测性能。
...
### [243] [MuBench: Assessment of Multilingual Capabilities of Large Language Models Across 61 Languages](https://arxiv.org/abs/2506.19468)
*Wenhan Han,Yifan Zhang,Zhixun Chen,Binbin Liu,Haobin Lin,Bingni Zhang,Taifeng Wang,Mykola Pechenizkiy,Meng Fang,Yin Zheng*

Main category: cs.CL

TL;DR: MuBench是一个覆盖61种语言的基准测试，用于评估多语言大语言模型（LLMs）的能力，发现其实际语言覆盖与声称存在差距，并提出多语言一致性（MLC）作为补充指标。


<details>
  <summary>Details</summary>
Motivation: 现有评估数据集在多语言能力和跨语言对齐方面存在不足，导致对多语言LLMs的评估碎片化。

Method: 引入MuBench基准测试，评估多语言LLMs，并提出MLC指标。此外，预训练了1.2B参数的模型，研究跨语言迁移动态。

Result: 发现多语言LLMs在英语和低资源语言之间存在性能差距，MLC能有效分析性能瓶颈。
...
### [244] [Commonsense Generation and Evaluation for Dialogue Systems using Large Language Models](https://arxiv.org/abs/2506.19483)
*Marcos Estecha-Garitagoitia,Chen Zhang,Mario Rodríguez-Cantelar,Luis Fernando D'Haro*

Main category: cs.CL

TL;DR: 论文探索了基于常识关系的对话系统轮次数据增强任务，并自动评估生成的合成轮次。利用预训练大语言模型（LLM）的扩展知识和零样本能力，结合Chain-of-Thought（CoT）方法，生成基于常识属性的对话增强数据，并提出自动评估框架。


<details>
  <summary>Details</summary>
Motivation: 研究旨在利用LLM的常识推理能力，为对话系统生成上下文相关的增强数据，并自动评估其质量，以提升对话系统的性能。

Method: 从5个知名对话数据集中随机提取200个部分对话，基于不同常识属性生成替代响应；提出基于指令的提示方法，利用LLM自动评估生成数据的质量。

Result: 初步结果表明，该方法能有效利用LLM的常识推理能力生成高质量的对话增强数据，并通过自动评估框架验证其有效性。

Conclusion: 该方法为对话系统的数据增强和自动评估提供了新思路，展示了LLM在常识推理任务中的潜力。
...
### [245] [Dialogic Pedagogy for Large Language Models: Aligning Conversational AI with Proven Theories of Learning](https://arxiv.org/abs/2506.19484)
*Russell Beale*

Main category: cs.CL

TL;DR: 本文综述了大型语言模型（LLM）在高等教育中的应用，探讨了如何通过提示策略和检索增强生成（RAG）将LLM行为与教育理论（如维果茨基的社会文化学习、苏格拉底方法和Laurillard的对话框架）对齐，并提出改进策略以优化教育效果。


<details>
  <summary>Details</summary>
Motivation: 研究动机在于探索LLM如何通过对话式学习改变教育，尤其是在高等教育中，同时解决LLM在直接提供答案而非促进知识共建方面的不足。

Method: 方法包括文献综述、教育理论与LLM能力的映射分析，以及提出改进策略（如设计提示以鼓励苏格拉底式提问和反思）。

Result: 研究结果揭示了LLM在教育中的潜力与局限，例如其在支持个性化学习方面的优势，但也存在与传统教育理论不符的挑战。

Conclusion: 结论是LLM可以通过优化提示和检索机制更好地与教育理论对齐，从而提升对话式学习的教育效果。
...
### [246] [Is Long-to-Short a Free Lunch? Investigating Inconsistency and Reasoning Efficiency in LRMs](https://arxiv.org/abs/2506.19492)
*Shu Yang,Junchao Wu,Xuansheng Wu,Derek Wong,Ninhao Liu,Di Wang*

Main category: cs.CL

TL;DR: 研究发现，大型推理模型（LRMs）在高效推理策略下可能引发行为不一致，如自相矛盾、事后合理化等，需进一步研究其潜在风险。
...
### [247] [AnTKV: Anchor Token-Aware Sub-Bit Vector Quantization for KV Cache in Large Language Models](https://arxiv.org/abs/2506.19505)
*Zeyu Li,Chuanfu Xiao,Yang Wang,Xiang Liu,Zhenheng Tang,Baotong Lu,Mao Yang,Xinyu Chen,Xiaowen Chu*

Main category: cs.CL

TL;DR: 论文提出AnTKV框架，通过量化KV缓存中的关键令牌（Anchor Token）来减少内存占用，同时保持高性能。
...
Method: 提出Anchor Score（AnS）量化令牌对量化误差的敏感性，并设计AnTKV框架结合向量量化和高效部署。

Result: AnTKV在LLaMA-3-8B上支持840K上下文长度，解码吞吐量提升3.5倍，并在超低位量化下表现优异。

Conclusion: AnTKV通过选择性保留高AnS令牌的精度，显著提升了超低位量化下的性能，优于现有方法。
...
### [248] [heiDS at ArchEHR-QA 2025: From Fixed-k to Query-dependent-k for Retrieval Augmented Generation](https://arxiv.org/abs/2506.19512)
*Ashish Chouhan,Michael Gertz*
...
<details>
  <summary>Details</summary>
Motivation: 解决电子健康记录（EHRs）中患者特定问题的回答生成，强调证据的可追溯性和准确性。

Method: 采用检索增强生成（RAG）框架，提出动态检索策略（query-dependent-k），包括新方法autocut*和elbow。

Result: 实验表明，动态检索策略优于固定k值方法，生成更事实性和相关的答案。

Conclusion: 动态检索策略在临床问答任务中表现更优，为RAG框架提供了新的改进方向。
...
TL;DR: 论文探讨了使用大型语言模型（LLMs）将自由文本药物剂量说明转换为结构化格式的方法，比较了基于提示的方法和微调方法，并提出了结合NERL和LLM的混合策略，显著提高了结构化准确性。


<details>
  <summary>Details</summary>
Motivation: 法国处方中的药物剂量说明通常模糊、不规则或口语化，限制了传统机器学习管道的效果，因此需要更有效的方法来自动化结构化处理。

Method: 研究比较了基于提示的LLM方法、微调LLM方法和传统的NERL方法，并通过错误分析提出了结合两者的混合策略。

Result: 混合策略（NERL低置信度时使用LLM）实现了91%的结构化准确性，同时降低了计算成本和延迟。

Conclusion: 结合NERL和LLM的混合方法在提高结构化准确性的同时减少了计算开销，为实际临床应用提供了可扩展的解决方案。
...
### [250] [KnowMap: Efficient Knowledge-Driven Task Adaptation for LLMs](https://arxiv.org/abs/2506.19527)
*Kelin Fu,Kaigui Bian*
...
<details>
  <summary>Details</summary>
Motivation: LLM在适应新任务时依赖静态预训练知识，传统微调方法成本高且可能导致灾难性遗忘。

Method: 动态构建知识库，用小模型微调知识嵌入以增强大模型的任务特定知识。

Result: 在ScienceWorld基准测试中，gpt-4-turbo性能提升17.71%。

Conclusion: KnowMap高效且有效，整合环境与经验知识可增强LLM推理能力。
...
### [251] [Health Sentinel: An AI Pipeline For Real-time Disease Outbreak Detection](https://arxiv.org/abs/2506.19548)
*Devesh Pant,Rishi Raj Grandhe,Vipin Samaria,Mukul Paul,Sudhir Kumar,Saransh Khanna,Jatin Agrawal,Jushaan Singh Kalra,Akhil VSSG,Satish V Khalikar,Vipin Garg,Himanshu Chauhan,Pranay Verma,Neha Khandelwal,Soma S Dhavala,Minesh Mathew*

Main category: cs.CL

TL;DR: Health Sentinel是一个多阶段信息提取管道，结合机器学习和非机器学习方法，从在线文章中提取疾病暴发或异常健康事件的结构化信息，以支持早期检测和干预。


<details>
  <summary>Details</summary>
Motivation: 传统基于指标的监测方法存在挑战，而在线媒体作为非正式信息来源日益重要，但手动筛选不切实际。

Method: 使用多阶段信息提取管道，结合ML和非ML方法，从在线文章中提取结构化健康事件信息。

Result: 自2022年4月以来，处理了超过3亿篇新闻文章，识别了95,000多个独特健康事件，其中3,500多个被专家列为潜在暴发。
...
### [252] [RCStat: A Statistical Framework for using Relative Contextualization in Transformers](https://arxiv.org/abs/2506.19549)
*Debabrata Mahapatra,Shubham Agarwal,Apoorv Saxena,Subrata Mitra*

Main category: cs.CL

TL;DR: RCStat利用原始注意力logits提出统计框架，通过相对上下文化（RC）实现高效压缩和更高保真度的归因分析。
...
Conclusion: RCStat在压缩和归因任务中表现优异，为注意力机制提供了更高效的统计方法。
...
### [253] [Has Machine Translation Evaluation Achieved Human Parity? The Human Reference and the Limits of Progress](https://arxiv.org/abs/2506.19571)
*Lorenzo Proietti,Stefano Perrella,Roberto Navigli*

Main category: cs.CL

TL;DR: 论文探讨了机器翻译（MT）评估中自动指标与人类判断的一致性，发现先进指标常与人类基线表现相当甚至更优，但需谨慎看待这一结果。


<details>
  <summary>Details</summary>
Motivation: 研究动机是明确自动指标的性能上限，并探讨人类基线在MT元评估中的作用。

Method: 方法是通过引入人类基线来评估MT指标的能力，比较自动指标与人类判断的一致性。

Result: 结果显示先进指标常与人类基线表现相当或更优，但需注意潜在问题。

Conclusion: 结论强调需谨慎看待指标性能，并探讨了MT评估领域进步测量的可靠性问题。
...
### [254] [ECCoT: A Framework for Enhancing Effective Cognition via Chain of Thought in Large Language Model](https://arxiv.org/abs/2506.19599)
*Zhenke Duan,Jiqun Pan,Jiani Tu,Xiaoyi Wang,Yanqing Wang*
...
### [255] [Social Hatred: Efficient Multimodal Detection of Hatemongers](https://arxiv.org/abs/2506.19603)
*Tom Marzea,Abraham Israeli,Oren Tsur*
...
<details>
  <summary>Details</summary>
Motivation: 在线仇恨言论的自动检测对净化网络环境至关重要，而用户层面的分析能更深入理解仇恨传播的社会现象。

Method: 采用多模态聚合方法，整合用户文本、活动及社交网络数据，评估了Twitter、Gab和Parler三个数据集。

Result: 在多种实验设置下，该方法显著优于传统文本和图基方法，并能有效分类隐晦信息和指导干预措施。

Conclusion: 多模态方法在不同平台和大规模数据集上表现优异，为仇恨言论检测提供了更全面的解决方案。
...
### [256] [Correcting Hallucinations in News Summaries: Exploration of Self-Correcting LLM Methods with External Knowledge](https://arxiv.org/abs/2506.19607)
*Juraj Vladika,Ihsan Soydemir,Florian Matthes*

Main category: cs.CL

TL;DR: 论文探讨了大型语言模型（LLMs）在生成文本时的幻觉问题，并研究了两种自校正方法在新闻摘要领域的应用，分析了其性能。


<details>
  <summary>Details</summary>
Motivation: 解决LLMs生成文本时的幻觉问题，尤其是自校正方法在新闻摘要领域的应用不足。

Method: 应用两种自校正系统，利用三种搜索引擎的证据校正幻觉摘要，并分析结果。

Result: 揭示了搜索引擎片段和少样本提示的益处，以及G-Eval与人工评估的高度一致性。

Conclusion: 自校正方法在新闻摘要领域具有潜力，搜索引擎和少样本提示能有效提升性能。
...
### [257] [Tailored Conversations beyond LLMs: A RL-Based Dialogue Manager](https://arxiv.org/abs/2506.19652)
*Lucie Galland,Catherine Pelachaud,Florian Pecune*

Main category: cs.CL

TL;DR: 提出了一种结合大语言模型（LLM）和基于强化学习的对话管理器的框架，用于目标导向的开放对话。


<details>
  <summary>Details</summary>
Motivation: 旨在通过结合LLM和强化学习，提升对话系统的适应性和效率，特别是在个性化响应和有限数据学习方面。

Method: 采用分层强化学习建模对话阶段，并结合元学习增强对不同用户特征的适应性。

Result: 在动机性访谈任务中，该框架在奖励指标上优于现有LLM基线。

Conclusion: 结果表明，通过条件化LLM构建目标导向的开放对话系统具有潜在优势。
...
### [258] [Breaking Barriers: Do Reinforcement Post Training Gains Transfer To Unseen Domains?](https://arxiv.org/abs/2506.19733)
*Chuxuan Hu,Yuxuan Zhu,Antony Kellermann,Caleb Biddulph,Suppakit Waiwitlikhit,Jason Benn,Daniel Kang*

Main category: cs.CL

TL;DR: RPT能提升LLMs的推理能力，但对新领域的泛化能力有限，尤其在未见过的领域表现不一致。


<details>
  <summary>Details</summary>
Motivation: 探讨RPT在不同领域的泛化能力，尤其是未见过的领域。

Method: 通过观察性和干预性研究，评估RPT模型在多个领域（包括未见过的领域）的表现。

Result: RPT在类似训练数据的任务上表现显著提升，但在不同推理模式的领域泛化能力不一致。

Conclusion: RPT的改进效果受限于训练数据的领域，泛化能力有限。
...
### [259] [Evaluating Rare Disease Diagnostic Performance in Symptom Checkers: A Synthetic Vignette Simulation Approach](https://arxiv.org/abs/2506.19750)
*Takashi Nishibayashi,Seiji Kanazawa,Kumpei Yamada*

Main category: cs.CL

TL;DR: 提出了一种基于合成病例模拟的方法，利用HPO数据库评估症状检查器算法更新对罕见疾病诊断性能的影响，验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 解决罕见疾病评估数据不足和人工创建病例成本高的问题，为算法更新提供预部署评估方法。

Method: 利用HPO数据库生成合成病例，模拟症状检查器访谈，通过R²系数验证预测效果。

Result: 对于有频率信息的疾病，R²值显示预测效果良好（召回率0.831，精确率0.78）；无频率信息时预测误差大。

Conclusion: 该方法透明、低成本，可帮助开发者高效优化罕见疾病诊断性能，支持早期诊断。
...
### [260] [Recycling the Web: A Method to Enhance Pre-training Data Quality and Quantity for Language Models](https://arxiv.org/abs/2506.04689)
*Thao Nguyen,Yang Li,Olga Golovneva,Luke Zettlemoyer,Sewoong Oh,Ludwig Schmidt,Xian Li*
...
Method: 提出REWIRE方法，通过改写被过滤的低质量文档，使其可用于训练。
...
### [261] [Arabic Dialect Classification using RNNs, Transformers, and Large Language Models: A Comparative Analysis](https://arxiv.org/abs/2506.19753)
*Omar A. Essameldin,Ali O. Elbeih,Wael H. Gomaa,Wael F. Elsersy*

Main category: cs.CL

TL;DR: 论文研究了18种阿拉伯方言的分类问题，使用了RNN、Transformer和大型语言模型（LLM），其中MARBERTv2表现最佳（65%准确率，64% F1分数）。


<details>
  <summary>Details</summary>
Motivation: 阿拉伯语是世界上使用最广泛的语言之一，但方言多样性大，研究旨在解决阿拉伯方言分类问题。

Method: 采用了RNN、Transformer模型和LLM（通过提示工程），并应用了最新的预处理技术。

Result: MARBERTv2表现最佳，准确率为65%，F1分数为64%。

Conclusion: 研究结果支持个性化聊天机器人、社交媒体监控和阿拉伯社区的可访问性应用。
...
### [262] [Accurate, fast, cheap: Choose three. Replacing Multi-Head-Attention with Bidirectional Recurrent Attention for Long-Form ASR](https://arxiv.org/abs/2506.19761)
*Martin Ratajczak,Jean-Philippe Robichaud,Jennifer Drexler Fox*

Main category: cs.CL

TL;DR: 论文提出了一种线性复杂度的循环注意力（RA）层，用于解决长语音识别中多头注意力（MHA）的二次复杂度问题，并通过新训练方法和正则化技术提升了性能。
...
Method: 采用双向循环注意力（RA）层，提出有限上下文注意力（LCA）基线，开发长形式训练范式及方向丢弃（Direction Dropout）正则化方法。

Result: RA层在短长语音识别中均达到MHA的准确度，且效率更高；长形式训练使RA比LCA准确度更高，吞吐量提升44%。

Conclusion: RA层是长语音识别的高效替代方案，方向丢弃正则化进一步优化了性能与吞吐量的权衡。
...
### [263] [SRFT: A Single-Stage Method with Supervised and Reinforcement Fine-Tuning for Reasoning](https://arxiv.org/abs/2506.19767)
*Yuqian Fu,Tinghong Chen,Jiajun Chai,Xihuai Wang,Songjun Tu,Guojun Yin,Wei Lin,Qichao Zhang,Yuanheng Zhu,Dongbin Zhao*
...
Method: 通过分析标记分布和学习动态，提出SRFT方法，统一SFT和RL，直接优化LLM。

Result: SRFT在五个数学推理基准测试中平均准确率达59.1%，优于零RL方法9.0%。

Conclusion: SRFT通过熵感知机制有效整合了SFT和RL，显著提升了模型性能。
...
### [264] [Why Do Open-Source LLMs Struggle with Data Analysis? A Systematic Empirical Study](https://arxiv.org/abs/2506.19794)
*Yuqi Zhu,Yi Zhong,Jintian Zhang,Ziheng Zhang,Shuofei Qiao,Yujie Luo,Lun Du,Da Zheng,Huajun Chen,Ningyu Zhang*

Main category: cs.CL

TL;DR: 研究探讨了如何提升开源大语言模型（LLMs）在数据分析任务中的能力，发现战略规划质量是关键，交互设计和数据质量也至关重要。


<details>
  <summary>Details</summary>
Motivation: 开源LLMs在数据分析任务中存在局限性，需要提升其推理能力。

Method: 通过构建多样化的真实场景数据集，评估模型在数据理解、代码生成和战略规划三个维度的表现。

Result: 发现战略规划质量是性能的关键，交互设计和数据质量对推理能力有显著影响。
...
### [265] [How Effectively Can BERT Models Interpret Context and Detect Bengali Communal Violent Text?](https://arxiv.org/abs/2506.19831)
*Abdullah Khondoker,Enam Ahmed Taufik,Md. Iftekhar Islam Tashik,S M Ishtiak Mahmud,Farig Sadeque*

Main category: cs.CL

TL;DR: 该研究通过微调BanglaBERT模型和集成模型，提高了孟加拉语社交媒体文本中煽动社区暴力内容的检测准确性，F1分数分别达到0.60和0.63，并分析了模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 网络仇恨言论的传播导致社区暴力，威胁社会和谐，但现有研究对社区暴力文本分类的探索不足。

Method: 研究采用微调的BanglaBERT模型和集成模型，扩展数据集以解决数据不平衡问题，并使用LIME工具解释模型决策。

Result: 微调模型和集成模型的F1分数分别为0.60和0.63，但模型在上下文理解和区分相关词汇方面存在局限性。
...
### [266] [MAM: Modular Multi-Agent Framework for Multi-Modal Medical Diagnosis via Role-Specialized Collaboration](https://arxiv.org/abs/2506.19835)
*Yucheng Zhou,Lingran Song,Jianbing Shen*

Main category: cs.CL

TL;DR: 论文提出了一种模块化多智能体框架（MAM），用于多模态医疗诊断，解决了现有医疗大语言模型在知识更新、全面性和灵活性上的不足。


<details>
  <summary>Details</summary>
Motivation: 当前统一的多模态医疗大语言模型在知识更新成本、全面性和灵活性方面存在局限，亟需改进。

Method: MAM将医疗诊断过程分解为多个专门角色（如全科医生、专科团队等），每个角色由基于LLM的智能体承担，实现模块化和协作。

Result: 在多种公开多模态医疗数据集上的实验表明，MAM性能显著优于基线模型，提升幅度达18%至365%。

Conclusion: MAM通过模块化和协作设计，有效提升了医疗诊断的效率和性能，为未来医疗AI发展提供了新思路。
...
### [267] [Signal Use and Emergent Cooperation](https://arxiv.org/abs/2506.18920)
*Michael Williams*
...
### [268] [Do LLMs Know When to Flip a Coin? Strategic Randomization through Reasoning and Experience](https://arxiv.org/abs/2506.18928)
*Lingyu Yang*
...
### [269] [A Comment On "The Illusion of Thinking": Reframing the Reasoning Cliff as an Agentic Gap](https://arxiv.org/abs/2506.18957)
...
### [270] [From Rows to Yields: How Foundation Models for Tabular Data Simplify Crop Yield Prediction](https://arxiv.org/abs/2506.19046)
...
### [271] [Baba is LLM: Reasoning in a Game with Dynamic Rules](https://arxiv.org/abs/2506.19095)
...
### [272] [Spiritual-LLM : Gita Inspired Mental Health Therapy In the Era of LLMs](https://arxiv.org/abs/2506.19185)
...
### [273] [Bayesian Evolutionary Swarm Architecture: A Formal Epistemic System Grounded in Truth-Based Competition](https://arxiv.org/abs/2506.19191)
*Craig Steven Wright*
...
### [274] [GBGC: Efficient and Adaptive Graph Coarsening via Granular-ball Computing](https://arxiv.org/abs/2506.19224)
...
### [275] [RecLLM-R1: A Two-Stage Training Paradigm with Reinforcement Learning and Chain-of-Thought v1](https://arxiv.org/abs/2506.19235)
...
### [276] [Emotion Detection on User Front-Facing App Interfaces for Enhanced Schedule Optimization: A Machine Learning Approach](https://arxiv.org/abs/2506.19280)
...
### [277] [Skywork-SWE: Unveiling Data Scaling Laws for Software Engineering in LLMs](https://arxiv.org/abs/2506.19290)
...
### [278] [FEAT: A Preference Feedback Dataset through a Cost-Effective Auto-Generation and Labeling Framework for English AI Tutoring](https://arxiv.org/abs/2506.19325)
...
### [279] [Evolutionary Level Repair](https://arxiv.org/abs/2506.19359)
...
### [280] [Conversational Intent-Driven GraphRAG: Enhancing Multi-Turn Dialogue Systems through Adaptive Dual-Retrieval of Flow Patterns and Context Semantics](https://arxiv.org/abs/2506.19385)
...
### [281] [Is an object-centric representation beneficial for robotic manipulation ?](https://arxiv.org/abs/2506.19408)
...
### [282] [Unsupervised Dataset Dictionary Learning for domain shift robust clustering: application to sitting posture identification](https://arxiv.org/abs/2506.19410)
...
### [284] [KunLunBaizeRAG: Reinforcement Learning Driven Inference Performance Leap for Large Language Models](https://arxiv.org/abs/2506.19466)
...
### [285] [NaviAgent: Bilevel Planning on Tool Dependency Graphs for Function Calling](https://arxiv.org/abs/2506.19500)
...
### [286] [NTRL: Encounter Generation via Reinforcement Learning for Dynamic Difficulty Adjustment in Dungeons and Dragons](https://arxiv.org/abs/2506.19530)
...
### [287] [Interpretable Hybrid Machine Learning Models Using FOLD-R++ and Answer Set Programming](https://arxiv.org/abs/2506.19573)
...
### [288] [Adaptive Domain Modeling with Language Models: A Multi-Agent Approach to Task Planning](https://arxiv.org/abs/2506.19592)
...
### [289] [ChordPrompt: Orchestrating Cross-Modal Prompt Synergy for Multi-Domain Incremental Learning in CLIP](https://arxiv.org/abs/2506.19608)
...
### [290] [Position: Intelligent Science Laboratory Requires the Integration of Cognitive and Embodied AI](https://arxiv.org/abs/2506.19613)
...
### [291] [On the efficacy of old features for the detection of new bots](https://arxiv.org/abs/2506.19635)
...
### [292] [Identifying Macro Causal Effects in C-DMGs over DMGs](https://arxiv.org/abs/2506.19650)
...
### [293] [From memories to maps: Mechanisms of in context reinforcement learning in transformers](https://arxiv.org/abs/2506.19686)
...
### [294] [Toward Decision-Oriented Prognostics: An Integrated Estimate-Optimize Framework for Predictive Maintenance](https://arxiv.org/abs/2506.19698)
...
### [295] [LLM-Driven Medical Document Analysis: Enhancing Trustworthy Pathology and Differential Diagnosis](https://arxiv.org/abs/2506.19702)
...
### [296] [From Reproduction to Replication: Evaluating Research Agents with Progressive Code Masking](https://arxiv.org/abs/2506.19724)
...
### [297] [Automatic Prompt Optimization for Knowledge Graph Construction: Insights from an Empirical Study](https://arxiv.org/abs/2506.19773)
...
### [298] [SAGE: Strategy-Adaptive Generation Engine for Query Rewriting](https://arxiv.org/abs/2506.19783)
...
### [299] [Learning Task Belief Similarity with Latent Dynamics for Meta-Reinforcement Learning](https://arxiv.org/abs/2506.19785)
...
### [300] [KnowRL: Exploring Knowledgeable Reinforcement Learning for Factuality](https://arxiv.org/abs/2506.19807)
...
### [301] [Evaluating Compliance with Visualization Guidelines in Diagrams for Scientific Publications Using Large Vision Language Models](https://arxiv.org/abs/2506.19825)
...
### [302] [Temporal-IRL: Modeling Port Congestion and Berth Scheduling with Inverse Reinforcement Learning](https://arxiv.org/abs/2506.19843)
...
### [303] [JoyAgents-R1: Joint Evolution Dynamics for Versatile Multi-LLM Agents with Reinforcement Learning](https://arxiv.org/abs/2506.19846)
...
### [304] [Signal Use and Emergent Cooperation](https://arxiv.org/abs/2506.18920)
*Michael Williams*

Main category: cs.AI

TL;DR: 研究探讨了自治代理如何通过通信信号协调活动并提升集体效率，使用NEC-DAC系统展示了代理如何通过学习形成共享行为系统（类似文化）。


<details>
  <summary>Details</summary>
Motivation: 探索自治代理如何通过通信信号自组织形成文化，并研究不同通信策略对其合作效率的影响。

Method: 使用NEC-DAC系统，每个代理配备神经网络进行决策，分析不同社会结构（如权威层级）对文化形成的影响。

Result: 研究表明合作文化显著影响部落绩效，信号不仅促进文化涌现，还支持其跨代传播。

Conclusion: 代理通过信号协调行为和学习形成文化，显著提升集体效率和合作能力。
...
### [305] [Do LLMs Know When to Flip a Coin? Strategic Randomization through Reasoning and Experience](https://arxiv.org/abs/2506.18928)
*Lingyu Yang*

Main category: cs.AI

TL;DR: 论文探讨了大型语言模型（LLM）在战略随机化方面的表现，提出了一种基于田忌赛马灵感的零和游戏，评估了五种LLM在不同提示风格下的表现。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解决现有研究中将随机化的认知决策与随机生成混为一谈的问题，以更准确地评估LLM的战略推理能力。

Method: 通过设计一个零和游戏（灵感来自田忌赛马），并采用三种提示风格（框架化、中立和暗示性）来测试五种LLM的随机化决策能力。

Result: 结果显示，较弱模型无论提示如何均保持确定性，而较强模型在明确暗示下表现出更多随机化行为。强模型在面对弱模型时采用确定性策略，面对同类时则趋向均衡策略。

Conclusion: 研究揭示了LLM在战略推理能力上的差异，为改进其抽象推理和自适应学习提供了方向。代码已开源以确保可复现性。
...
### [306] [A Comment On "The Illusion of Thinking": Reframing the Reasoning Cliff as an Agentic Gap](https://arxiv.org/abs/2506.18957)
*Sheraz Khan,Subha Madhavan,Kannan Natarajan*

Main category: cs.AI

TL;DR: 论文指出大型推理模型（LRMs）在特定复杂度阈值后性能骤降，但评论认为这是实验限制而非认知边界，并通过工具使用展示了性能反转。


<details>
  <summary>Details</summary>
Motivation: 探讨LRMs性能下降的根本原因，反驳其认知边界的结论，强调实验限制的影响。

Method: 通过工具使用和动态接口重新评估模型性能，对比静态文本范式下的表现。

Result: 工具使用显著提升模型性能，解决之前无法完成的任务，并展示了代理推理的层次性。

Conclusion: LRMs的性能问题源于实验限制而非推理能力不足，工具使用可显著改善其表现。
...
### [307] [From Rows to Yields: How Foundation Models for Tabular Data Simplify Crop Yield Prediction](https://arxiv.org/abs/2506.19046)
*Filip Sabo,Michele Meroni,Maria Piles,Martin Claverie,Fanie Ferreira,Elna Van Den Berg,Francesco Collivignarelli,Felix Rembold*

Main category: cs.AI

TL;DR: TabPFN应用于南非次国家级作物产量预测，与传统ML模型表现相当，但更高效且易于实现。
...
Method: 使用TabPFN模型，结合EO和天气数据，通过留一年交叉验证评估性能，并与6种ML模型和3种基线模型对比。
...
### [308] [Baba is LLM: Reasoning in a Game with Dynamic Rules](https://arxiv.org/abs/2506.19095)
*Fien van Wetten,Aske Plaat,Max van Duijn*

Main category: cs.AI

TL;DR: 论文探讨了大型语言模型（LLMs）在2D解谜游戏Baba is You中的表现，发现尽管LLMs在语言任务上表现良好，但在动态规则推理上仍有困难。


<details>
  <summary>Details</summary>
Motivation: 研究LLMs在结合语言能力和推理能力的游戏中的表现，以评估其解决复杂问题的能力。

Method: 评估了六种LLMs在不同提示类型下的表现，并对两种模型（Mistral、OLMo）进行了微调。

Result: 大型模型（如GPT-4o）表现较好，但小型未适应模型难以理解游戏机制；微调提升了分析能力，但对解决方案生成帮助有限。

Conclusion: LLMs在动态规则推理上仍有挑战，游戏规则动态变化的场景适合测试LLMs的推理能力。
...
### [309] [Spiritual-LLM : Gita Inspired Mental Health Therapy In the Era of LLMs](https://arxiv.org/abs/2506.19185)
*Janak Kapuriya,Aman Singh,Jainendra Shukla,Rajiv Ratn Shah*

Main category: cs.AI

TL;DR: 该研究提出了一种结合《薄伽梵歌》精神智慧与GPT-4o的新框架GITes，显著提升了情感支持的效果，并在多个指标上取得了显著改进。
...
Method: 通过整合《薄伽梵歌》的精神智慧与GPT-4o，创建GITes数据集，并评估其性能。

Result: 最佳模型Phi3-Mini 3.2B Instruct在多个指标上显著提升，如ROUGE（122.71%）、Spiritual Insight（15.92%）。

Conclusion: 结合精神智慧的AI系统有潜力提升用户满意度，但需进一步验证。
...
### [310] [Bayesian Evolutionary Swarm Architecture: A Formal Epistemic System Grounded in Truth-Based Competition](https://arxiv.org/abs/2506.19191)
*Craig Steven Wright*
...
<details>
  <summary>Details</summary>
Motivation: 构建一个数学上严格的AI系统框架，通过竞争和信念修正实现代理的演化，并验证真理的可进化性。

Method: 基于贝叶斯推理、测度论和群体动力学，代理通过竞争调整后验信念，利用哈希加密身份和因果推断算子确保系统稳健性。

Result: 系统证明了真理是进化吸引子，可验证知识来源于对抗性认知压力。

Conclusion: 该框架为自调节、可计算的群体系统提供了理论基础，展示了真理的进化稳定性。
...
### [311] [GBGC: Efficient and Adaptive Graph Coarsening via Granular-ball Computing](https://arxiv.org/abs/2506.19224)
*Shuyin Xia,Guan Wang,Gaojie Xu,Sen Zhao,Guoyin Wang*

Main category: cs.AI

TL;DR: 论文提出了一种基于多粒度特性的图粗化方法GBGC，通过自适应粒度球计算优化粗化结果和效率。


<details>
  <summary>Details</summary>
Motivation: 传统图粗化方法主要基于谱保持视角，忽略了图的多粒度特性，导致粗化效果和效率不足。

Method: GBGC采用自适应粒度球图细化机制，将原始图从粗到细分割为不同大小的粒度球，并以这些球为超节点构建粗化图。

Result: GBGC的处理速度比其他方法快数十到数百倍，时间复杂度更低，且准确性几乎总是高于原始图。
...
### [312] [RecLLM-R1: A Two-Stage Training Paradigm with Reinforcement Learning and Chain-of-Thought v1](https://arxiv.org/abs/2506.19235)
*Yu Xie,Xingkai Ren,Ying Qi,Yao Hu,Lianlei Shan*
...
Conclusion: RecLLM-R1为推荐模型和业务政策的集成优化提供了有效途径。
...
### [313] [Emotion Detection on User Front-Facing App Interfaces for Enhanced Schedule Optimization: A Machine Learning Approach](https://arxiv.org/abs/2506.19280)
*Feiting Yang,Antoine Moevus,Steve Lévesque*

Main category: cs.AI

TL;DR: 该论文探讨了将情绪检测技术集成到日历应用中，通过生物特征和行为两种方法提升用户体验。生物特征方法使用心电图数据和神经网络预测情绪，行为方法通过用户交互数据分类情绪。结果显示行为方法更准确，尤其是鼠标交互。


<details>
  <summary>Details</summary>
Motivation: 通过情绪检测技术提升日历应用的个性化和适应性，改善用户的生产力和参与度。

Method: 采用两种方法：1) 生物特征方法，基于心电图数据和LSTM/GRU神经网络预测情绪；2) 行为方法，通过用户交互数据（如鼠标移动、点击和键盘输入）分类情绪。

Result: 行为方法在鼠标交互中达到约90%的准确率，表现优于生物特征方法。GRU网络在生物特征方法中表现更好，预测Valence的准确率为84.38%。

Conclusion: 行为方法在情绪检测中更具一致性和准确性，尤其是鼠标交互。GRU网络在生物特征方法中优于LSTM。
...
### [314] [Skywork-SWE: Unveiling Data Scaling Laws for Software Engineering in LLMs](https://arxiv.org/abs/2506.19290)
*Liang Zeng,Yongcong Li,Yuzhen Xiao,Changshi Li,Chris Yuhao Liu,Rui Yan,Tianwen Wei,Jujie He,Xuchen Song,Yang Liu,Yahui Zhou*
...
<details>
  <summary>Details</summary>
Motivation: 软件工程数据集的整理过程耗时且依赖人工标注，现有数据集规模有限，限制了LLM代理的能力测试。

Method: 采用增量式自动化数据整理流程，构建包含10,169个Python任务实例的数据集，并用于微调Skywork-SWE模型。

Result: Skywork-SWE模型在SWE-bench基准测试中达到38.0% pass@1准确率，结合测试时扩展技术后提升至47.0%。

Conclusion: 自动化数据整理和模型微调显著提升了LLM在软件工程任务中的性能，为未来研究提供了新的基准和工具。
...
### [315] [FEAT: A Preference Feedback Dataset through a Cost-Effective Auto-Generation and Labeling Framework for English AI Tutoring](https://arxiv.org/abs/2506.19325)
*Hyein Seo,Taewook Hwang,Yohan Lee,sangkeun Jung*

Main category: cs.AI

TL;DR: FEAT框架通过结合人工和LLM生成教师反馈，构建了三个互补数据集，实验表明少量高质量数据（5-10%）能显著提升性能。
...
Method: 提出FEAT框架，构建DM（人工+LLM）、DG（纯LLM）和DA（DG+少量DM）三个数据集。

Result: 实验显示DA（含5-10% DM）性能优于纯DM。
...
### [316] [Evolutionary Level Repair](https://arxiv.org/abs/2506.19359)
*Debosmita Bhaumik,Julian Togelius,Georgios N. Yannakakis,Ahmed Khalifa*
...
<details>
  <summary>Details</summary>
Motivation: 解决游戏关卡设计中的非功能性修复问题，如完整性、可达性和性能优化，同时限制修改次数。

Method: 采用基于搜索的解决方案，包括进化算法和多样性质量算法，修复机器学习生成的关卡。
...
### [317] [Conversational Intent-Driven GraphRAG: Enhancing Multi-Turn Dialogue Systems through Adaptive Dual-Retrieval of Flow Patterns and Context Semantics](https://arxiv.org/abs/2506.19385)
*Ziqi Zhu,Tao Hu,Honglong Zhang,Dan Yang,HanGeng Chen,Mengran Zhang,Xilun Chen*

Main category: cs.AI

TL;DR: CID-GraphRAG通过动态意图转移图和双检索机制，显著提升了多轮客服对话的上下文连贯性和目标导向性。
...
Conclusion: CID-GraphRAG通过意图转移结构与语义检索的协同效应，成为知识密集型多轮对话的有效框架。
...
### [318] [Is an object-centric representation beneficial for robotic manipulation ?](https://arxiv.org/abs/2506.19408)
*Alexandre Chapin,Emmanuel Dellandrea,Liming Chen*

Main category: cs.AI

TL;DR: 论文探讨了对象中心表示（OCR）在机器人操作任务中的潜力，通过模拟环境中的多对象任务验证其泛化能力。


<details>
  <summary>Details</summary>
Motivation: 现有OCR研究多关注场景分解，缺乏对学习表示推理能力的评估，而机器人操作任务涉及多对象交互，是验证OCR潜力的理想场景。

Method: 创建多对象、高随机化的机器人操作模拟任务，评估经典OCR方法与整体表示方法在泛化场景中的表现。

Result: OCR方法在复杂场景结构中表现优于整体表示方法，能克服困难场景的挑战。

Conclusion: OCR在机器人操作任务中展现出潜力，尤其在复杂场景中具有优势。
...
### [319] [Unsupervised Dataset Dictionary Learning for domain shift robust clustering: application to sitting posture identification](https://arxiv.org/abs/2506.19410)
*Anas Hattay,Mayara Ayat,Fred Ngole Mboula*

Main category: cs.AI

TL;DR: 论文提出了一种名为U-DaDiL的无监督学习方法，用于坐姿识别的鲁棒聚类，解决了传统方法在数据集适应性和领域偏移上的不足。
...
Result: 在Office31数据集上的实验表明，聚类对齐准确率显著提升。

Conclusion: U-DaDiL为解决领域偏移和鲁棒聚类问题提供了有前景的方向。
...
### [320] [Commander-GPT: Dividing and Routing for Multimodal Sarcasm Detection](https://arxiv.org/abs/2506.19420)
*Yazhou Zhang,Chunwang Zou,Bo Wang,Jing Qin*

Main category: cs.AI

TL;DR: Commander-GPT是一个模块化决策路由框架，通过协调多个专门化的LLM代理来提升多模态讽刺理解能力，相比现有技术有显著改进。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLM）在多模态讽刺理解任务中表现不佳，因此需要一种更高效的方法来整合多个LLM的能力。

Method: 提出Commander-GPT框架，利用三种集中式指挥官协调多个专门化LLM代理，分别处理子任务（如上下文建模、情感分析等），最终由指挥官整合信息并做出讽刺判断。

Result: 在MMSD和MMSD 2.0基准测试中，F1分数分别比现有最优方法平均提高了4.4%和11.7%。
...
### [321] [KunLunBaizeRAG: Reinforcement Learning Driven Inference Performance Leap for Large Language Models](https://arxiv.org/abs/2506.19466)
*Cheng Li,Jiexiong Liu,Yixuan Chen,Qihang Zhou,KunLun Meta*
...
### [322] [NaviAgent: Bilevel Planning on Tool Dependency Graphs for Function Calling](https://arxiv.org/abs/2506.19500)
*Yan Jiang,Hao Zhou,LiZhong GU,Ai Han,TianLong Li*
...
<details>
  <summary>Details</summary>
Motivation: 现有方法依赖静态知识和单一执行路径，导致错误恢复能力差和搜索空间爆炸，无法有效协调复杂异构工具链。

Method: NaviAgent采用双层架构：多路径决策器动态选择最优动作，图编码导航器构建工具依赖异构图并集成启发式搜索策略。

Result: 实验表明，NaviAgent在任务成功率上显著优于基线方法，且执行步骤接近最优基线，平衡了质量与效率。

Conclusion: NaviAgent通过图导航和动态决策显著提升了LLM在复杂工具链调用中的表现，尤其在大型模型上效果更佳。
...
### [323] [NTRL: Encounter Generation via Reinforcement Learning for Dynamic Difficulty Adjustment in Dungeons and Dragons](https://arxiv.org/abs/2506.19530)
*Carlo Romeo,Andrew D. Bagdanov*

Main category: cs.AI

TL;DR: NTRL利用强化学习自动调整D&D战斗难度，提升战斗时长和战术深度，同时保持公平性。


<details>
  <summary>Details</summary>
Motivation: 解决D&D中DM手动平衡战斗难度的复杂性，避免打断叙事流程。

Method: 将问题建模为上下文多臂老虎机，基于实时队伍属性生成战斗。

Result: 延长战斗时长200%，减少战后生命值16.67%，提升战术性，同时保持70%胜率。

Conclusion: NTRL在提升战斗难度和战术深度方面优于人工DM，同时保持游戏公平性。
...
### [324] [Interpretable Hybrid Machine Learning Models Using FOLD-R++ and Answer Set Programming](https://arxiv.org/abs/2506.19573)
*Sanne Wielinga,Jesse Heyninck*

Main category: cs.AI

TL;DR: 提出了一种结合符号推理（ASP）与黑盒机器学习的方法，通过FOLD-R++算法生成可解释规则，修正不确定预测，提升医疗数据集上的准确性和F1分数。


<details>
  <summary>Details</summary>
Motivation: 在高风险领域（如医疗）中，机器学习的高性能方法（如神经网络）缺乏可解释性，而符号方法（如ASP）虽可解释但预测能力有限，因此需要结合两者优势。

Method: 采用混合方法，将FOLD-R++算法生成的ASP规则与黑盒ML分类器结合，选择性修正不确定预测并提供可解释的规则。

Result: 在五个医疗数据集上实验显示，该方法显著提升了准确性和F1分数。

Conclusion: 研究表明，结合符号推理与传统ML可以在不牺牲准确性的前提下实现高可解释性。
...
### [325] [Adaptive Domain Modeling with Language Models: A Multi-Agent Approach to Task Planning](https://arxiv.org/abs/2506.19592)
*Harisankar Babu,Philipp Schillinger,Tamim Asfour*
...
Conclusion: TAPAS通过动态生成和适应，有效解决了复杂任务规划问题。
...
### [326] [ChordPrompt: Orchestrating Cross-Modal Prompt Synergy for Multi-Domain Incremental Learning in CLIP](https://arxiv.org/abs/2506.19608)
*Zhiyuan Wang,Bokui Chen*

Main category: cs.AI

TL;DR: ChordPrompt框架通过跨模态提示和领域自适应文本提示，提升了预训练视觉语言模型在多领域增量学习中的性能。


<details>
  <summary>Details</summary>
Motivation: 现有提示学习方法主要关注类增量学习，缺乏多领域任务增量学习的策略，且多采用单模态提示，忽略了跨模态信息交换的潜力。

Method: 提出ChordPrompt框架，利用视觉和文本提示的协同作用，引入跨模态提示和领域自适应文本提示。

Result: 在多领域增量学习基准测试中，ChordPrompt在零样本泛化和下游任务性能上优于现有方法。

Conclusion: ChordPrompt通过跨模态和领域自适应提示，有效解决了视觉语言模型在多领域增量学习中的挑战。
...
### [327] [Position: Intelligent Science Laboratory Requires the Integration of Cognitive and Embodied AI](https://arxiv.org/abs/2506.19613)
*Sha Zhang,Suorong Yang,Tong Xie,Xiangyuan Xue,Zixuan Hu,Rui Li,Wenxi Qu,Zhenfei Yin,Tianfan Fu,Di Hu,Andres M Bran,Nian Ran,Bram Hoex,Wangmeng Zuo,Philippe Schwaller,Wanli Ouyang,Lei Bai,Yanyong Zhang,Lingyu Duan,Shixiang Tang,Dongzhan Zhou*

Main category: cs.AI

TL;DR: 提出智能科学实验室（ISLs）框架，结合认知与实体智能，以解决当前科学发现的局限性。


<details>
  <summary>Details</summary>
Motivation: 科学发现受限于人类能力和AI系统的虚拟环境限制，需要结合认知与实体智能以实现自主实验和偶然发现。

Method: 提出多层次的闭环框架ISLs，整合科学推理基础模型、基于代理的工作流程编排和实体代理。

Result: ISLs有望克服科学发现的当前限制，实现AI驱动科学的变革潜力。

Conclusion: ISLs是推动科学发现的关键，结合认知与实体智能，为自主实验和偶然发现提供新途径。
...
### [328] [On the efficacy of old features for the detection of new bots](https://arxiv.org/abs/2506.19635)
*Rocco De Nicola,Marinella Petrocchi,Manuel Pratelli*

Main category: cs.AI

TL;DR: 该研究比较了四种先进特征集在检测新型机器人（bots）上的性能，发现通用分类器和低成本计算特征可能有效。


<details>
  <summary>Details</summary>
Motivation: 恶意机器人对在线生态系统造成危害，如传播垃圾信息和操纵公众意见，因此需要有效的检测方法。

Method: 使用Twitter作为基准，比较了四种特征集的性能，包括Botometer的输出分数、账户资料和时间线特征，以及Twitter客户端信息。

Result: 在六个最新发布的Twitter账户数据集上，分析结果表明通用分类器和低成本特征可能适用于检测进化后的机器人。

Conclusion: 低成本计算的特征和通用分类器可能是检测新型机器人的可行方案。
...
### [329] [Identifying Macro Causal Effects in C-DMGs over DMGs](https://arxiv.org/abs/2506.19650)
*Simon Ferreira,Charles K. Assaad*
...
<details>
  <summary>Details</summary>
Motivation: 现实世界中高维数据和循环因果关系的普遍性使得完全指定的ADMGs难以构建，因此需要研究部分指定的因果表示方法。

Method: 通过引入ioSCMs和DMGs，研究C-DMGs在DMGs上的宏观因果效应识别问题，并证明do-calculus的适用性。

Result: 在DMGs上的C-DMGs中，do-calculus无条件地适用于识别宏观因果效应，且非可识别性标准可扩展到部分C-DMGs。

Conclusion: 研究为处理循环因果关系的复杂系统提供了理论支持，扩展了do-calculus的应用范围。
...
### [330] [From memories to maps: Mechanisms of in context reinforcement learning in transformers](https://arxiv.org/abs/2506.19686)
*Ching Fang,Kanaka Rajan*

Main category: cs.AI

TL;DR: 该论文研究了如何通过Transformer模型模拟人类和动物的快速学习能力，发现记忆作为计算资源支持灵活行为，并与大脑海马-内嗅系统相似。


<details>
  <summary>Details</summary>
Motivation: 探索人类和动物快速适应新环境的能力，以及如何通过Transformer模型模拟这种能力，研究记忆在快速学习中的作用。

Method: 使用Transformer模型在分布式的规划任务中进行上下文强化学习，分析模型的学习算法和表征学习。

Result: 发现模型通过缓存中间计算支持上下文强化学习，其表征与大脑海马-内嗅系统相似，不同于标准模型无关或基于模型的规划。

Conclusion: 记忆可作为计算资源支持灵活行为，研究结果为自然认知中的快速学习提供了机制假设。
...
### [331] [Toward Decision-Oriented Prognostics: An Integrated Estimate-Optimize Framework for Predictive Maintenance](https://arxiv.org/abs/2506.19698)
*Zhuojun Xie,Adam Abdin,Yiping Fang*

Main category: cs.AI

TL;DR: 论文提出了一种集成估计优化（IEO）框架，通过联合调整预测模型并直接优化维护结果，以减少预测误差对维护决策的影响，相比传统估计后优化（ETO）框架，平均维护遗憾降低了22%。


<details>
  <summary>Details</summary>
Motivation: 尽管机器学习在预测性维护（PdM）中的应用日益增多，但模型不确定性限制了工业广泛采用。研究旨在解决预测准确性是否必然带来更好的维护决策，以及如何减轻预测误差对决策的影响。

Method: 提出IEO框架，通过联合优化预测模型和维护目标，开发了一种适用于小样本数据的随机扰动梯度下降算法。

Result: 实证研究表明，IEO框架比ETO框架平均减少22%的维护遗憾，提高了决策质量和鲁棒性。

Conclusion: IEO框架通过将预测模型训练与维护目标对齐，显著改善了在模型不确定性下的决策质量，为数据驱动的PdM提供了可靠方法。
...
### [332] [LLM-Driven Medical Document Analysis: Enhancing Trustworthy Pathology and Differential Diagnosis](https://arxiv.org/abs/2506.19702)
*Lei Kang,Xuanshuo Fu,Oriol Ramos Terrades,Javier Vazquez-Corral,Ernest Valveny,Dimosthenis Karatzas*

Main category: cs.AI

TL;DR: 提出了一种基于LLaMA-v3的医疗文档分析平台，通过低秩适配优化，解决了隐私和诊断准确性问题，并在DDXPlus数据集上表现优异。


<details>
  <summary>Details</summary>
Motivation: 医疗文档分析对临床诊断至关重要，但现有大型语言模型（LLMs）存在隐私问题，限制了其在临床中的应用。

Method: 采用LLaMA-v3模型，结合低秩适配技术（LoRA），优化用于差异诊断任务，并开发了一个基于网络的平台。

Result: 在DDXPlus数据集上表现优于现有方法，提供透明且可靠的诊断结果。

Conclusion: 该方法满足了可靠、可解释且保护隐私的AI需求，是医疗文档分析的重要进展。
...
### [333] [From Reproduction to Replication: Evaluating Research Agents with Progressive Code Masking](https://arxiv.org/abs/2506.19724)
*Gyeongwon James Kim,Alex Wilf,Louis-Philippe Morency,Daniel Fried*

Main category: cs.AI

TL;DR: AutoExperiment是一个评估AI代理根据研究论文自然语言描述实现和运行机器学习实验能力的基准测试，通过调整缺失代码量来测试代理从部分复制到完全复现的能力。
...
Method: 基准测试提供研究论文、部分代码（关键函数被屏蔽）和实验运行命令，要求代理生成缺失代码并在沙盒环境中运行实验。难度通过缺失函数数量$n$调整。

Result: 实验显示，随着$n$增加，代理性能迅速下降；动态交互环境的代理表现优于固定“无代理”环境，单次与多次尝试成功率差距显著。

Conclusion: AutoExperiment揭示了长周期代码生成、上下文检索和自主实验执行的关键挑战，为AI驱动的科学实验提供了新基准。
...
### [334] [Automatic Prompt Optimization for Knowledge Graph Construction: Insights from an Empirical Study](https://arxiv.org/abs/2506.19773)
*Nandana Mihindukulasooriya,Niharika S. D'Souza,Faisal Chowdhury,Horst Samulowitz*

Main category: cs.AI

TL;DR: 论文研究了自动提示优化在知识图谱三元组抽取任务中的应用，通过实验验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 手工设计针对特定任务的提示对大型语言模型（LLM）来说耗时且脆弱，自动提示优化可以解决这一问题。

Method: 通过改变提示策略、LLM模型、模式复杂性、输入文本长度和多样性等变量，评估了三种自动提示优化工具（DSPy、APE、TextGrad）在SynthIE和REBEL数据集上的表现。

Result: 自动提示优化生成的提示与人工设计相当，且在模式复杂性和文本规模增加时表现更优。
...
### [335] [SAGE: Strategy-Adaptive Generation Engine for Query Rewriting](https://arxiv.org/abs/2506.19783)
*Teng Wang,Hailei Gong,Changwang Zhang,Jun Wang*
...
Result: 在HotpotQA等基准测试中取得最优NDCG@10结果，同时降低了推理成本。

Conclusion: 策略引导的强化学习方法为信息检索系统提供了高效、可扩展且可解释的新范式。
...
### [336] [Learning Task Belief Similarity with Latent Dynamics for Meta-Reinforcement Learning](https://arxiv.org/abs/2506.19785)
*Menglong Zhang,Fuyuan Qian*

Main category: cs.AI

TL;DR: SimBelief是一个基于任务信念相似性的元强化学习框架，用于稀疏奖励环境中的高效任务识别和探索。


<details>
  <summary>Details</summary>
Motivation: 传统Bayes-Adaptive深度强化学习方法在稀疏奖励环境中难以准确重建奖励信号，导致次优利用。

Method: 提出SimBelief框架，通过测量BAMDP中的任务信念相似性，提取相似任务分布的共同特征，并结合潜在任务信念度量学习任务结构。

Result: 在稀疏奖励的MuJoCo和panda-gym任务上优于现有基线方法。

Conclusion: SimBelief通过任务信念相似性实现了高效的任务识别和探索，适用于稀疏奖励环境。
...
### [337] [KnowRL: Exploring Knowledgeable Reinforcement Learning for Factuality](https://arxiv.org/abs/2506.19807)
*Baochang Ren,Shuofei Qiao,Wenhao Yu,Huajun Chen,Ningyu Zhang*

Main category: cs.AI

TL;DR: KnowRL通过将基于知识验证的事实性奖励整合到RL训练中，减少慢思考模型的幻觉问题。
...
Method: 提出KnowRL，将事实性奖励引入RL训练，指导模型进行基于事实的慢思考。
...
### [338] [Evaluating Compliance with Visualization Guidelines in Diagrams for Scientific Publications Using Large Vision Language Models](https://arxiv.org/abs/2506.19825)
*Johannes Rückert,Louise Bloch,Christoph M. Friedrich*

Main category: cs.AI

TL;DR: 研究利用视觉语言模型（VLMs）分析图表，以识别数据可视化原则中的潜在问题，并比较了五种VLMs和五种提示策略的效果。


<details>
  <summary>Details</summary>
Motivation: 研究人员常不遵循数据可视化原则，导致图表信息不准确或不完整，因此需要自动化工具来识别这些问题。

Method: 使用五种开源VLMs和五种提示策略，通过从数据可视化指南中提取的问题集来分析图表。

Result: VLMs在分析图表类型、3D效果、轴标签等方面表现良好，但在图像质量和刻度标记方面效果较差。Qwen2.5VL和总结提示策略表现最佳。

Conclusion: VLMs可用于自动识别图表中的潜在问题，如缺失轴标签和图例，未来可扩展用于更多数据可视化方面。
...
### [339] [Temporal-IRL: Modeling Port Congestion and Berth Scheduling with Inverse Reinforcement Learning](https://arxiv.org/abs/2506.19843)
*Guo Li,Zixiang Xu,Wei Zhang,Yikuan Hu,Xinyu Yang,Nikolay Aristov,Mingjie Tang,Elenna R Dugundji*
...
Method: 利用历史AIS数据重建停泊计划，通过Temporal-IRL模型学习停泊调度优先级和模式，预测船舶序列和停泊时间。
...
### [340] [JoyAgents-R1: Joint Evolution Dynamics for Versatile Multi-LLM Agents with Reinforcement Learning](https://arxiv.org/abs/2506.19846)
*Ai Han,Junxing Hu,Pu Wei,Zhiqian Zhang,Yuhang Guo,Jiawei Lu,Zicheng Zhang*

Main category: cs.AI

TL;DR: 提出了一种名为JoyAgents-R1的多智能体强化学习方法，通过GRPO和自适应记忆机制提升异构智能体的协作效率和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 解决异构多智能体在联合进化中的协作效率低和训练不稳定的问题。

Method: 采用GRPO联合训练，结合节点级蒙特卡洛采样和边际效益驱动的选择策略，并引入自适应记忆进化机制。

Result: 在通用和领域特定场景中，JoyAgents-R1性能接近更大LLM，同时基于较小开源模型。

Conclusion: JoyAgents-R1通过优化采样和记忆机制，实现了异构多智能体的高效协作和稳定训练。
...
### [341] [Trustworthy Artificial Intelligence for Cyber Threat Analysis](https://arxiv.org/abs/2506.19052)
...
### [342] [PolyGuard: Massive Multi-Domain Safety Policy-Grounded Guardrail Dataset](https://arxiv.org/abs/2506.19054)
...
### [343] [Enhancing Security in LLM Applications: A Performance Evaluation of Early Detection Systems](https://arxiv.org/abs/2506.19109)
...
### [344] [Network Structures as an Attack Surface: Topology-Based Privacy Leakage in Federated Learning](https://arxiv.org/abs/2506.19260)
...
### [345] [WebGuard++:Interpretable Malicious URL Detection via Bidirectional Fusion of HTML Subgraphs and Multi-Scale Convolutional BERT](https://arxiv.org/abs/2506.19356)
...
### [346] [SoK: Can Synthetic Images Replace Real Data? A Survey of Utility and Privacy of Synthetic Image Generation](https://arxiv.org/abs/2506.19360)
...
### [347] [Yotta: A Large-Scale Trustless Data Trading Scheme for Blockchain System](https://arxiv.org/abs/2506.19368)
...
### [348] [ZK-SERIES: Privacy-Preserving Authentication using Temporal Biometric Data](https://arxiv.org/abs/2506.19393)
...
### [349] [An ETSI GS QKD compliant TLS implementation](https://arxiv.org/abs/2506.19409)
...
### [350] [FuncVul: An Effective Function Level Vulnerability Detection Model using LLM and Code Chunk](https://arxiv.org/abs/2506.19453)
...
### [351] [PhishingHook: Catching Phishing Ethereum Smart Contracts leveraging EVM Opcodes](https://arxiv.org/abs/2506.19480)
...
### [352] [PrivacyXray: Detecting Privacy Breaches in LLMs through Semantic Consistency and Probability Certainty](https://arxiv.org/abs/2506.19563)
...
### [353] [Decompiling Smart Contracts with a Large Language Model](https://arxiv.org/abs/2506.19624)
...
### [354] [A Survey of LLM-Driven AI Agent Communication: Protocols, Security Risks, and Defense Countermeasures](https://arxiv.org/abs/2506.19676)
...
### [355] [KnowML: Improving Generalization of ML-NIDS with Attack Knowledge Graphs](https://arxiv.org/abs/2506.19802)
...
### [356] [Machine Learning with Privacy for Protected Attributes](https://arxiv.org/abs/2506.19836)
...
### [357] [Privacy-Preserving LLM Interaction with Socratic Chain-of-Thought Reasoning and Homomorphically Encrypted Vector Databases](https://arxiv.org/abs/2506.17336)
...
### [358] [Trustworthy Artificial Intelligence for Cyber Threat Analysis](https://arxiv.org/abs/2506.19052)
*Shuangbao Paul Wang,Paul Mullin*

Main category: cs.CR

TL;DR: 论文提出了一种基于机器学习的网络安全威胁检测与评估工具，结合无监督和监督学习，在AWS云服务器日志数据上表现出高效威胁识别能力。


<details>
  <summary>Details</summary>
Motivation: 人工智能虽带来创新，但算法中的偏见和不道德问题降低了应用的可信度。研究旨在利用机器学习提升威胁检测的准确性和可信度。

Method: 采用两阶段（无监督和监督学习）分析方法，处理AWS云服务器日志数据，结合强化学习和量子力学提升分类速度和准确性。

Result: 算法能够高置信度地识别网络威胁，表现出优于传统方法的性能。

Conclusion: 该工具为网络安全威胁检测提供了高效且可信的解决方案，结合量子力学和强化学习的潜力值得进一步探索。
...
### [359] [PolyGuard: Massive Multi-Domain Safety Policy-Grounded Guardrail Dataset](https://arxiv.org/abs/2506.19054)
*Mintong Kang,Zhaorun Chen,Chejian Xu,Jiawei Zhang,Chengquan Guo,Minzhou Pan,Ivan Revilla,Yu Sun,Bo Li*

Main category: cs.CR

TL;DR: PolyGuard是一个多领域安全策略驱动的护栏数据集，填补了现有护栏基准在领域覆盖和政策依据上的不足，并评估了19种护栏模型的表现。


<details>
  <summary>Details</summary>
Motivation: 现有护栏基准缺乏标准化安全政策的依据，且忽视领域特定风险，PolyGuard旨在解决这些问题。

Method: 构建PolyGuard数据集，覆盖八个安全关键领域，基于真实安全指南，包含多样交互格式和对抗性输入。

Result: 评估显示模型在不同风险类别上表现不一，对对抗性攻击仍脆弱，但安全风险覆盖范围随模型进化扩大。
...
### [360] [Enhancing Security in LLM Applications: A Performance Evaluation of Early Detection Systems](https://arxiv.org/abs/2506.19109)
*Valerii Gakh,Hayretdin Bahsi*

Main category: cs.CR

TL;DR: 论文研究了提示注入攻击对LLM应用的威胁，分析了现有开源解决方案（如LLM Guard、Vigil和Rebuff）的检测性能，提出了改进建议，并比较了它们的优缺点。


<details>
  <summary>Details</summary>
Motivation: 随着LLM应用的普及和多样化，提示注入攻击（如提示泄露）威胁系统安全，现有防御措施不足，需要研究现有检测技术的有效性。

Method: 研究分析了多种开源解决方案的提示泄露检测技术，比较了LLM Guard、Vigil和Rebuff的性能，并提出了改进建议。

Result: 发现Vigil和Rebuff的检测技术存在不足，提出了改进方案；比较结果显示Vigil适合低误报率场景，Rebuff适合一般需求。

Conclusion: 现有检测技术需优化，Vigil和Rebuff各有适用场景，未来需进一步改进防御措施。
...
### [361] [Network Structures as an Attack Surface: Topology-Based Privacy Leakage in Federated Learning](https://arxiv.org/abs/2506.19260)
*Murtaza Rangwala,Richard O. Sinnott,Rajkumar Buyya*

Main category: cs.CR

TL;DR: 该论文首次全面分析了联邦学习系统中基于网络拓扑的隐私泄露问题，提出了三种攻击向量，并在多种对抗知识场景下验证了攻击效果，同时提出并验证了结构噪声注入作为防御机制的有效性。


<details>
  <summary>Details</summary>
Motivation: 现有隐私研究主要关注基于梯度的攻击，而网络拓扑知识的隐私影响尚未得到充分研究。论文旨在填补这一空白，分析拓扑知识对隐私的威胁。

Method: 通过系统评估4,720个攻击实例，分析了六种对抗知识场景，提出了三种攻击向量（通信模式分析、参数幅度分析和结构位置相关性），并验证了结构噪声注入的防御效果。

Result: 在完全知识条件下，三种攻击向量的成功率分别为84.1%、65.0%和47.2%；80%的部分知识场景仍保持高于安全阈值的攻击效果。结构噪声注入可额外减少51.4%的攻击。

Conclusion: 网络拓扑是联邦学习系统中的基本隐私漏洞，但通过拓扑感知的防御机制（如结构噪声注入）可以有效缓解这一问题。
...
### [362] [WebGuard++:Interpretable Malicious URL Detection via Bidirectional Fusion of HTML Subgraphs and Multi-Scale Convolutional BERT](https://arxiv.org/abs/2506.19356)
*Ye Tian,Zhang Yumin,Yifan Jia,Jianguo Sun,Yanbin Wang*
...
Result: 实验表明，WebGuard++在固定FPR下，TPR显著高于现有方法，性能提升1.1x-7.9x。

Conclusion: WebGuard++通过多组件协同工作，有效提升了恶意URL检测的准确性和可解释性。
...
### [363] [SoK: Can Synthetic Images Replace Real Data? A Survey of Utility and Privacy of Synthetic Image Generation](https://arxiv.org/abs/2506.19360)
*Yunsung Chung,Yunbei Zhang,Nassir Marrouche,Jihun Hamm*

Main category: cs.CR

TL;DR: 该论文对隐私保护数据合成（PPDS）中的合成图像生成方法进行了系统分类和比较，并通过基准测试评估了不同方法的隐私风险和实用性。


<details>
  <summary>Details</summary>
Motivation: 由于生成模型的进步推动了隐私保护数据合成领域的发展，但缺乏对不同设置下合成图像生成方法的全面调查和比较。

Method: 系统分类了图像合成方法、隐私攻击和缓解措施，并通过模型无关的成员推理攻击（MIA）作为隐私风险衡量标准进行实证比较。

Result: 研究回答了PPDS中的关键问题，如合成数据是否能有效替代真实数据、哪种发布策略能平衡实用性和隐私等。

Conclusion: 研究为合成数据生成方法的实用性与隐私权衡提供了可操作的见解，并指导了现实应用中的最优数据发布策略。
...
### [364] [Yotta: A Large-Scale Trustless Data Trading Scheme for Blockchain System](https://arxiv.org/abs/2506.19368)
*Xiang Liu,Zhanpeng Guo,Liangxi Liu,Mengyao Zheng,Yiming Qiu,Linshan Jiang*

Main category: cs.CR

TL;DR: 论文提出了一种名为Yotta的批量数据交易方案，首次形式化了Web 3.0中大规模数据交易的属性需求，并通过创新的加密工作流结合IPFS和zk-SNARK技术实现高效安全的交易。


<details>
  <summary>Details</summary>
Motivation: 当前基于区块链智能合约的数据交换方法无法同时支持大规模数据交易和确保数据安全，且缺乏对大规模数据交易应满足的基本属性的讨论。

Method: 提出Yotta方案，利用创新的加密工作流结合IPFS和zk-SNARK技术，设计了一种完整的批量数据交易方案。

Result: 模拟结果显示，Yotta比基线方法性能提升高达130倍，并展现出优秀的可扩展性，满足所有属性需求。

Conclusion: Yotta是首个满足Web 3.0大规模数据交易需求的完整方案，具有高效性和安全性。
...
### [365] [ZK-SERIES: Privacy-Preserving Authentication using Temporal Biometric Data](https://arxiv.org/abs/2506.19393)
*Daniel Reijsbergen,Eyasu Getahun Chekole,Howard Halim,Jianying Zhou*

Main category: cs.CR

TL;DR: ZK-SERIES 是一种隐私保护的时间序列认证协议，适用于低性能设备，如智能手机，能在1.3秒内完成认证。


<details>
  <summary>Details</summary>
Motivation: 生物特征认证需要保护隐私，且现有方法效率不足，尤其是在低性能设备上。

Method: 使用零知识乘法证明和批量范围证明，优化兼容低性能设备。

Result: 在5年老旧手机上完成认证仅需1.3秒，验证了实用性和扩展性。

Conclusion: ZK-SERIES 提供高效、隐私保护的生物特征认证方案，适用于广泛设备。
...
### [366] [An ETSI GS QKD compliant TLS implementation](https://arxiv.org/abs/2506.19409)
*Thomas Prévost,Bruno Martin,Olivier Alibart*

Main category: cs.CR

TL;DR: 论文提出了一种基于QKD的TLS协议修改方案，保持了客户端和服务器的向后兼容性，并测试了其性能。


<details>
  <summary>Details</summary>
Motivation: 推动量子密钥分发（QKD）在互联网中的广泛应用。

Method: 使用Rustls库实现TLS协议修改，结合ETSI GS QKD 014 v1.1.1标准，保持向后兼容性。

Result: 成功用于QKD加密的视频会议通话，性能分析显示与TLS 1.3相比握手时间有所变化。

Conclusion: 该协议为QKD在互联网中的推广提供了可行方案，性能表现值得进一步优化。
...
### [367] [FuncVul: An Effective Function Level Vulnerability Detection Model using LLM and Code Chunk](https://arxiv.org/abs/2506.19453)
*Sajal Halder,Muhammad Ejaz Ahmed,Seyit Camtepe*

Main category: cs.CR

TL;DR: FuncVul是一种基于代码块的函数级漏洞检测模型，专注于C/C++和Python中的关键代码段，显著提高了漏洞检测的准确性和F1分数。


<details>
  <summary>Details</summary>
Motivation: 现有方法通常忽视具体漏洞函数，而识别这些函数对降低开源软件风险至关重要。FuncVul旨在解决这一问题。

Method: FuncVul利用GraphCodeBERT微调模型，结合代码块和补丁信息，通过大型语言模型标记漏洞样本。

Result: FuncVul在多个数据集上表现优异，平均准确率达87-92%，F1分数为86-92%，比全函数预测提升53.9%准确率和42.0% F1分数。

Conclusion: FuncVul通过聚焦关键代码段，显著提升了函数级漏洞检测效果，代码和数据集已开源。
...
### [368] [PhishingHook: Catching Phishing Ethereum Smart Contracts leveraging EVM Opcodes](https://arxiv.org/abs/2506.19480)
*Pasquale De Rosa,Simon Queyrut,Yérom-David Bromberg,Pascal Felber,Valerio Schiavoni*

Main category: cs.CR

TL;DR: PhishingHook是一个通过分析智能合约字节码和操作码来检测钓鱼活动的机器学习框架，实验表明其平均准确率约为90%。


<details>
  <summary>Details</summary>
Motivation: 以太坊虚拟机器（EVM）的广泛使用导致钓鱼活动增加，现有方法存在安全风险，需要一种不依赖交易重放的新检测方法。

Method: PhishingHook直接分析智能合约的字节码和操作码，使用16种机器学习技术（分为四类）进行恶意模式检测。

Result: 在7,000个真实恶意智能合约上测试，PhishingHook的平均准确率达到90%。

Conclusion: PhishingHook提供了一种高效且安全的钓鱼活动检测方法，并公开了代码和数据集以促进研究。
...
### [369] [PrivacyXray: Detecting Privacy Breaches in LLMs through Semantic Consistency and Probability Certainty](https://arxiv.org/abs/2506.19563)
*Jinwen He,Yiyang Lu,Zijin Lin,Kai Chen,Yue Zhao*

Main category: cs.CR

TL;DR: PrivacyXray是一种通过分析LLM内部状态检测隐私泄露的新框架，解决了现有隐私提取攻击无法验证信息准确性的问题。


<details>
  <summary>Details</summary>
Motivation: LLMs在敏感领域广泛应用，但隐私提取攻击（如越狱）暴露了其漏洞，且缺乏公开数据集验证提取的隐私信息准确性。

Method: PrivacyXray通过分析LLM内部状态（语义一致性和概率确定性）检测隐私泄露，使用四种指标：层内和层间语义相似性、词级和句级概率分布。

Result: PrivacyXray在五个LLM上平均准确率达92.69%，比现有方法平均提高20.06%。

Conclusion: PrivacyXray通过合成真实隐私数据和基于LLM内部状态的检测机制，解决了隐私检测的关键挑战，具有稳定性和实用性。
...
### [370] [Decompiling Smart Contracts with a Large Language Model](https://arxiv.org/abs/2506.19624)
*Isaac David,Liyi Zhou,Dawn Song,Arthur Gervais,Kaihua Qin*

Main category: cs.CR

TL;DR: 论文提出了一种基于大语言模型（LLM）的新型反编译管道，将EVM字节码转换为可读且语义准确的Solidity代码，显著提升了区块链智能合约的安全分析能力。


<details>
  <summary>Details</summary>
Motivation: 区块链浏览器（如Etherscan）上大多数智能合约未开源，导致安全分析困难，现有反编译工具生成的代码可读性差，亟需改进。

Method: 采用静态程序分析将字节码转换为三地址码（TAC），再通过微调的Llama-3.2-3B模型生成高质量Solidity代码。

Result: 实验表明，该方法语义相似度达0.82，显著优于传统反编译工具，且代码可读性大幅提升。

Conclusion: 该研究为智能合约安全分析提供了实用工具，公开系统已上线，网址为https://evmdecompiler.com。
...
### [371] [A Survey of LLM-Driven AI Agent Communication: Protocols, Security Risks, and Defense Countermeasures](https://arxiv.org/abs/2506.19676)
*Dezhang Kong,Shi Lin,Zhenhua Xu,Zhebo Wang,Minghao Li,Yufeng Li,Yilun Zhang,Zeyang Sha,Yuyuan Li,Changting Lin,Xun Wang,Xuan Liu,Muhammad Khurram Khan,Ningyu Zhang,Chaochao Chen,Meng Han*

Main category: cs.CR

TL;DR: 本文综述了AI代理通信安全，将其生命周期分为三个阶段，分析了各阶段的安全风险及防御措施，并展望了未来研究方向。


<details>
  <summary>Details</summary>
Motivation: 随着AI代理通信成为未来生态的基础，其暴露的安全隐患可能对现实场景造成严重损害，亟需系统研究。

Method: 将代理通信生命周期分为用户-代理交互、代理间通信和代理-环境通信三阶段，分析各阶段协议及安全风险。

Result: 总结了各阶段的安全风险及可能的防御对策。

Conclusion: 讨论了开放问题和未来方向，为代理通信安全研究提供指导。
...
### [372] [KnowML: Improving Generalization of ML-NIDS with Attack Knowledge Graphs](https://arxiv.org/abs/2506.19802)
*Xin Fan Guo,Albert Merono Penuela,Sergio Maffeis,Fabio Pierazzi*

Main category: cs.CR

TL;DR: KnowML框架通过知识图谱和大型语言模型提升ML-NIDS的泛化能力，显著提高攻击变体的检测效果。


<details>
  <summary>Details</summary>
Motivation: 现有ML-NIDS依赖同质数据集和人工设计，泛化能力不足，无法有效检测多样攻击变体。

Method: 提出KnowML框架，利用LLM构建攻击策略知识图谱，并通过符号推理生成知识增强输入。

Result: 在28种攻击变体上测试，KnowML的F1分数高达99%，而基线模型部分变体检测完全失败。

Conclusion: KnowML通过知识引导显著提升ML-NIDS的检测能力，为安全领域提供新思路。
...
### [373] [Machine Learning with Privacy for Protected Attributes](https://arxiv.org/abs/2506.19836)
*Saeed Mahloujifar,Chuan Guo,G. Edward Suh,Kamalika Chaudhuri*

Main category: cs.CR

TL;DR: 论文提出了一种称为特征差分隐私（FDP）的新框架，用于在特定属性需要保护时减少差分隐私（DP）对模型效用的负面影响。


<details>
  <summary>Details</summary>
Motivation: 传统差分隐私在仅需保护特定属性时会不必要地降低模型效用，因此需要更灵活的隐私保护方法。

Method: 通过模拟定义FDP，支持增删和替换隐私变体，并允许保护和非保护特征的自适应分离。还提出了改进的DP-SGD算法以满足FDP。

Result: 在AFHQ数据集上，FDP显著提升了模型效用（FID从286.7降至101.9，ε=8）。

Conclusion: FDP为私有数据分析提供了新方法，能在保持强隐私保证的同时减少效用损失。
...
### [374] [Privacy-Preserving LLM Interaction with Socratic Chain-of-Thought Reasoning and Homomorphically Encrypted Vector Databases](https://arxiv.org/abs/2506.17336)
*Yubeen Bae,Minchan Kim,Jaejin Lee,Sangbum Kim,Jaehyung Kim,Yejin Choi,Niloofar Mireshghallah*

Main category: cs.CR

TL;DR: 提出了一种结合强大但不可信的LLM与本地弱模型的方法，通过分步查询和加密搜索保护用户隐私，性能提升7.1%。
...
Method: 采用Socratic Chain-of-Thought Reasoning，先由强大LLM生成非隐私查询，再通过加密搜索本地数据，最后由本地模型生成响应。

Result: 在LoCoMo基准测试中，结合GPT-4o与本地Llama-3.2-1B模型的框架比单独使用GPT-4o性能提升7.1%。

Conclusion: 该方法为分解任务并在不可信强LLM与本地弱模型间分配提供了初步解决方案，保护了用户隐私。
